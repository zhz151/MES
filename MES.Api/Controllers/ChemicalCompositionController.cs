...
public class ChemicalCompositionController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<ImportPreviewResult>>> Preview(IFormFile file)
    {
        ...
        var msg = $"预览完成: 共 {result.TotalRows} 行，有效 {result.ValidCount}，重复 {result.DuplicateCount}，错误 {result.ErrorCount}";
        return Ok(ApiResponse<ImportPreviewResult>.Ok(result, msg));
    }
