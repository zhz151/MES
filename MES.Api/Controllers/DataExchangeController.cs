...
public class DataExchangeController : ControllerBase
{
    ...
    public DataExchangeController(IDataExchangeService service, IDataFixService fixService, ILogger<DataExchangeController> logger)
    {
        _service = service;
    ...
    public async Task<ActionResult<ApiResponse<DataFixReport>>> FixAllSystemFields()
    {
        try
        {
            ...
            return Ok(ApiResponse<DataFixReport>.Ok(report,
                $"修复完成：组内序号 {report.SequenceNumbersFixed} 条，工段委外状态 {report.OutsourceStatusFixed} 条，批次跟踪 {report.BatchTrackingFixed} 条，采购订单 {report.PurchaseOrdersFixed} 条，委外订单 {report.SubcontractOrdersFixed} 条，设备日期 {report.EquipmentFixed} 条"));
        }
        catch (Exception ex)
        {
