...
[ApiController]
[Route("api/grain-size-test")]
[Authorize]
public class GrainSizeTestController : ControllerBase
{
    private readonly IGrainSizeTestService _service;
    ...
    public async Task<ActionResult<ApiResponse<GrainSizeTestDto>>> GetById(int id)
    {
        ...
        if (result == null) return NotFound(ApiResponse<GrainSizeTestDto>.Fail("记录不存在"));
        return Ok(ApiResponse<GrainSizeTestDto>.Ok(result));
    }
    ...
    public async Task<ActionResult<ApiResponse<GrainSizeTestDto>>> Update(int id, [FromBody] UpdateGrainSizeTestRequest request)
    {
        ...
        return Ok(ApiResponse<GrainSizeTestDto>.Ok(result, "更新成功"));
    }
    ...
    public async Task<ActionResult<ApiResponse<List<GrainSizeTestDto>>>> BatchCreate([FromBody] List<CreateGrainSizeTestRequest> requests)
    {
        ...
        if (requests == null || requests.Count == 0) return BadRequest(ApiResponse<List<GrainSizeTestDto>>.Fail("请求数据不能为空"));
        var result = await _service.BatchCreateAsync(requests);
        return Ok(ApiResponse<List<GrainSizeTestDto>>.Ok(result, "批量创建成功"));
    }
    ...
    public async Task<IActionResult> PrintBatchFile([FromBody] GrainSizeTestPrintBatchRequest request)
    {
        ...
        return File(pdfBytes, "application/pdf", "晶粒度检验-选中.pdf");
    }
    ...
    public async Task<IActionResult> PrintAllFile([FromBody] GrainSizeTestPrintAllRequest request)
    {
        ...
        return File(pdfBytes, "application/pdf", "晶粒度检验-全部.pdf");
    }
}
