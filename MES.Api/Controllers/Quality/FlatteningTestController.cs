...
[ApiController]
[Route("api/flattening-test")]
[Authorize]
public class FlatteningTestController : ControllerBase
{
    private readonly IFlatteningTestService _service;

    public FlatteningTestController(IFlatteningTestService service) => _service = service;

    [HttpGet("{id}")]
    [Authorize(Roles = Roles.Policies.QualityView)]
    public async Task<ActionResult<ApiResponse<FlatteningTestDto>>> GetById(int id)
    {
        var result = await _service.GetByIdAsync(id);
        if (result == null) return NotFound(ApiResponse<FlatteningTestDto>.Fail("记录不存在"));
        return Ok(ApiResponse<FlatteningTestDto>.Ok(result));
    }

    [HttpGet("all")]
    [Authorize(Roles = Roles.Policies.QualityView)]
    public async Task<ActionResult<ApiResponse<PagedResult<FlatteningTestDto>>>> GetAll(
    ...
    public async Task<ActionResult<ApiResponse<FlatteningTestDto>>> Update(int id, [FromBody] UpdateFlatteningTestRequest request)
    {
        ...
        return Ok(ApiResponse<FlatteningTestDto>.Ok(result, "更新成功"));
    }
    ...
    [HttpPost("batch")]
    [Authorize(Roles = Roles.Policies.QualityEdit)]
    public async Task<ActionResult<ApiResponse<List<FlatteningTestDto>>>> BatchCreate([FromBody] List<CreateFlatteningTestRequest> requests)
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse<List<FlatteningTestDto>>.Fail("请求参数无效"));
        if (requests == null || requests.Count == 0) return BadRequest(ApiResponse<List<FlatteningTestDto>>.Fail("请求数据不能为空"));
        var result = await _service.BatchCreateAsync(requests);
        return Ok(ApiResponse<List<FlatteningTestDto>>.Ok(result, "批量创建成功"));
    }
    ...
    [Authorize(Roles = Roles.Policies.QualityView)]
    public async Task<IActionResult> PrintBatchFile([FromBody] FlatteningTestPrintBatchRequest request)
    {
