...
[ApiController]
[Route("api/hardness-test")]
[Authorize]
public class HardnessTestController : ControllerBase
{
    private readonly IHardnessTestService _service;

    public HardnessTestController(IHardnessTestService service) => _service = service;

    [HttpGet("{id}")]
    [Authorize(Roles = $"{Roles.Staffs.Quality},{Roles.Directors.Quality},{Roles.Admin}")]
    public async Task<ActionResult<ApiResponse<HardnessTestDto>>> GetById(int id)
    {
        var result = await _service.GetByIdAsync(id);
        if (result == null) return NotFound(ApiResponse<HardnessTestDto>.Fail("记录不存在"));
        return Ok(ApiResponse<HardnessTestDto>.Ok(result));
    }

    [HttpGet("all")]
    [Authorize(Roles = $"{Roles.Staffs.Quality},{Roles.Directors.Quality},{Roles.Admin}")]
    public async Task<ActionResult<ApiResponse<PagedResult<HardnessTestDto>>>> GetAll(
    ...
    public async Task<ActionResult<ApiResponse<HardnessTestDto>>> Update(int id, [FromBody] UpdateHardnessTestRequest request)
    {
        ...
        return Ok(ApiResponse<HardnessTestDto>.Ok(result, "更新成功"));
    }
    ...
    [HttpPost("batch")]
    [Authorize(Roles = $"{Roles.Directors.Quality},{Roles.Admin}")]
    public async Task<ActionResult<ApiResponse<List<HardnessTestDto>>>> BatchCreate([FromBody] List<CreateHardnessTestRequest> requests)
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse<List<HardnessTestDto>>.Fail("请求参数无效"));
        if (requests == null || requests.Count == 0) return BadRequest(ApiResponse<List<HardnessTestDto>>.Fail("请求数据不能为空"));
        var result = await _service.BatchCreateAsync(requests);
        return Ok(ApiResponse<List<HardnessTestDto>>.Ok(result, "批量创建成功"));
    }
    ...
    [Authorize(Roles = $"{Roles.Staffs.Quality},{Roles.Directors.Quality},{Roles.Admin}")]
    public async Task<IActionResult> PrintBatchFile([FromBody] HardnessTestPrintBatchRequest request)
    {
        ...
        return File(pdfBytes, "application/pdf", "硬度检验-选中.pdf");
    }

    [HttpPost("print-all-file")]
    [Authorize(Roles = $"{Roles.Staffs.Quality},{Roles.Directors.Quality},{Roles.Admin}")]
    public async Task<IActionResult> PrintAllFile([FromBody] HardnessTestPrintAllRequest request)
    {
        ...
        return File(pdfBytes, "application/pdf", "硬度检验-全部.pdf");
    }
}
