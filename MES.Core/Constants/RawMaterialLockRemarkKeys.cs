...
/// 显示层使用中文（质量补料/生产返整补足/执行用料计划/完善用料计划）。由 WorkOrderExecutionService
...
{
    /// <summary>质量补料：投料不满足且附返整不满足，连返整量算上仍不足，真缺料需补料</summary>
    public const string QualityReplenish = "QualityReplenish";

    /// <summary>生产返整补足：投料满足且附返整满足，缺口可由返整量补齐、处于返整执行</summary>
    public const string ExecuteRework = "ExecuteRework";

    /// <summary>执行用料计划：投料不满足但计划状态满足/超量，需执行用料计划投料</summary>
    public const string ExecutePlan = "ExecutePlan";

    /// <summary>完善用料计划：投料不满足且用料计划不完善，需完善用料计划</summary>
    ...
    public static readonly IReadOnlyDictionary<string, string> KeyToChinese =
        ...
        {
            [QualityReplenish] = "质量补料",
            [ExecuteRework] = "生产返整补足",
            [ExecutePlan] = "执行用料计划",
            [ImprovePlan] = "完善用料计划",
        };

    /// <summary>规范中文 → Key（迁移前存量归一用；含旧前缀名兼容）</summary>
    public static readonly IReadOnlyDictionary<string, string> ChineseToKey =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ["质量补料"] = QualityReplenish,
            ["生产返整补足"] = ExecuteRework,
            ["执行用料计划"] = ExecutePlan,
            ["完善用料计划"] = ImprovePlan,
            // 旧前缀名兼容（2026-09-07 更名前的 A质量补料/B执行返整/C执行计划/D完善计划）
