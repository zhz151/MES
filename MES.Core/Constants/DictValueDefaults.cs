...
/// 覆盖 string 字典 Keys 常量类（紧急度/产类/流转/关注目标/汇总行/责任类别/NCR责任类别/原锁备注/生产关注/岗位/岗位类别/NCR处置方式），
...
public static class DictValueDefaults
{
    ...
    /// <summary>NCR 处置方式（NcrDisposalKeys，不合格品处置方式字典化，可扩展档位）</summary>
    public const string NcrDisposalKey = "NcrDisposalKey";
    ...
    /// <summary>全量：DictKey → Value → DisplayName（工段/工序专门表 + 12 个可配置字典全部内置值）</summary>
    public static readonly IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> All =
        new Dictionary<string, IReadOnlyDictionary<string, string>>(StringComparer.Ordinal)
        {
            ...
            [NcrDisposalKey] = Copy(NcrDisposalKeys.KeyToChinese),
        };
