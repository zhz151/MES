...
public class MaterialPlanLinkFilterDto
{
    ...
    /// <summary>
    /// 「错误-用料计划及其执行」卡片联动：主号-关注档位（1=主号完成 3=生产执行 4=成品检验）。
    /// 与下方 HasMissingWeight/HasPendingInputWeight 组合生效；设置时忽略 Remark/Urgency/PurchaseOnly/ExcludeSingleFinishPurchase。
    /// </summary>
    public int? ScheduleStage { get; set; }

    /// <summary>「错误-用料计划及其执行」联动：要求 理论原料未至（TotalMissingWeight &gt; 0，3% 门槛口径）</summary>
    public bool HasMissingWeight { get; set; }

    /// <summary>「错误-用料计划及其执行」联动：要求 工单到料未投（PendingInputWeight &gt; 0）</summary>
