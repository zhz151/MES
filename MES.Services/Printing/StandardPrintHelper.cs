...
/// <summary>
/// 产品标准 PDF 打印模板（QuestPDF）
/// </summary>
public static class StandardPrintHelper
{
    public static byte[] GeneratePdf(ProductionStandardDto standard)
    {
        return GenerateBatchPdf(new List<ProductionStandardDto> { standard });
    }

    public static byte[] GenerateBatchPdf(List<ProductionStandardDto> standards)
    {
        return Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Element(h => ComposeHeader(h, "产 品 标 准 列 表"));

                page.Content().Element(c => ComposeContent(c, standards));
    ...
    private static void ComposeContent(IContainer container, List<ProductionStandardDto> standards)
    {
        container.Table(table =>
        {
            table.ColumnsDefinition(columns =>
            {
                ...
                columns.ConstantColumn(100);  // 标准编码
                columns.RelativeColumn();     // 标准名称
            ...
            table.Header(header =>
            {
                string[] headers = { "序号", "标准编码", "标准名称", "状态", "备注" };
            ...
            foreach (var s in standards)
            {
                ...
                table.Cell().Element(CellStyle).Text(s.StandardCode).FontSize(8).AlignCenter();
                table.Cell().Element(CellStyle).Text(s.StandardName).FontSize(8);
