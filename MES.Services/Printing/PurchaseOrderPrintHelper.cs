    ...
    private static void ComposeContent(IContainer container, List<PurchaseOrderDto> orders)
    {
        container.Table(table =>
        {
            table.ColumnsDefinition(columns =>
            {
                ...
                columns.ConstantColumn(38);   // 投料制成倍
            ...
            table.Header(header =>
            {
                string[] headers = { "序号", "采购单号", "下单日期", "来源工单号", "物料分类", "厂内钢种", "规格",
                    "单支重量", "支数", "投料制成倍", "采购重量", "要求到货日", "供应商", "状态", "已到货" };
            ...
            foreach (var o in orders)
            {
                ...
                table.Cell().Element(CellStyle).Text(EnumHelper.GetDisplayName(o.MaterialCategory)).FontSize(7);
