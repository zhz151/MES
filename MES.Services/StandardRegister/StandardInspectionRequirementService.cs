...
public class StandardInspectionRequirementService : IStandardInspectionRequirementService
{
    private readonly AppDbContext _context;

    public StandardInspectionRequirementService(AppDbContext context) => _context = context;

    public async Task<PagedResult<StandardInspectionRequirementDto>> GetPagedAsync(QueryParams query)
    {
        var queryable = _context.StandardInspectionRequirements
            .AsNoTracking()
            .AsQueryable();
        ...
        var items = await queryable
            .Skip(query.Skip)
            .Take(query.PageSize)
            .Select(x => new StandardInspectionRequirementDto
            {
                Id = x.Id,
        ...
        return new PagedResult<StandardInspectionRequirementDto>
        {
            Items = items,
    ...
    public async Task<StandardInspectionRequirementDto?> GetByIdAsync(int id)
    {
        var entity = await _context.StandardInspectionRequirements
            .AsNoTracking()
            .FirstOrDefaultAsync(x => x.Id == id);
        if (entity == null)
            throw new BusinessException("标准号检验项要求记录不存在");
        return ToDto(entity);
    }

    public async Task<StandardInspectionRequirementDto> CreateAsync(CreateStandardInspectionRequirementRequest request)
    {
        var exists = await _context.StandardInspectionRequirements
            .AnyAsync(x => x.StandardNo == request.StandardNo);
        if (exists)
            throw new BusinessException($"标准号 '{request.StandardNo}' 已存在");

        var entity = new StandardInspectionRequirement
        {
        ...
        _context.StandardInspectionRequirements.Add(entity);
        await _context.SaveChangesAsync();
        return ToDto(entity);
    }

    public async Task<StandardInspectionRequirementDto> UpdateAsync(int id, UpdateStandardInspectionRequirementRequest request)
    {
        var entity = await _context.StandardInspectionRequirements
            .FirstOrDefaultAsync(x => x.Id == id);
        if (entity == null)
    ...
    public async Task DeleteAsync(int id)
    {
        ...
        if (entity == null)
            throw new BusinessException("标准号检验项要求记录不存在");
        _context.StandardInspectionRequirements.Remove(entity);
        await _context.SaveChangesAsync();
    }
    ...
    public async Task<byte[]> PrintBatchAsync(int[] ids, List<PrintColumnDef> columns)
    {
        ...
        return StandardInspectionRequirementPrintHelper.GenerateBatchPdf(selected, columns);
    }
    ...
    public async Task<byte[]> PrintAllAsync(string? keyword, string? sortBy, bool isDescending, List<PrintColumnDef> columns)
    {
        ...
        var result = await GetPagedAsync(query);
        return StandardInspectionRequirementPrintHelper.GenerateBatchPdf(result.Items, columns);
    }

    public async Task<Dictionary<string, List<string>>> GetFilterContextsAsync()
    {
        var all = await _context.StandardInspectionRequirements
            .AsNoTracking()
            .ToListAsync();
    ...
    private static StandardInspectionRequirementDto ToDto(StandardInspectionRequirement e) => new()
    {
        Id = e.Id,
