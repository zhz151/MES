...
public class NcrService : INcrService
{
    ...
    public async Task<Dictionary<string, List<string>>> GetFilterContextsAsync()
    {
        ...
        // 枚举列
        results["pipecategory"] = await queryable
            .Select(r => r.PipeCategory.ToString())
        ...
        // 字符串列
        results["reportdepartment"] = await queryable
            .Where(r => r.ReportDepartment != null)
            .Select(r => r.ReportDepartment!)
            .Distinct().ToListAsync();
        results["plantgrade"] = await queryable
            .Where(r => r.PlantGrade != null)
            .Select(r => r.PlantGrade!)
            .Distinct().ToListAsync();
        results["specification"] = await queryable
            .Where(r => r.Specification != null)
            .Select(r => r.Specification!)
            .Distinct().ToListAsync();
        results["disposalremark"] = await queryable
            .Where(r => r.DisposalRemark != null)
            .Select(r => r.DisposalRemark!)
            .Distinct().ToListAsync();
        results["analysisconfirmer"] = await queryable
            .Where(r => r.AnalysisConfirmer != null)
            .Select(r => r.AnalysisConfirmer!)
            .Distinct().ToListAsync();
        results["responsibledept"] = await queryable
            .Where(r => r.ResponsibleDept != null)
            .Select(r => r.ResponsibleDept!)
            .Distinct().ToListAsync();
        results["actionplanner"] = await queryable
            .Where(r => r.ActionPlanner != null)
            .Select(r => r.ActionPlanner!)
            .Distinct().ToListAsync();
        results["actionverifier"] = await queryable
            .Where(r => r.ActionVerifier != null)
            .Select(r => r.ActionVerifier!)
            .Distinct().ToListAsync();
