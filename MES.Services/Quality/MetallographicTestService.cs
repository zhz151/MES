...
public class MetallographicTestService : IMetallographicTestService
{
    ...
    {
        return await _cache.GetOrCreateAsync(CacheKeys.MetallographicTestFilterContexts, async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDefaults.MemoryCacheExpiry;

            var all = await _context.MetallographicTests
                .AsNoTracking()
                .Select(r => new { r.Inspector, r.BatchNo, r.Grade, r.Specification, r.InspectionStandard, r.EtchingMethod, r.Magnification, r.Judgment, r.InspectionDate })
                .ToListAsync();
