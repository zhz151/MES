...
public class SubcontractOrderService : ISubcontractOrderService
{
    ...
    public SubcontractOrderService(AppDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<PagedResult<SubcontractOrderDto>> GetPagedAsync(SubcontractQueryParams query)
    {
        ...
        if (!string.IsNullOrEmpty(query.Keyword))
        {
            ...
            var matchedSupplierIds = await _context.SupplierProfiles
                .AsNoTracking()
                .Where(s => !s.IsDeleted && s.SupplierName.Contains(kw))
                .Select(s => s.Id)
                .ToListAsync();
        ...
        var items = await queryable
            ...
            .Select(s => new SubcontractOrderDto
            {
                ...
                InWeight = s.InWeight,
                SourceWorkOrderNo = s.SourceWorkOrderNo,
                Remark = s.Remark,
    ...
    public async Task<SubcontractOrderDto> GetByIdAsync(int id)
    {
        var entity = await _context.SubcontractOrders
            .AsNoTracking()
            .Include(s => s.ReturnItems.Where(r => !r.IsDeleted).OrderBy(r => r.Sequence))
    ...
    public async Task<SubcontractOrderDto> UpdateAsync(int id, UpdateSubcontractOrderRequest request)
    {
        ...
        var supplier = await _context.SupplierProfiles.FindAsync(entity.SupplierId);
        if (supplier != null) dto.SupplierName = supplier.SupplierName;
        dto.ReturnItems = entity.ReturnItems.Where(r => !r.IsDeleted).OrderBy(r => r.Sequence).Select(r => new SubcontractReturnItemDto
        {
            Id = r.Id,
            SubcontractOrderId = r.SubcontractOrderId,
            Sequence = r.Sequence,
    ...
    public async Task SyncAllAsync()
    {
        var orders = await _context.SubcontractOrders
            .Where(s => !s.IsDeleted && s.Status != "Cancelled" && s.Status != "Completed")
            .ToListAsync();

        var orderNos = orders.Select(o => o.OrderNo).ToList();
        var batches = await _context.InventoryBatches
            .AsNoTracking()
            .Where(b => !b.IsDeleted && b.SourceOrderNo != null && orderNos.Contains(b.SourceOrderNo))
            .ToListAsync();
    ...
    public async Task SyncSingleAsync(int id)
    {
        ...
        var batches = await _context.InventoryBatches
            .AsNoTracking()
            .Where(b => !b.IsDeleted && b.SourceOrderNo == order.OrderNo)
            .ToListAsync();
    ...
    private async Task<string> GenerateOrderNoAsync()
    {
        ...
        var prefix = $"WW{today}";
        var last = await _context.SubcontractOrders
            .Where(s => s.OrderNo.StartsWith(prefix))
            .OrderByDescending(s => s.OrderNo)
            .Select(s => s.OrderNo)
            .FirstOrDefaultAsync();

        int seq = 1;
        if (last != null && int.TryParse(last[^3..], out var lastSeq))
            seq = lastSeq + 1;
