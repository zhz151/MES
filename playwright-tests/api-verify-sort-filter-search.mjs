...
// ============================================================
// Controller 映射表（来自 Controller 静态扫描）
// ============================================================
const CONTROLLERS = [
  // Batch
  ...
  // Configuration — 无 filter-contexts
...
// ============================================================
// 从 verify-sort-keys.mjs 提取的已知 SortKey（用于排序测试）
// ============================================================
const SORT_KEYS = {
  'Certificate':  ['certificateno', 'customername', 'issuedate', 'productstandard', 'productname', 'deliverystatus'],
  'Order':        ['ordernumber', 'signdate', 'status', 'customername', 'totalcontractweight'],
};
...
async function testBasicList(ctrl) {
  ...
  // 检查是否有 data 字段（可能是 PagedResult 或 数组）
  const hasData = body.data !== undefined && body.data !== null;
  if (!hasData) return { pass: false, status, error: '响应缺少 data 字段', url };
...
async function testSort(ctrl) {
  ...
  const results = [];
  for (const key of sortKeys.slice(0, 3)) { // 最多测试 3 个 SortKey
...
async function main() {
  ...
  for (const ctrl of CONTROLLERS) {
    ...
    // 实时输出
    const status = listRes.pass ? '✓' : '✗';
    const sortMark = sortRes.pass === true ? 'S✓' : (sortRes.pass === null ? 'S—' : 'S✗');
    const searchMark = searchRes.pass ? 'Q✓' : 'Q✗';
    const filterMark = filterRes.pass ? 'F✓' : 'F✗';
    const fcMark = fcRes.pass === true ? 'C✓' : (fcRes.pass === null ? 'C—' : 'C✗');
    const failDetails = [];
    if (!listRes.pass) failDetails.push('LIST');
    if (sortRes.pass === false) failDetails.push('SORT');
    if (!searchRes.pass) failDetails.push('SEARCH');
    if (!filterRes.pass) failDetails.push('FILTER');
    ...
    const items = listRes.pass && listRes.body?.data?.items
      ? ` (${listRes.body.data.items.length} items, total=${listRes.body.data.totalCount})`
      : '';
  ...
  for (const [category, label] of [['list', '基础列表'], ['sort', '排序'], ['search', '搜索'], ['filter', '筛选'], ['fc', '筛选上下文']]) {
    ...
    if (r.fail.length > 0) {
      ...
      for (const f of r.fail) {
        const urlStr = f.url ? f.url.substring(0, 100) : 'N/A';
