...
/// 工序/产类/阶段三约束存关系成员子表 PieceRateProductionCategoryKey（0 行=该维全选，
/// 禁止显式全列表与空并存，见 ConstraintKeys）。
/// </summary>
public class PieceRateProductionCategory : BaseEntity
{
    ...
    /// <summary>约束集合成员行（子表，级联删除；按 ConstraintType 分 Process/ProductStatus/Stage，0 行=该维全选）</summary>
    public List<PieceRateProductionCategoryKey> ConstraintKeys { get; set; } = new();
