...
| 核心上下文 | 工单上下文 | 生产工单、主号/次号工单号规则、定尺工单（FixedLengthWorkOrder）联通视图、6类用料计划、工单执行状况读模型 | ✅ 已实现 |
...
| 核心上下文 | 计划排程上下文 | 订单总况、原锁计划、工单排程、生产工段待产量现况、生产段流转量分析、批次计划、冷轧计划、冷轧排程、**成检计划**（工单需求调整页面已迁移至工单上下文）；读模型「主号关注」（ScheduleStage 五档全主号级）、「主号计划性」（UrgencyLevel） | **✅ V4.1 已完成** |
| **支撑上下文** | **配置上下文** | **11 个参数表：标准工量天数、交货状态附加天数、业务参数配置、日产能预估、重点工段日产、日流转量配置、工位管理、员工管理、枚举显示配置（EnumDisplayDefinition）、字典显示配置（DictValueDefinition）、工序组定义（ProcessDefinition，含默认工段 DefaultSections）** | **✅ V3.4 已完成** |
...
| NotificationType | 通知类型（字符串存储，7 个值：NewMaterial/DeleteBlocked/OutboundAlert/WorkOrderDeleted（已废弃，仅兼容存量数据）/OrderDeleted/OrderChanged/WorkOrderChanged） |
...
| ResponsibilityCategory | 责任类别：ProductionInternal（生产厂内）, ProductionOutsource（生产外协）, MaterialTubeBlank（原料荒管）, MaterialPurchased（原料外购）, MaterialSurplus（原料余库） |
