...
**版本：V4.4**
**最后更新：2026-09-16**
> 本次变更（V4.4）：一级菜单「**扫码管理**」更名「**扫码操作**」并下沉至「系统参数」之后（纯显示名 + 菜单位置；`Scan*` 角色代码 / 组 `Policy=null` / 叶子名与路由不变）。
> 历史变更（V4.3）：菜单显示名同步 —— 计划排程组「批次计划」更名「生产计划」、一级菜单「生产标准」更名「产品标准」（含上下文文档改名）。仅显示名变更，实体/表名/路由/角色代码不变。
**适用范围：不锈钢无缝钢管MES系统**
...
| SignalR | 看板实时推送 | V2.0 |
| QRCoder | 二维码生成 | ✅ 已实现（扫码操作 + 工艺流转卡/工位/员工二维码） |
...
| 核心上下文 | 产品标准上下文 | 标准号、牌号对照、标准牌号化学成分、工厂牌号化学成分、工厂牌号化分验证规则、牌号物理性能、子标准速览、标准号检验项要求 | ✅ 已实现 |
...
| 核心上下文 | 计划排程上下文 | 订单总况、原锁计划、工单排程、生产计划、冷轧计划、冷轧排程、**成检计划**（工单需求调整页面已迁移至工单上下文；工段待产量/段落流转分析查询经生产计划页折叠展示与报表总览消费）；读模型「主号关注」（ScheduleStage 五档全主号级）、「主号计划性」（UrgencyLevel） | **✅ 已实现** |
| 核心上下文 | 工资结算上下文 | 员工薪酬配置固定化、考勤表、生产/成检计件类别与结算采集、每日工资、集体月结、靠工月结、杂辅工、津贴与处罚、月工资汇总（DataTool 8 表归「工资」上下文）。⚠️ 2026-09-16：生产/成检计件标准两页已随新一级菜单「基础资料」迁出（档位 `BasicData*`），**实体/Service/Controller/路由仍在工资结算命名空间，上下文边界不变** | ✅ 已实现 |
| **支撑上下文** | **配置上下文** | **11 个参数表：工段工量天数、交货状态附加天数、系统参数、规格日产预估、重点工段日产、段落日产配置（SectionParagraphConfig，3 类配置驱动自动生成：冷轧拔/普通工段/检验，段落仅参数可编辑）、工位管理、员工管理、枚举显示配置（EnumDisplayDefinition）、字典显示配置（DictValueDefinition）、工序组定义（ProcessDefinition，含默认工段 DefaultSections）**。⚠️ 2026-09-16 菜单归属：工位管理/员工管理 2 表 →「**基础资料**」一级菜单（档位 `BasicData*`），其余 9 表 →「**系统参数**」菜单（原「参数表」，更名后角色代码 `Configuration*` 不变） | **✅ 已实现** |
...
| FlowDirection | 不合格品流向（5 档，由过程检/成品检记录带出，只读）：Rework（返整(新卡流转)）, InProcessWarehouse（入在制库(可改制)）, FinishedWarehouse（可入备库(尺寸偏差)）, Scrap（入次品库(报废)）, Return（退货） |
| 处置方式（字典） | 已并入字典 DictValueDefinitions（DictKey=NcrDisposalKey，可加值；内置 8 档：Concession（让步放行）, Reprocess（返工）, Rework（返整(新卡流转)）, InProcessWarehouse（入在制库(可改制)）, FinishedWarehouse（可入备库(尺寸偏差)）, ScrapCorrection（入次品库(修正改制)）, Scrap（入次品库(报废)）, Return（退货））；让步放行 + 返工由生产/检验工段操作前预先判定（不产生流向） |
...
>
> **枚举/字典显示参数化**：所有枚举类字段与 string 字典字段的中文显示统一走配置表（EnumDisplayDefinition 41 枚举 / DictValueDefinition 12 字典）优先、静态 Helper 兜底；int 档位字段统一走 `IntStatusDisplayHelper`（存储值→中文单一出口）；前端下拉走 `GetEnumOptions`/`GetEnumFilterOptions`，禁止散落 switch。
