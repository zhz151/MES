...
| **支撑上下文** | **配置上下文** | **13 个参数表：标准工量天数、交货状态附加天数、业务参数配置、日产能预估、重点工段日产、流转类别日产配置（SectionFlowCategorySettings）、段落日产配置（SectionParagraphConfig）、组合归类表（CombinationGroup）、工位管理、员工管理、枚举显示配置（EnumDisplayDefinition）、字典显示配置（DictValueDefinition）、工序组定义（ProcessDefinition，含默认工段 DefaultSections）** | **✅ V3.4 已完成** |
