...
| 核心上下文 | 计划排程上下文 | 订单总况、原锁计划、工单排程、批次计划、冷轧计划、冷轧排程、**成检计划**（工单需求调整页面已迁移至工单上下文；工段待产量查询页面已从菜单栏移除；工段流转分析组件全套删除；段落流转分析查询页面已从菜单栏移除）；读模型「主号关注」（ScheduleStage 五档全主号级）、「主号计划性」（UrgencyLevel） | **✅ V4.1 已完成** |
| **支撑上下文** | **配置上下文** | **11 个参数表：工段工量天数、交货状态附加天数、系统参数、规格日产预估、重点工段日产、段落日产配置（SectionParagraphConfig，3 类配置驱动自动生成：冷轧拔/普通工段/检验，段落仅参数可编辑）、工位管理、员工管理、枚举显示配置（EnumDisplayDefinition）、字典显示配置（DictValueDefinition）、工序组定义（ProcessDefinition，含默认工段 DefaultSections）** | **✅ V3.4 已完成** |
...
| **V2.4** | — | **（版本记录缺失，压缩为一行摘要，git log 可溯）** |
