...
| scanner.js + BarcodeDetector/jsQR | 手机摄像头扫码组件（支持条码+二维码） | ✅ V2.0 |
| SignalR | 看板实时推送 | V2.0 |
...
| 核心上下文 | 工单上下文 | 生产工单 |
| 核心上下文 | 仓库上下文 | 库存管理 | ✅ 已实现 |
| 核心上下文 | 物料上下文 | 物料采购、委外、档案 | ✅ 已实现 |
| 核心上下文 | 批次上下文 | 生产批次、流转卡 | ✅ 已实现 |
...
| 核心上下文 | 计划排程上下文 | 订单总况、原锁计划、工单排程、生产工段待产量现况、生产段流转量分析、批次计划、冷轧计划、冷轧排程、**成检计划**（工单需求调整页面已迁移至工单上下文） | **✅ V4.1 已完成** |
| **支撑上下文** | **配置上下文** | **8 个参数表：标准工量天数、交货状态附加天数、业务参数配置、日产能预估、重点工段日产、日流转量配置、工位管理、员工管理** | **✅ V3.4 已完成** |
...
| MaterialName | 物料名称：WeldedPipe, SeamlessPipe |
| DeliveryState | 交货状态：固溶酸洗、光亮、硬态等 |
| RequirementType | 产品要求类型：Normal, Special |
| NotificationType（工单通知） | 通知类型字符串：WorkOrderDeleted/OrderDeleted/OrderChanged |
| NotificationChangeType | 订单变更通知类型（仅 OrderChangeNotificationDto 展示使用，新通知统一使用 NotificationType 字符串） |
| MaterialPlanStatus | 用料计划状态：NotPlanned/Partial/TheoreticalSatisfied/Satisfied/Excess |
| InventoryPlanStatus | 库存计划状态：Planned/Confirmed/Cancelled |
| ReworkType | 库料改制类型：EmptyDrawing/FewerPass/ManualSelect |
| MaterialType | 物料类型（15种）：Finished（备料成品）、OrderFinished（订单成品）、CriticalFinished（临界成品）、Surplus（余库料）、SemiFinished（半成品）、DefectSemi（次品半成品）、DefectFinished（次品成品）、RoughTube（荒管）、RoundBar（圆棒）、DefectRoundBar（次品圆棒）、DefectRoughTube（次品荒管）、Scrap（报废品）、SpecialDeliveryStatus（特定交态成品）、WorkInProgress（在制品）、DefectWIP（次品在制） |
| FinishedProductType | 成品类型：Critical/Order |
| OutboundType | 出库类型（8种） |
...
| NotificationType | 通知类型（字符串存储，实际使用 WorkOrderDeleted/OrderDeleted/OrderChanged） |
