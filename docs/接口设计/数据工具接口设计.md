...
**本次变更（V7.21）**：注册完整性审计与补登记 —— ① **补注册 7 个业务实体**（84 → **91**）：`Certificate`（质量-质量证明书）、`CertificateItem`（质量-质量证明书附表）、`WorkOrderPlan`（工单-工单计划）、`RawMaterialLockPreExecution`（工单-原锁预执行）、`InMainWorkOrderPlan`（工单-在产主工单计划）、`BatchPlanSchedule`（批次-批次计划）、`ColdRollSpecSchedule`（批次-冷轧规格排程，复合键 `ProcessType + BilletSpec + RollingSpec`）；② **Ncr 补「来源反馈单ID」FK 列**（`NonconformingFeedbackId` ↔ NonconformingFeedback.Id，列数 40 → 41，**旧模板须重新下载**）；③ **SectionOutsource 补「计价单位 / 单价 / 总金额」三列**；④ **NonconformingFeedback / InspectionPatrol 的「产类」加 `isSystem: true`**（服务端按批次计算，仅导出不可导入）；⑤ **修复「先删后插」实体「导出后原样回导」静默丢数据 bug**（带旧 ID 行降级为新增 + 全部行失败整体回滚，`HasRolledBack = true`）。`EntityOrder` 同步插入新实体的依赖位置。
