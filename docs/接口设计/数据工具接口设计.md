...
| 5 | OrderDemandAdjustment | 工单-工单需求调整 | 无单键 | 催单/分批交货/暂停标记，WorkOrderId 唯一 |
| 6 | Material | 物料-物料 | 复合键（MaterialCategory+PlantGrade+Specification） | 物料分类基础档案 |
| 7 | PurchaseOrder | 物料-采购订单 | 单键（OrderNo） | 依赖供应商档案 |
...
| 仓库-库存批次 | 入库来源 | InboundSource | 外购、委外、退货入库、生产入库、检验入库、移库入库、其它 |
| 仓库-库存批次 | 制造状态 | DeliveryState | 同上（11种） |
| 仓库-出库记录 | 出库类型 | OutboundType | 生产领料、销售出库、退货出库、委外加工、报废出库、检验领用、移库出库、其他出库 |
...
**工单需求调整（OrderDemandAdjustment）**：工单号(FK→工单)、催单(bool)、分批交货(bool)、暂停(bool)、调整备注
- 无单键（WorkOrderId 唯一）

### 批次6：物料

**物料（Material）**：物料编码(系统)、物料分类、厂内钢种、名义规格、是否启用、备注
- 复合主键：物料分类 + 厂内钢种 + 名义规格
- 说明：`物料编码` 为系统自动编码，导入时跳过写入
...
**出库记录（OutboundRecord）**：批次号(FK→库存批次)、出库类型(OutboundType枚举)、物料单号、目标单位、出库支数、出库重量(kg)、出库米数(m)、出库日期、备注
