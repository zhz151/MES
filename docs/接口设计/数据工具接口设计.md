...
0. **工序组工段步骤号连续化**（V7.11）：按批次将工序组全部非空工段步骤号重排为 1..N 连续（仅补缺号、不改变相对执行顺序），修复前端手填导致的缺号/孤立号（FlowJudgment 规则2 依赖连续性）
1. **组内序号**：修正 5 类记录（ProductionRecord/ProcessInspection/SectionOutsource/PicklingInRecord/MaterialReceiveCheck）的 `SequenceNumber`（语义A=工段步骤号）。前 3 类按"批次号+工序名称+制造规格+工段名称"匹配工序组并修正 `ProcessGroupId`；入缸按 `ProcessGroupId` 直接定位取 `GetSectionSequence`；成检到料取 `pg.Inspection`。**V7.11 扩展覆盖入缸与成检到料**
...
| 版本 | 日期 | 说明 |
|------|------|------|
| **V7.15** | **2026-09-04** | **月工资汇总补注册（全仓 79→80）**：新增 PayrollMonthlySummaryRecord（工资-月工资汇总）——员工某结算月完整应发/实发快照（整月替换、每人每月一行），**复合键 EmployeeId+Year+Month** 对齐唯一索引 UK_PayrollMonthlySummary_Employee_Month；列 员工工号 FK（→Employee.Code）/结算年/结算月/出勤天数 + 本月基础工资 BaseWage + 本月杂辅工资 MiscWorkAmount + 9 津贴·扣减列（岗位补贴 PositionAllowance·工龄奖 SeniorityBonus·满勤奖 FullAttendanceBonus·带班费 LeadBonus·夜班津贴 NightShiftAllowance·高温费 HighTempAllowance·工伤补贴 InjurySubsidy·处罚 Penalty·代缴社保 SocialSecurity，**处罚/代缴列存负**）+ 应发工资及津贴 TotalPayable + 实发工资及津贴 TotalPaid；DisplayName「工资-月工资汇总」归工资上下文（工资上下文 7→8 张完整在册）；概述/总结计数校准 **80**；测试 `Registry_包含所有N个实体` 计数 79→80（方法改名 `Registry_包含所有80个实体`） |
| **V7.14** | **2026-09-03** | **工资上下文补注册 2 张（全仓 77→79）**：补齐工资结算最后 2 张缺口——PayrollMiscWorkRecord（工资-杂辅工记录：**无业务键**→按无键追加式注册，列 员工工号 FK/日期 WorkDate/内容 Content/小时数 Hours/杂辅工资 Amount/备注，带 ID 下载文件按 ID 精确覆盖、无 ID 模板新增即插入，同 ProductionRecord/Ncr）与 PayrollAllowanceRecord（工资-津贴与处罚：**复合键 EmployeeId+Year+Month** 对齐唯一索引 UK_PayrollAllowance_Employee_Month，列 员工工号 FK/结算年/结算月 + 9 金额列 满勤奖·工龄奖·夜班津贴·岗位补贴·高温费·工伤补贴·带班费·处罚·代缴社保）；工资上下文 5→7 张完整在册、DisplayName 均「工资-xxx」归工资上下文；概述/总结计数校准 **79**、ContextOrder 清单补「工资」（…配置→扫码→工资→系统）；对应测试 `Registry_包含所有N个实体` 计数 77→79（DataExchangeServiceTests 210 过） |
| **V7.13** | **2026-09-03** | **每日工资表补注册（全仓 74）**：新增 PayrollDailyWageRecord（工资-每日工资表）——每日工资两表（非计件工资/个人计价工资）保存快照，复合键 EmployeeId+WageDate，列：员工工号(FK→Employee)、日期、每日工资 Amount decimal、计薪模式快照 SalaryMode（isEnum，SalaryMode 枚举名）、备注；DisplayName「工资-每日工资表」与考勤表同归工资上下文；概述/总结计数校准 74；对应测试 `Registry_包含所有N个实体` 计数同步 |
| **V7.12** | **2026-09-03** | **配置/字典 6 表补注册（全仓 73）**：此前计件单表移除等净 67，本版补齐 6 张参数/字典表——ProcessDefinition（配置-工序组定义，单键 ProcessKey）、ColdRollCapacity（配置-冷轧产能档案，复合键 ProcessType+BilletSpec+RollingSpec+IsFinished）、ColdRollMachineConfig（配置-冷轧机台数配置，单键 ProcessType）、ColdRollMachineGroupConfig（配置-冷轧机台组配置，单键 GroupKey）、EnumDisplayDefinition（配置-枚举显示配置，复合键 EnumKey+Value）、DictValueDefinition（配置-字典显示配置，复合键 DictKey+Value）；DisplayName 均「配置-xxx」归配置上下文，bool 列走「是/否」转换；§2.1/§6 同步补 6 条目、概述/总结计数校准 73；测试 `Registry_包含所有N个实体` 计数同步（DataExchangeServiceTests 193 过） |
| **V7.11** | **2026-08-28** | **一键修复扩展：工序组工段步骤号连续化 + 序号对齐覆盖入缸/成检到料**：①`DataFixService` 新增 `FixProcessGroupSectionNumbersAsync`，按批次将工序组全部非空工段步骤号重排为 1..N 连续（仅补缺号、不改相对顺序，先连续化再记录对齐），`DataFixReport` 新增 `ProcessGroupSectionNumbersFixed`；②`FixSequenceNumbersAsync` 由 3 类扩展为 5 类记录——入缸按 `ProcessGroupId` 直接定位取 `GetSectionSequence`、成检到料取 `pg.Inspection`；③§3.6 响应示例/修复范围同步更新 |
| **V7.10** | **2026-08-26** | **权限描述修正 + 显示名称对齐 + 幽灵列删除**：功能定位/§3 权限由「仅管理员」修正为数据工具三档角色（实体列表/导出/模板/预览=`DataToolView`、导入=`DataToolEdit`、一键修复=`DataToolDelete`）；§2.1/§2.2 显示名称对齐 V2.16（工位/员工归「扫码」、ConfigParameter 归「系统-系统参数」、7 张排程/流转配置表归「配置」、OrderDemandAdjustment「工单-需求调整」）；§6 幽灵列删除（SalesOrder 客户编码FK、SubcontractOrder 来源工单号、MaterialReceiveCheck 9 列、FinalInspection 8 列+补定尺切割匹配 CutLengthMatchType 枚举、InProcessReworkPlan 4 列）；物料名称枚举修正 PipeManufacturingType |
...
| **V6.10** | **2026-07-08** | **上下文迁移**：ChemicalComposition（工厂牌号化学成分）和 ChemicalValidationRule（工厂牌号化分验证规则）从质量管理上下文迁移至生产标准上下文；显示名从"质量-"改为"标准-" |
