...
**功能定位**：跨上下文的通用数据工具，支持 84 个实体的 Excel 导入导出。端点权限按数据工具三档角色控制：实体列表/导出/模板/预览=`DataToolView`、导入=`DataToolEdit`、一键修复=`DataToolDelete`（策略尾部均含 Admin 隐式全权）。
...
```yaml
已定义内容:
  - 84个实体注册定义 ✅（含9批依赖顺序、枚举字段一览、完整列定义）
