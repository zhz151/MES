...
| 1 | Workstation | 工位管理 | 单键（Code） | 扫码报工工位定义（工段+设备+报工模板类型） |
| 1 | Employee | 员工管理 | 单键（Code） | 员工基础信息（扫码报工和工资结算基础） |
| 1 | ConfigParameter | 系统参数(全局参数) | 复合键（Category+ParamKey） | 业务参数配置（数值型，分类代码+参数键唯一） |
...
| 1 | StandardWorkDay | 工段工量天数(排程/用料) | 无单键 | 各工段标准天数配置，支持牌号前缀覆盖 |
| 1 | StandardWorkDayDeliveryState | 交货状态附加天数(排程/用料) | 无单键 | 各交货状态附加天数配置，含默认值 |
| 1 | DailyOutputEstimate | 规格日产预估(工单执行) | 无单键 | 按成品外径区间设定日产能（吨/天） |
| 1 | DailyProductionCapacity | 重点工段日产(生产总览) | 无单键 | 按重点工段设定日产能力 |
| 1 | SectionFlowCategorySetting | 流转类别日产配置(流转分析) | 单键（CategoryName） | 流转类别日产配置（每类别一行；组合明细由组合段落归类 CombinationGroup 承载） |
| 1 | SectionParagraphConfig | 段落日产配置(段落流转) | 单键（ParagraphName） | 段落日产配置（独立配置表，段落类别唯一） |
| 1 | CombinationGroup | 组合段落归类(段落流转) | 复合键（ProcessGroupName+SectionName+ProductStatus） | (工序组,工段,产类)三维组合，归属流转类别/段落 |
