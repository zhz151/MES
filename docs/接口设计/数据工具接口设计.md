...
**在产改制计划（InProcessReworkPlan）**：所属工单ID(FK→工单)、计划日期、生产批次ID(FK→生产批次)、批次号、挂牌号、物料名称(PipeManufacturingType枚举)、工厂牌号、规格、长度状态(LengthStatus枚举)、批次总支数、批次总重量、现有有效原料支数、现有有效原料重量、投料制成倍、使用支数、使用重量、要求到货日、计划状态(InventoryPlanStatus枚举)、备注、改制类型(ReworkType枚举)、工艺周期(天)
- 无单键
