...
**功能定位**：跨上下文的通用数据工具，支持 67 个实体的 Excel 导入导出。所有端点（实体列表/导出/模板/预览/导入/一键修复）均要求管理员（Admin）角色。
...
| 1 | CustomerProfile | 订单-客户档案 | 单键（CustomerUnit） | 客户基本信息 |
...
| 1 | SubStandardQuickView | 标准-子标准速览 | 无单键 | 按标准号快速查看各检验项目引用标准 |
...
| 1 | SectionFlowCategorySetting | 生产-工段日流转量 | 单键（CategoryCode） | 段落分类及变异量系数预设 |
| 1 | SectionFlowCategoryItem | 生产-工段日流转量子项 | 无单键（FK→SectionFlowCategorySetting） | 日流转量子项明细 |
...
| 工单-库存计划 | 计划状态 | InventoryPlanStatus | 已计划、已确认、已取消 |
...
> **注意**：出库记录的"出库类型"字段在数据库中存储为字符串（如 `ProductionPick`），非枚举类型，导入导出时不经过 EnumHelper 转换。
...
**交态附加天数（StandardWorkDayDeliveryState）**：交货状态、附加天数、牌号前缀、备注
- 无单键
...
**工段日流转量（SectionFlowCategorySetting）**：类别编码、类别名称、变异量预算日产、偏少天数值、过多天数值、备注
- 主键：类别编码

**工段日流转量子项（SectionFlowCategoryItem）**：类别编码(FK→工段日流转量)、工序组名称、工段名称、变异量系数、排序号
- 无单键，FK 通过类别编码解析为 SettingId
...
**技术要求（ProductRequirement）**：订单号(复合FK→订单项次)、项次号(复合FK→订单项次)、技术要求类型、化学成分要求、力学性能要求、公差要求、表面质量要求、无损检测要求、其他要求
...
**工序组（ProcessGroup）**：所属批次号(FK→生产批次)、组内序号、工序名称、制造规格、外径公差、壁厚公差、制造长度、断切处理、制成倍数、备注、冷轧拔、油管断、去油、固溶、矫直、断切、测壁厚、酸洗、外抛光、内修磨、外点磨、检验、打焊头、润滑、入库
- 复合键：批次号 + 组内序号
- FK 通过 BatchNo 解析为 ProductionBatchId

**库存批次（InventoryBatch）**：批次号、仓库编码(FK→仓库档案)、物料类型、厂内钢种、名义规格、入库来源、来料单位、入库日期、炉号、生产批号、长度状态、最小长度(mm)、最大长度(mm)、入库支数、入库重量(kg)、理论单支重(kg)、米数(m)、当前剩余支数、当前剩余重量(kg)、当前剩余米数(m)、实际规格、表面状态、放置区域、放置架号、次品原因、责任类型、原始来料单位、挂牌号、次品备注、工单号、订单号、是否关联工单、项次ID、来源单号、备注
- 主键：批次号
- 共 35 列

**出库记录（OutboundRecord）**：批次号(FK→库存批次)、出库类型(OutboundType枚举)、物料单号、目标单位、出库支数、出库重量(kg)、出库米数(m)、出库日期、备注

**生产报工（ProductionRecord）**：ID(系统)、批次号(FK→生产批次)、挂牌号、组内序号(FK→工序组)、工序名称、工厂牌号、制造规格、工段名称、执行序号、执行日期、设备名称、操作人、班次、加工支数、加工重量(kg)、固溶温度(℃)、保温时间(min)、制造状态、长度状态(LengthStatus枚举)、断切倍数、成品断切长度(mm)、切后支数、平头数、备注、数据来源(系统)
...
**来料检验（MaterialReceiveCheck）**：批次号(FK→生产批次)、到料日期、制造物品、挂牌号、工单号、订单号、来料单位、炉号、工厂牌号、规格、生产类型、强制完成(bool)、班次、确认人、备注、数据来源(系统)
- 复合键：批次号
- FK 通过 BatchNo 解析为 ProductionBatchId

**过程检验（ProcessInspection）**：批次号(FK→生产批次)、挂牌号、组内序号、工序名称、工厂牌号、制造规格、工段名称、检验日期、设备名称、检验员、班次、检验支数、检验重量(kg)、检验项目、合格支数、合格重量(kg)、**合格中让步放行支**、**让步说明**、不合格返整支数、不合格入库支数、不合格报废支数、不合格描述、来料单位、备注、数据来源
- 复合键：批次号 + 工序名称 + 制造规格 + 工段名称 + 检验日期
- FK 通过 BatchNo 解析为 ProductionBatchId，组内序号解析为 ProcessGroupId
- 枚举字段：检验项目（ProcessInspectionItem 枚举）
...
**去油酸洗入缸记录（PicklingInRecord）**：批次号(FK→生产批次)、挂牌号、工序名称、制造规格、工段名称、组内序号(FK→工序组)、入缸日期、状态(PicklingStatus枚举)、设备名称、操作人、班次、加工支数、加工重量(kg)、制造状态、工厂牌号、备注、数据来源(系统)
...
**荒管采购工序组（SemiPlanProcessGroup）**：所属荒管计划ID(FK→荒管采购计划)、组内序号、工序名称、制造规格、外径公差、壁厚公差、制造长度、断切处理、制成倍数、备注、冷轧拔、油管断、去油、固溶、矫直、断切、测壁厚、酸洗、外抛光、内修磨、外点磨、检验、打焊头、润滑、入库
- 复合键：所属荒管计划ID + 组内序号

**库存计划工序组（InventoryPlanProcessGroup）**：所属库存计划ID(FK→库存计划)、组内序号、工序名称、制造规格、外径公差、壁厚公差、制造长度、断切处理、制成倍数、备注、冷轧拔、油管断、去油、固溶、矫直、断切、测壁厚、酸洗、外抛光、内修磨、外点磨、检验、打焊头、润滑、入库
- 复合键：所属库存计划ID + 组内序号

**圆棒穿孔工序组（PiercingPlanProcessGroup）**：所属穿孔计划ID(FK→圆棒穿孔计划)、组内序号、工序名称、制造规格、外径公差、壁厚公差、制造长度、断切处理、制成倍数、备注、冷轧拔、油管断、去油、固溶、矫直、断切、测壁厚、酸洗、外抛光、内修磨、外点磨、检验、打焊头、润滑、入库
- 复合键：所属穿孔计划ID + 组内序号

**在产改制计划（InProcessReworkPlan）**：所属工单ID(FK→工单)、计划日期、生产批次ID(FK→生产批次)、批次号、挂牌号、物料名称、工厂牌号、规格、长度状态、批次总支数、批次总重量、现有有效原料支数、现有有效原料重量、投料制成倍、使用支数、使用重量、要求到货日、计划状态、备注、改制类型、工艺周期(天)
- 无单键

**在产改制工序组（InProcessReworkPlanProcessGroup）**：所属在产改制计划ID(FK→在产改制计划)、组内序号、工序名称、制造规格、外径公差、壁厚公差、制造长度、断切处理、制成倍数、备注、冷轧拔、油管断、去油、固溶、矫直、断切、测壁厚、酸洗、外抛光、内修磨、外点磨、检验、打焊头、润滑、入库
- 复合键：所属在产改制计划ID + 组内序号
...
| 版本 | 日期 | 说明 |
|------|------|------|
| **V7.3** | **2026-08-02** | **成切存疑枚举化**：ProductionBatch 成切存疑列从 bit（true/false）改为 CutDoubtType 枚举字符串（QuantityMismatch/MissingRecords/Normal/空=略），枚举字段清单补充成切存疑（CutDoubtType）；导出值格式同步为枚举字符串 |
...
```yaml
已定义内容:
  - 67个实体注册定义 ✅（含9批依赖顺序、枚举字段一览、完整列定义）
