...
**本次变更（V7.23，2026-09-16）**：**新增一级菜单「基础资料」后的数据工具正名**（**纯显示名**，实体 / 表名 / 端点 / 权限 / `EntityOrder` 位置一律不变）—— ① `Workstation` 显示名「扫码-工位管理」→「**基础资料-工位管理**」、`Employee` 显示名「扫码-员工管理」→「**基础资料-员工管理**」（工位/员工档案已自「扫码管理」迁入「基础资料」）；② `DataExchangeRegistry.ContextOrder` 第 10 位「扫码」同步改为「**基础资料**」（扫码组已无任何注册实体）；③ `ConfigParameter` 显示名「系统-系统参数(全局参数)」→「**系统-全局参数**」（一级菜单「参数表」更名「系统参数」后组内同名叶子去重名改「全局参数」，显示名同步去重名）；④ §2.1 实体表 / §2.2 依赖关系图内显示名同步。**⑤ 正文与注册表全量对账（本次补历史欠账，**纯文档**、零代码零端点零权限）**：本文档 §2.1 此前长期只有 **71 行**、§2.2 图 **79 节点**，而 `DataExchangeRegistry.Registry` 实为 **91 实体** → 本次补齐 **20 个缺失实体行**（工资 8 张月表、`FactoryInspectionRequirement`、`OutsourceVendorProfile`、`Certificate`/`CertificateItem`、`BatchPlanSchedule`/`ColdRollSpecSchedule`、`NonconformingFeedback`/`InspectionPatrol`/`InspectionPatrolItem`、`WorkOrderPlan`/`RawMaterialLockPreExecution`/`InMainWorkOrderPlan`）并同步入图（另补 6 个「配置-」实体）；顺带修正 **2 处批次号错标**（`StandardRegisterItem` 1→**2**、`PurchaseOrder` 6→**7**，后者使原「批次6」子图正名为「批次7」）；**修正 §2.2 重复节点 ID 缺陷**（`SRI` 曾被「标准-标准号子项目」与「物料-委外子项」共用 → 后者改 `SCRI`）。现文档 **表格 91/91、关系图 91/91 与注册表逐项一致**。**旧导出文件与已下发导入模板均无需重下**（导入识别按实体 Key + 表头名，与显示名无关）。护栏：`DataExchangeServiceTests.Registry_基础资料与全局参数_显示名与菜单正名同步`。

**历史变更（V7.22，2026-09-15）**：菜单正名核对（**纯显示名**，实体 / 表名 / 端点 / 权限一律不变）——`DataExchange` 实体显示名「批次-批次计划」→「**批次-生产计划**」，本文件正文未直接引述该显示名，故**无内容改动**，仅台账递抬。

**历史变更（V7.21）**：注册完整性审计与补登记 —— ① **补注册 7 个业务实体**（84 → **91**）：`Certificate`（质量-质量证明书）、`CertificateItem`（质量-质量证明书附表）、`WorkOrderPlan`（工单-工单计划）、`RawMaterialLockPreExecution`（工单-原锁预执行）、`InMainWorkOrderPlan`（工单-在产主工单计划）、`BatchPlanSchedule`（批次-批次计划）、`ColdRollSpecSchedule`（批次-冷轧规格排程，复合键 `ProcessType + BilletSpec + RollingSpec`）；② **Ncr 补「来源反馈单ID」FK 列**（`NonconformingFeedbackId` ↔ NonconformingFeedback.Id，列数 40 → 41，**旧模板须重新下载**）；③ **SectionOutsource 补「计价单位 / 单价 / 总金额」三列**；④ **NonconformingFeedback / InspectionPatrol 的「产类」加 `isSystem: true`**（服务端按批次计算，仅导出不可导入）；⑤ **修复「先删后插」实体「导出后原样回导」静默丢数据 bug**（带旧 ID 行降级为新增 + 全部行失败整体回滚，`HasRolledBack = true`）。`EntityOrder` 同步插入新实体的依赖位置。

**历史变更（V7.20）**：质量域**不合格报告（Ncr）新增 4 列 + 「流向」列正名为「次品流向」**—— ① `DataExchangeRegistry` Ncr 实体新增 **让步放行支数**（`ConcessionQuantity`，`int?`）、**让步放行重量**（`ConcessionWeight`，`int?`）、**让步说明**（`ConcessionRemark`，`string?`）、**来源待处理组键**（`SourceGroupKey`，`string?`，系统回写：被动行由待处理组生成，供同一组避免重复建单）；② **「流向」列名改「次品流向」**（字段不变，仍 `FlowDirection?`、`isEnum: true`、只读带出，导出中文/导入枚举名或中文均可），与 NCR 前端 G1 列名对齐；③ 列序调整 —— `… 来源检验项目 / 让步放行支数 / 让步放行重量 / 让步说明 / 来源待处理组键 / 次品流向 / 处置方式 …`；④ 列数 36 → **40**。**旧导入模板表头失效，须重新下载**（Ncr 模板须包含上述 4 个新表头，且原「流向」表头已更名为「次品流向」）。

**历史变更（V7.19）**：质量域**不合格报告（Ncr）新增「流向」枚举列（5 档），「处置方式」由枚举列改字典列**（`DictKey = NcrDisposalKey`，内置 8 档）—— ① `DataExchangeRegistry` Ncr 实体新增 `流向`（`FlowDirection?`，`isEnum: true`，可空，只读带出）；`处置方式` 由 `DisposalMethod?` 枚举改 **`string?` 非枚举**，导入经 `ResolveDictValueKey(NcrDisposalKey, …) ?? NcrDisposalKeys.ToKey(…)` 归一（中文/英文 Key 均可），导出经 `DictValueDisplayHelper.GetText` 输出字典中文显示名。② 列数 35 → **36**。**旧导入模板表头失效，须重新下载**（另请留意：处置方式列的可填值由 5 档扩为 8 档）。

**历史变更（V7.18）**：质量域**成品检验「不合格品去向」五档化**，`DataExchangeRegistry` 成品检验实体新增两列——**不合格入在制库支数**（DefectInProcessWarehouseQuantity）、**次品入在制库重量(kg)**（DefectInProcessWarehouseWeight，系统回写），列序调整为 支数 返整/入在制库/入次品库/退货/可入备库、重量 返整/入在制库/入次品库/退货/可入备库。过程检验实体不变（仍 4 档）。**旧导入模板表头失效，须重新下载**。

**历史变更（V7.17）**：质量域**过程检验 / 成品检验「不合格处理」四档化**，`DataExchangeRegistry` 两实体列名与字段同步——过程检验「不合格入库支数/不合格报废支数」→**不合格入在制库支数/不合格入次品库支数**、新增**不合格退货支数**（DefectReturnQuantity），另 4 个系统回写理论重列名改为「理论返整重/理论入在制重/理论入次库重/**理论退货重**」（新增后两项中的退货项）；成品检验「不合格入库支数/不合格报废支数」→**不合格可入备库支数/不合格入次品库支数**、新增**不合格退货支数**（DefectReturnQuantity）、「次品入库重量/次品报废重量」→**次品可入备库重量/次品入次品库重量**、新增**次品退货重量**（DefectReturnWeight/系统回写）。**旧导入模板表头失效，须重新下载**。

**本次变更（V7.16）**：8 个理化试验实体（硬度/晶粒度/扩口/压扁/晶间腐蚀/金相/点腐蚀/室温拉伸）字段 `FurnaceNo` → `BatchNo`，`DataExchangeRegistry` 表头同步——**硬度检验（HardnessTest）/ 晶粒度检验（GrainSizeTest）导入模板表头由「炉批号」改为「生产编号」，旧模板须重新下载**；其余 6 个原表头已是「生产编号」不变。
...
**功能定位**：跨上下文的通用数据工具，支持 91 个实体的 Excel 导入导出。端点权限按数据工具三档角色控制：实体列表/导出/模板/预览=`DataToolView`、导入=`DataToolEdit`、一键修复=`DataToolDelete`（策略尾部均含 Admin 隐式全权）。

> **显示名称规范**：所有实体中文显示名均采用 `{上下文前缀}-{实体名}` 格式，上下文前缀按 `DataExchangeRegistry.ContextOrder` 固定顺序：订单→工单→批次→质量→物料→仓库→设备→标准→配置→**基础资料**→工资→系统（工位/员工归「基础资料」、5 张排程/流转配置表归「配置」、工资结算 8 张月表归「工资」、ConfigParameter 归「系统」）。
...
| 1 | Workstation | 基础资料-工位管理 | 单键（Code） | 扫码报工工位定义（工段+设备+报工模板类型） |
| 1 | Employee | 基础资料-员工管理 | 单键（Code） | 员工基础信息（扫码报工和工资结算基础） |
| 1 | ConfigParameter | 系统-全局参数 | 复合键（Category+ParamKey） | 系统参数（数值型，分类代码+参数键唯一） |
...
| 1 | FactoryInspectionRequirement | 标准-工厂检验项要求 | 单键（StandardNo） | 标准号对应的工厂检验项目要求明细（32 项检验要求列） |
| 1 | OutsourceVendorProfile | 批次-委外单位档案 | 单键（VendorCode） | 委外单位档案（委外工段/联系人/是否本厂车间） |
| 1 | Certificate | 质量-质量证明书 | 单键（CertificateNo） | 质量证明书（签发日期/客户名称/产品标准/交货状态），含附表 |
| 1 | AttendanceRecord | 工资-考勤表 | 复合键（EmployeeId+AttendDate） | 员工×日期出勤小时，FK 员工工号→Employee.Code |
| 1 | PayrollDailyWageRecord | 工资-每日工资表 | 复合键（EmployeeId+WageDate） | 员工×日期每日工资，FK 员工工号→Employee.Code |
| 1 | PayrollCollectiveScore | 工资-月度评分 | 复合键（EmployeeId+Year+Month） | 集体计件月度评分，FK 员工工号→Employee.Code |
| 1 | PayrollCollectiveWageRecord | 工资-集体计件月结 | 复合键（EmployeeId+WageYear+WageMonth） | 集体计件月结工资，FK 员工工号→Employee.Code |
| 1 | PayrollAttendanceWageRecord | 工资-靠工计件月结 | 复合键（EmployeeId+WageYear+WageMonth） | 靠工计件月结工资，FK 员工工号→Employee.Code |
| 1 | PayrollMiscWorkRecord | 工资-杂辅工记录 | 无单键 | 杂辅工记录，FK 员工工号→Employee.Code |
| 1 | PayrollAllowanceRecord | 工资-津贴与处罚 | 复合键（EmployeeId+Year+Month） | 津贴与处罚登记，FK 员工工号→Employee.Code |
| 1 | PayrollMonthlySummaryRecord | 工资-月工资汇总 | 复合键（EmployeeId+Year+Month） | 月工资汇总快照，FK 员工工号→Employee.Code |
...
| 2 | StandardRegisterItem | 标准-标准号子项目 | 无单键（FK→StandardRegister） | 标准号子项目（检验项目/强制性/取样要求等） |
| 2 | CertificateItem | 质量-质量证明书附表 | 无单键（FK→Certificate.CertificateNo） | 证明书附表（批次/炉号/化学成分/力学与检验结果） |
...
| 8 | BatchPlanSchedule | 批次-生产计划 | 无单键（FK→ProductionBatch.BatchNo） | 批次排程计划（暂停/流转/等级/执行序/抢单），批次号唯一 |
| 8 | ColdRollSpecSchedule | 批次-冷轧规格排程 | 复合键（ProcessType+BilletSpec+RollingSpec） | 冷轧规格排程（机台/单机单日量/完工要求/排程类型） |
...
| 9 | NonconformingFeedback | 质量-不合格反馈 | 无单键（FK→ProductionBatch / ProcessGroup） | 不合格反馈单（被动来源），批次号+工序组序号定位 |
| 9 | InspectionPatrol | 质量-巡检 | 无单键（FK→ProductionBatch / ProcessGroup） | 巡检单（主动来源），批次号+工序组序号定位，含明细 |
| 9 | InspectionPatrolItem | 质量-巡检明细 | 无单键（FK→InspectionPatrol.Id） | 巡检单明细（巡检项/结果/排序），父表按 Id 关联 |
| 9 | WorkOrderPlan | 工单-工单计划 | 无单键（FK→WorkOrder） | 工单状态/紧急性/关注工序/流转性四项覆盖，工单号唯一 |
| 9 | RawMaterialLockPreExecution | 工单-原锁预执行 | 无单键（FK→WorkOrder） | 原锁预执行标记 + 预算投料日，工单号唯一 |
| 9 | InMainWorkOrderPlan | 工单-在产主工单计划 | 无单键（FK→WorkOrder + ProductionBatch） | 在产主工单计划（分配重量/支数/工艺周期/计划状态） |
...
graph TB
    subgraph 批次1_独立实体
        ...
        PDEF[配置-工序组定义(批次/工艺)]
        CRC[配置-冷轧产能档案(冷轧排程)]
        CRMC[配置-冷轧机台数配置(冷轧排程)]
        CRMG[配置-冷轧机台组配置(冷轧排程)]
        EDD[配置-枚举显示配置(全局显示)]
        DVD[配置-字典显示配置(全局显示)]
        CONFIG[系统-全局参数]
        FIR[标准-工厂检验项要求]
        ...
        PDW[工资-每日工资表] -->|FK| EMP
        PCS[工资-月度评分] -->|FK| EMP
        PCW[工资-集体计件月结] -->|FK| EMP
        PAW[工资-靠工计件月结] -->|FK| EMP
        PMW[工资-杂辅工记录] -->|FK| EMP
        PAL[工资-津贴与处罚] -->|FK| EMP
        PMS[工资-月工资汇总] -->|FK| EMP
    end
    ...
    subgraph 批次9
        ...
        NCF[质量-不合格反馈] -->|FK| PB
        IPAT[质量-巡检] -->|FK| PB
        IPATI[质量-巡检明细] -->|FK| IPAT
        WOP[工单-工单计划] -->|FK| WO
        RMLP[工单-原锁预执行] -->|FK| WO
        IMWOP[工单-在产主工单计划] -->|FK| WO
        IMWOP -->|FK| PB
    end
```
...
**不合格报告（Ncr）**：反馈日期、反馈部门、反馈人、钢管类别(MaterialType枚举)、生产编号、工单号、工厂牌号、规格、次品支数、次品重量、问题描述、来源检验项目、**让步放行支数**、**让步放行重量**、**让步说明**、**来源待处理组键**、**次品流向(FlowDirection枚举，只读)**、处置方式(**字典 NcrDisposalKey**)、处置备注、处置是否完结(bool)、处置完结日期、原因分析、严重程度(SeverityLevel枚举)、分析确认人、分析确认日期、责任类别(字典 NcrResponsibilityKey)、责任部门、生产操作日期、生产责任人、责任人处理、责任人处理完结(bool)、责任人处理完结日期、纠正预防措施、计划人、计划日期、验证人、验证日期、结果判定、验证结论(VerifyResult枚举)、状态(NcrStatus枚举)
- 无单键（共40列）
- 说明：按「问题反馈→不合格品处置→原因分析→责任人及处理→纠正预防措施及结果验证」五组组织。⚠️ V7.19：处置方式由枚举列改字典列（导出为中文显示名、导入按**字典**优先反查 Key）；⚠️ V7.20：新增「让步放行支数/让步放行重量/让步说明/来源待处理组键」4 列（仅**被动（超阈值遗漏）**行有值；`SourceGroupKey` 为系统回写，用于同组去重），「流向」列名正名为「**次品流向**」（导出中文、导入枚举名/中文均可）
...
**订单项次（OrderItem）**：订单号(FK→销售订单)、项次号、交货日期、延期罚款、结算方式、物料名称、产品标准编码、交货状态、标准牌号、工厂牌号、密度(g/cm³)、外径(mm)、壁厚(mm)、规格、外径下偏差(mm)、外径上偏差(mm)、壁厚下偏差(mm)、壁厚上偏差(mm)、长度状态、最小长度(mm)、最大长度(mm)、数量(支)、米数(m)、合同重量(kg)、理算重量(kg)、计价单位(元/Kg/元/米/元/支)、单价(元)、总价(元)、备注
- 复合键：订单号 + 项次号
...
**过程检验（ProcessInspection）**：批次号(FK→生产批次)、挂牌号、组内序号、工序名称、工厂牌号、制造规格、工段名称、检验日期、设备名称、检验员、班次(ShiftType枚举)、检验支数、检验重量(kg)、检验项目、合格支数、合格重量(kg)、**合格中让步放行支**、**让步说明**、不合格返整支数、不合格入在制库支数、不合格入次品库支数、不合格退货支数、不合格描述、理论返整重(kg)、理论入在制重(kg)、理论入次库重(kg)、理论退货重(kg)、来料单位、备注、数据来源
...
**成品检验（FinalInspection）**：生产编号(FK→生产批次)、定尺长度、非定尺长度范围、定尺切割匹配(CutLengthMatchType枚举,系统)、成检类型(InspectionType枚举)、检验项目(InspectionItem枚举)、检验日期、设备名称、班次(ShiftType枚举)、操作员、检验支数、检验重量(kg)、合格支数、合格重量(kg)、合格中让步放行支、让步说明、不合格返整支数、不合格入在制库支数、不合格入次品库支数、不合格退货支数、不合格可入备库支数、次品返整重量(kg)、次品入在制库重量(kg)、次品入次品库重量(kg)、次品退货重量(kg)、次品可入备库重量(kg)、不合格情况描述、外径范围、壁厚范围、长度余量范围、压力Mpa、保压时间s、资格等级、检验标准、检验等级、检验仪器型号、检验方式、标样尺寸、标样人工缺陷、探头类型、耦合剂、检测设备校准频率、检测频率、检测灵敏度、检测相位、检测速度、检验备注、数据来源
- 无单键，FK 通过 BatchNo 解析为 ProductionBatchId
...
**硬度检验（HardnessTest）**：检验日期、检验员、生产编号、牌号、规格、试样编号、试样尺寸、检验标准、硬度模式、硬度测定值、判定
- 无单键

**晶粒度检验（GrainSizeTest）**：检验日期、检验员、生产编号、牌号、规格、试样编号、试样尺寸、检验标准、晶粒度级别、晶粒度测定方法、观察倍数、判定
- 无单键
...
```yaml
已定义内容:
  - 91个实体注册定义 ✅（含9批依赖顺序、枚举字段一览、完整列定义）
