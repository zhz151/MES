...
| 1 | ConfigParameter | 系统参数(全局参数) | 复合键（Category+ParamKey） | 系统参数（数值型，分类代码+参数键唯一） |
...
- 主键：流转类别
- 说明：每个流转类别一行，存储用户可编辑参数；(工序组,工段,产类)组合明细由组合段落归类 CombinationGroup 承载

**段落日产配置（SectionParagraphConfig）**：段落类别、日流转设定、偏少天数值、过多天数值、备注
- 主键：段落类别
- 说明：独立配置表，段落类别唯一

**组合段落归类（CombinationGroup）**：工序组、工段、产类、归属流转类别(FK→流转类别日产配置)、归属段落
- 复合键：工序组 + 工段 + 产类
