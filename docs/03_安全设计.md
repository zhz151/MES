...
用户决策：**取消全部二级菜单权限，纯一级模型**。角色 = `{菜单前缀}{档位}`，共 46 个（Admin + 15 主菜单 × 三档，含 2026-09 新增「工资结算」三档）：
...
**Admin（系统级）**：隐式全权——所有策略尾部固定 `",Admin"`，因此 Admin 自动拥有全部 15 主菜单的三档权限。
...
- `Roles.Menus.{Module}{Tier}`：15 菜单 × 3 档角色名常量（`Roles.Menus.OrderViewer` 等）。
- `Roles.Policies.{Module}{Action}`：授权策略字符串，统一管理 `[Authorize(Roles = "...")]` 中的组合，尾部恒含 `,Admin`。
- `Roles.GetAllRoles()`：全部 46 角色数组（用于初始化/校验）。

**报表数据域说明**：订单/工单/批次/质量/物料/仓库/计划排程 7 个数据域的**端点读**策略同时含 Report 三档角色（`XxxView = "...,ReportViewer,ReportEditor,ReportFull,Admin"`），使报表用户能读数据；**菜单门控用 XxxMenu 策略**（不含 Report 角色），避免报表用户看到数据域菜单。其余数据域（设备/产品标准/参数表等）端点读不含 Report 角色。
...
> 新建用户默认档位：9 个业务域菜单（订单/工单/计划排程/批次/质量/物料/设备/产品标准/仓库）= Viewer，其余系统菜单（报表/数据工具/扫码/工资结算/参数表/用户管理）= None。新建密码明文默认 `123456`。
...
```csharp
// AppMenu.cs（节选，2026-09-13 菜单整序后）——一级单项（报表总览/数据工具/用户管理）直接在叶上带 View 档 Policy
...
new() { Label = "数据工具", Href = "/data-exchange", Policy = Roles.Policies.DataToolView },  // 参数表后、用户管理前
```
...
组级门控策略约定：订单/工单/计划排程/批次/质量/物料/仓库 7 个业务域用独立 `Roles.Policies.XxxMenu`；产品标准/设备/报表/数据工具/工资结算/参数表/用户管理等**无独立 `XxxMenu`**，组/叶 Policy 直接复用 `Roles.Policies.XxxView`（含 Admin）；扫码组**整组无 Policy**（仅登录），其下「工位/员工」叶子单独带 `Roles.Policies.ScanView`。
...
授权模型:
  - 基于角色的访问控制（RBAC），纯一级
  - 46 个角色 = Admin + 15 主菜单 × 3 档（Viewer/Editor/Full，含工资结算三档）
...
已实现 API 保护:
  ...
  - 工资结算上下文 ✅（AttendanceController, PayrollCollectiveController, PayrollAttendanceController, PayrollMiscWorkController, PayrollAllowanceController, PayrollDailyWageController, PayrollMonthlySummaryController, PieceRateProductionCategoryController, PieceRateFinalInspectionCategoryController）
...
一句话:
  - 安全设计采用 JWT 认证 + 46 角色（15 主菜单 × 三档）纯一级授权，
  - 订单、工单、仓库、物料、批次、质量、设备、计划排程、产品标准、配置、工资结算、数据工具模块权限已完整实现。
```
