...
> 新建用户默认档位：9 个业务域菜单（订单/工单/计划排程/批次/质量/物料/设备/生产标准/仓库）= Viewer，其余系统菜单（报表/数据工具/扫码/工资结算/参数表/用户管理）= None。新建密码明文默认 `123456`。
...
组级门控策略约定：订单/工单/计划排程/批次/质量/物料/仓库 7 个业务域用独立 `Roles.Policies.XxxMenu`；生产标准/设备/报表/数据工具/工资结算/参数表/用户管理等**无独立 `XxxMenu`**，组/叶 Policy 直接复用 `Roles.Policies.XxxView`（含 Admin）；扫码组**整组无 Policy**（仅登录），其下「工位/员工」叶子单独带 `Roles.Policies.ScanView`。
...
一句话:
  ...
  - 订单、工单、仓库、物料、批次、质量、设备、计划排程、生产标准、配置、工资结算、数据工具模块权限已完整实现。
```
