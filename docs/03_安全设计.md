...
```csharp
// AppMenu.cs（节选）——组/叶按需带 Policy
new() { Label = "订单管理", Policy = Roles.Policies.OrderMenu, Children = [ /* 订单列表 / 客户管理 / 订单成品(在库) */ ] },
new() { Label = "扫码报工", Href = "/scan" },                                    // 无 Policy：仅登录可见
new() { Label = "工位",      Href = "/scan/workstations", Policy = Roles.Policies.ScanView },
```
