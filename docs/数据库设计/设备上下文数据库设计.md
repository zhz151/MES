...
> V2.3（2026-09-16）：文档与代码对账（纯文档修订，无 DDL）——① §三 补录 Equipment 索引（`UK_Equipment_Code` 唯一 + `IX_Equipment_Name` / `_LifecycleStatus` / `_Location` / `_RelatedSection` / `_NeedInspection` / `_NeedMaintenance`，原文整节缺失）；② §6.5 索引名更正 `IX_RepairOrder_RepairStatus` → `IX_RepairOrder_Status`；③ §6.2 字段类型更正 `RepairCategory` / `OtherRepairPersons` 实为 `nvarchar(max)`（代码未设 MaxLength，原记 nvarchar(20)/nvarchar(500)）。
...
> **注：** `RunningStatus`/`InspectionStatus`/`MaintStatus` 为 Equipment 实体物化存储字段，由 `EquipmentStatusCalculator` 在点检/保养/维修写操作后同步更新，支持 DB 级排序和筛选。
...
> **注：** 上述状态字段为物化存储，统一由 `EquipmentStatusCalculator.cs` 管理。删除操作后不会回退历史日期（LastInspectionDate/LastMaintDate/LastRepairDate 删除后不恢复）。

### 3.6 索引

| 索引名 | 字段 | 唯一 | 说明 |
|--------|------|------|------|
| UK_Equipment_Code | EquipmentCode | ✅ | 设备编号唯一 |
| IX_Equipment_Name | EquipmentName | | 按设备名称查询 |
| IX_Equipment_LifecycleStatus | LifecycleStatus | | 按生命周期筛选 |
| IX_Equipment_Location | Location | | 按所在区域筛选 |
| IX_Equipment_RelatedSection | RelatedSection | | 按关联工段筛选 |
| IX_Equipment_NeedInspection | NeedInspection | | 按是否需点检筛选 |
| IX_Equipment_NeedMaintenance | NeedMaintenance | | 按是否需保养筛选 |
...
| RepairCategory | nvarchar(max) | ❌ | - | 维修类别：厂内维修/外协维修/换模 | 👤 |
...
| OtherRepairPersons | nvarchar(max) | ❌ | - | 辅助维修人（多人协作时补充，逗号分隔） | 👤 |
| CreatedTime | datetimeoffset | ✅ | GETUTCDATE() | 创建时间 | 📅 |
| CreatedBy | nvarchar(50) | ✅ | - | 创建人 | 📅 |
| UpdatedTime | datetimeoffset | ✅ | GETUTCDATE() | 更新时间 | 📅 |
| UpdatedBy | nvarchar(50) | ✅ | - | 更新人 | 📅 |
