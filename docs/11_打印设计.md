...
> **本次变更（V2.20，2026-09-15）**：菜单正名（**纯显示名**，页面 / 路由 / 打印 Helper / 端点一律不变）——一级菜单「生产标准」→「**产品标准**」、计划排程组「批次计划」→「**生产计划**」；§4.3 打印 Helper 表、§4.5 内联形态表、§5.7 排程上下文、§5.9 章节名及其「上下文」列的旧名同步替换，本文件无实现改动。
...
| StandardRegisterPrintHelper | ⓫ | 标准号列表 | 产品标准上下文 |
| GradeChemicalCompositionPrintHelper | ⓫ | 标准牌号化学成分列表 | 产品标准上下文 |
| GradePhysicalPropertyPrintHelper | ⓫ | 牌号物理性能列表 | 产品标准上下文 |
| SubStandardQuickViewPrintHelper | ⓫ | 子标准速览列表 | 产品标准上下文 |
| StandardInspectionRequirementPrintHelper | ⓫ | 标准号检验项要求列表 | 产品标准上下文 |
| ChemicalCompositionPrintHelper | ⓫ | 工厂牌号化学成分列表 | 产品标准上下文 |
| ChemicalValidationRulePrintHelper | ⓫ | 工厂牌号化分验证规则列表 | 产品标准上下文 |
...
| BatchPlanPrintHelper | ⓫ | 生产计划列表 | 排程上下文 |
...
| FactoryInspectionRequirementPrintHelper | ⓫ | 工厂检验项要求列表 | 产品标准上下文 |
...
| GradeMappingService | ⓫ | 牌号映射 | 产品标准上下文 |
...
| 生产计划 | Mode B（Helper）⓫ | BatchPlanPrintHelper | BatchPlans.razor |
...
| 段落流转分析 | Mode B（内联）⓫ | SectionParagraphFlowAnalysisService | 经生产计划页折叠展示与报表总览消费（原独立查询页已移除） |
