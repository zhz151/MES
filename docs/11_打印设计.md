...
| Helper | 用途 | 备注 |
|--------|------|------|
| ProcessCardPrintHelper | 工艺流转卡 PDF | 含工序组表格、质检要求等富布局 |
| SalesOrderPrintHelper | 销售订单 PDF + 产品要求 PDF | 双模式 |
| SubcontractOrderPrintHelper | 委外订单 PDF | 双模式 |
| WorkOrderPrintHelper | 工单 PDF | 双模式 |
| MaterialPlanPrintHelper | 物料计划 PDF（6 种计划类型） | 双模式：原料采购、成品采购、库存使用、库料改制、圆棒穿孔、在产改制 |
| ReportPrintHelper | 产量报表打印 | 日产量报表 PDF 生成 |

> 注：标有"双模式"的 Helper 同时提供 前端打印入口。

### 3.3 枚举显示

Mode A 枚举字段直接在 switch 中手动映射：
...
#### 形态二：Service/Controller 内联（共 20 处，无独立 Helper）
...
| PicklingController（入缸） | ⓫ | 酸洗入缸记录 | 批次上下文 |
| PicklingController（出缸） | ⓫ | 酸洗完工记录 | 批次上下文 |
| InventoryService | ⓫ | 库存/入库/出库列表 | 仓库上下文 |
| WorkOrderScheduleController | ⓫ | 工单计划 | 计划排程 |
| BatchPlanController | ⓫ | 批次计划 | 计划排程 |
| FinalInspectionPlanController | ⓫ | 成检计划 | 计划排程 |
| RawMaterialLockPlanAndExecutionController | ⓫ | 原锁计划 | 计划排程 |
| SectionFlowAnalysisController | ⓫ | 工段流转分析 | 计划排程 |
| SectionProductionStatusController | ⓫ | 工段待产量 | 计划排程 |
| OrderDemandAdjustmentController | ⓫ | 工单需求调整 | 工单上下文 |
| WorkOrderExecutionController | ⓫ | 工单执行状况 | 工单上下文 |
| PendingDeliveryController | ⓫ | 待发货订单成品 | 仓库上下文 |
| CustomerService | 纯A类 | 客户信息 | 订单上下文 |
| MaterialService | 纯A类 | 物料明细 | 物料上下文 |
...
| 工艺流转卡 | 前端 + Mode A | ProcessCardPrintHelper | ProcessCardPrint.razor |
| 生产记录 | Mode B（Helper）⓫ | ProductionRecordPrintHelper | ProductionRecords.razor |
| 成检到料 | Mode B（Helper）⓫ | MaterialCheckPrintHelper | MaterialChecks.razor |
...
| NCR 报告 | Mode B（Helper）⓫ | NcrPrintHelper | Ncrs.razor |
...
| 订单列表 | Mode B（内联）⓫ | OrderService | Orders.razor |
| 工单需求调整 | Mode B（内联）⓫ | OrderDemandAdjustmentController | OrderDemandAdjustment.razor |
...
| 打印内容 | 模式 | 实现位置 | 前端页面 |
|---------|------|---------|---------|
| 物料明细 | Mode B（内联）⓫ | MaterialService | Materials.razor |
...
| 工单执行状况 | Mode B（内联）⓫ | WorkOrderExecutionController | WorkOrderExecution.razor |
...
| 打印内容 | 模式 | 实现位置 | 前端页面 |
|---------|------|---------|---------|
| 排程计划 | Mode B（内联）⓫ | WorkOrderScheduleController | WorkOrderSchedules.razor |
| 批次计划 | Mode B（内联）⓫ | BatchPlanController | BatchPlans.razor |
| 成检计划 | Mode B（内联）⓫ | FinalInspectionPlanController | FinalInspectionPlan.razor |
| 原锁计划 | Mode B（内联）⓫ | RawMaterialLockPlanAndExecutionController | RawMaterialLockPlanAndExecution.razor |
...
| 待发货订单成品 | Mode B（内联）⓫ | PendingDeliveryController | PendingDelivery.razor |

### 5.9 报表上下文

| 打印内容 | 模式 | 实现位置 | 前端页面 |
|---------|------|---------|---------|
| 日产量报表 | Mode A | ReportPrintHelper | （待建设） |
...
| 打印模式 | 处理方式 | 示例 |
|---------|---------|------|
| 前端打印 | 前端 `DisplayHelper.GetXxxText()` 直接输出中文 | `GetProductionTypeText(item.ProductionType)` |
