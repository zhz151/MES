...
| 工段流转分析 | Mode B（内联）⓫ | SectionFlowAnalysisController | 批次计划页内嵌折叠（原独立页已删除 2026-08-24） |
| 工段待产量 | Mode B（内联）⓫ | SectionProductionStatusController | 经流转分析/段落流转分析上卷消费（原独立页已删除 2026-08-24） |
...
| 版本 | 日期 | 变更说明 |
|------|------|----------|
| **V2.11** | **2026-08-26** | **待发货项迁至订单上下文**：PendingDeliveryPrintHelper 所属上下文 仓库→订单（打印标题「订单成品(实时库存)」，PDF 文件名「订单成品(实时库存).pdf」）；§4.3 形态一 Helper 表行同步，§5.4 订单上下文补「订单成品(实时库存)」条目（Mode B Helper），§5.8 仓库上下文移除该行 |
...
| **V2.7** | **2026-08-15** | **文档失效内容清理**：核对 §4.3 形态二/§5.7 计划排程上下文中的工段流转分析（SectionFlowAnalysisService）与工段待产量（SectionProductionStatusService）——两个查询页面已从菜单栏移除，但 Service/Controller 及 print-file 端点仍存在、页面 .razor 仍在，打印条目保留；仅 Version bump（页面 .razor 已删除，见 V2.10） |
