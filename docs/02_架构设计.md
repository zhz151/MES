...
flowchart LR
    ...
    class SWD,SWDD,CPG,DOE,DPC,SFC,SFCI,WS,EMP,EDD,DVD,PFD config
```
...
| 工单上下文 | ✅ 已完成 | 工单生成、工单状态管理、工单主号追溯、订单变更检测、6类用料计划（含库料改制、在产改制）、**定尺工单（FixedLengthWorkOrder）联通视图**、**主号/次号工单号规则**（主号=管型前缀+2位序号，次号2位全模式非空） |
...
| **计划排程上下文** | **✅ V4.1 已完成** | **调度管理层——8 个页面：工单总览(ProductionOverview)宏观产能负荷、原锁计划(RawMaterialLockPlanAndExecution)原料锁定阶段调度、工单排程(WorkOrderSchedule)生产执行阶段排程、生产工段待产量现况(SectionProductionStatus)、生产段流转量分析(SectionFlowAnalysis)、批次计划(BatchPlan)在产明细计划、冷轧计划(ColdRollPlan)冷轧时间桶分布、成检计划(FinalInspectionPlan)待检验/检验中。工单需求调整(OrderDemandAdjustment) UI/API 已迁移至工单上下文，数据库实体保留在本上下文供 LEFT JOIN 引用。读模型字段「关注状态→主号关注(ScheduleStage)」「工单计划性→主号计划性(UrgencyLevel)」，ScheduleStage 五档全主号级** |
