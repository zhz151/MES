...
| 权限管理 | ✅ 已完成 | 43 角色（Admin + 14 主菜单 × Viewer/Editor/Full 三档，纯一级），基于 JWT 认证 |
| 用户管理 | ✅ 已完成 | 基于 Identity 的用户管理（基础） |
| 物料上下文 | ✅ 已完成 | 采购订单、委外加工、供应商档案（物料档案主档已删除） |
| 数据工具 | ✅ 已完成 | 数据导入导出（Excel），覆盖67个实体的导入/导出/模板/预览，跨上下文通用工具 |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| **V4.20** | **2026-08-30** | **无供需组统一产能平衡**：§六 计划排程上下文排程建议引擎——凡配了 MaxMachines 的组（无论有无供需链）都走产能平衡，无供需组（三辊/冷拔独立池、或取消供给目标组后的 5060）不再无条件固定「急+/急/急-」纯人工档，超上限向窄收 + 矛盾 A'、不足向宽放 + 矛盾 A；未配机台数上限保持默认原始档 |
| **V4.19** | **2026-08-29** | **删除冷轧机台组组角色字段（FlowRole 移除）**：§六 计划排程上下文「冷轧机台组配置」——组角色 None/Supplier/Demander 三值废除，供需链由 `SupplyTargetGroupKey` 显式表达（供给方=配了目标、需求方=被指向，每组天然可同时为供给方与需求方）；链合法性新语义——凡配供给目标组→目标存在 + 无环（去掉「目标非 None」约束，2030→冷拔(None) 多级链末端合法）；迁移 `20260829153516_RemoveFlowRoleFromColdRollMachineGroupConfig` DROP COLUMN FlowRole |
| **V4.18** | **2026-08-29** | **冷轧机台组供需链配置化（方案 A）**：§六 计划排程上下文「冷轧机台组配置」——新增 `SupplyTargetGroupKey` 字段显式表达供需链，强不变量从「Supplier/Demander 各恰 1」放宽为链合法性（供给方组必有目标/目标存在/无环——**「目标非 None」约束已随 V4.19 移除**），允许多条并行链/多级链；引擎 `ComputeFlowDemand` 按链对返回 (demandInflow, supplierOutflow)，多级链中间节点 flowState Role=Both；配置页新增「供给目标组」列 |
| **V4.17** | **2026-08-29** | **冷轧机台组归组配置化**：§四 数据流图排程上下文子图补充 ColdRollMachineGroupConfig 实体（CRMG）；§六 计划排程上下文模块说明补「冷轧机台组配置」——归组由编译期 `ColdRollMachineGroupKeys.Groups` 改为配置表驱动（FlowRole 三值，Supplier/Demander 各恰 1 强约束——**FlowRole 已随 V4.19 删除**），排机估算/排程建议机台类型组输入，工序多选/机型下拉/工段 Tab 联动配置化 |
| **V4.16** | **2026-08-15** | **文档失效内容清理**：§六 计划排程上下文页面 8→6（移除已从菜单栏下线的生产工段待产量现况/生产段流转量分析，备注段落流转分析一并下线）；配置上下文参数表 11→13（新增段落日产配置 SectionParagraphConfigs、组合归类表 CombinationGroups）；工单上下文用料计划 6类→7类（新增在产主工单）；数据工具覆盖实体数 67→68 |
| **V4.15** | **2026-08-09** | **主号化 + 定尺长度匹配 + 关注主号级等同步**：仓库「项次→主号」（InventoryBatch.ProductionMainNo）；定尺切割长度匹配 CutLengthMatchType（生产记录/成品检验/成品入库三模块，长度集合 FixedLengthWorkOrder）；读模型「关注状态→主号关注」「工单计划性→主号计划性」（ScheduleStage 五档全主号级）；入库状态四档（WoWarehousingStatus 0无入库/1部分/2完结/3超额，WarehouseThreshold 配置驱动）；配置上下文扩展为 11 个参数表（新增枚举显示配置/字典显示配置/工序组定义，含 DefaultSections）；数据流图配置上下文子图补充 3 个实体 |
...
| V3.7~V3.10 | — | （版本记录缺失，压缩为一行摘要，git log 可溯） |
| **V4.14** | **2026-08-02** | **批次成切存疑枚举化**：批次上下文 CutDoubt 字段从 bool? 改为 CutDoubtType? 枚举字符串，新增"疑问-缺少"判定（需求=是、执行=否、批次已到成检/完成且非强制完成 → 缺失成品切割记录） |
| **V4.13** | **2026-08-01** | **批次/成检/工单同步更新**：批次上下文补充批次成切跟踪四字段（CutRequirement/CutExecution/CutQuantity/CutDoubt），工序组制成倍数（ManufacturingMultiple）已删除、可改制换算改用批次级 ProductionRatio；质量上下文补充成检字段扩展（InspectionType/重量/定尺非定尺范围）；工单上下文补充定尺工单（FixedLengthWorkOrder）联通视图；版本历史顺序修正（V3.5/V4.12 按日期重排） |
| **V4.12** | **2026-07-26** | **文档同步更新**：版本号更新（数据工具覆盖实体数保持 67，无变化） |
