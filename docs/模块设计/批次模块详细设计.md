...
- 操作日志：状态变更等关键操作自动记录到 BatchOperationLog 表
...
| 工单确认 | 输入工单号 → 确认 → 有工单时29字段自动填充；工单号为"非工单"时清空工单字段（保留Specification/外径公差/壁厚公差/LengthStatus/MinLength/MaxLength不变，设置DeliveryState="固溶酸洗"、ManufacturingItem="备料成品"、PlantGrade=SourcePlantGrade、MaterialName="SeamlessPipe"、ProductionRatio=0） |
...
| 操作日志 | 第8区块，默认收起，点击"展开"加载并显示操作日志表格（操作类型/详情/操作人/操作时间） |
...
| 格式设置 | 5区块（批次基本信息/质量要求/仓库来源信息/工单信息/工序组）共72个字段，MudSwitch列显隐 |
| 列偏好持久化 | 通过 ColumnPrefsService 保存到 localStorage（key: `process_card_print_list`），切换开关自动保存 |
| 默认状态 | 格式设置面板默认收起（showSettings=false） |
| 批次选择 | MudTable 服务端分页 + 关键词搜索 + 状态筛选 + 多选勾选 + 列分组（与批次首页分组对齐） |
| 打印选中 | 勾选批次 → "打印选中"按钮 → 仅打印已选批次 |
| 打印全部 | "打印全部"按钮 → 打印所有批次 |
| PDF输出 | A4横向，表格行布局（标题行 → 表头行 → 数据行） |

**列分组（与批次首页对齐，G1/G2/G3 子集）：**
...
| 新建页 | `/section-outsources/create`：批量表格输入 + ColumnDisplaySelect + 重置；BatchNo 替代 ProductionBatchId。输入 BatchNo + ManufacturingSpec 后自动触发 `OnAutoFillAsync` 填充 TagNo/ProcessName/PlantGrade（同生产记录新建页） |
...
| 项目 | 说明 |
|------|------|
| 路由 | `@page "/outsource-recoveries"`（新建：通过工段委外页"批量回收"入口，弹出 RecoveryCreateDialog） |
| 授权 | BatchStaff/BatchDirector/Admin |
...
| 归属 | 菜单已移至质量管理模块，后端实现仍在批次上下文（ProductionRecordService + MaterialReceiveCheck 实体） |
| 表格 | MudTable 服务端分页 + auto-table，**内联编辑** |
...
- **执行序号范围验证（3A）**：新建生产记录时，该批次的 SequenceNumber 不得超过"该批次最近有记录的日期的最大 SeqNumber + 7"。即取该批次所有 ExecDate < 当前记录日期的记录中，ExecDate 最大的一天对应的最大 SequenceNumber + 7 作为上限。使用 .Date 截断日期部分进行比较（2026-05-12 修正，2026-05-24 改为按记录日期对比而非固定"今天"）
...
- 所有批次状态变更（暂停/恢复/强制完成/转为在产/作废）自动记录到 BatchOperationLog 表
- 操作类型：状态变更（详情格式：`状态变更: {oldStatus} → {newStatus}`）
- **有效数量变更**：CurrentValidQty/CurrentValidWeight 数值变化时，自动记录操作类型"有效数量变更"（详情格式：`有效数量变更: 有效支数={old}→{new}, 有效重量={old}→{new}kg`）
- 日志与状态变更在同一事务中写入，保证一致性
- 操作日志在批次详情页默认收起，点击"展开"加载
...
- **已知枚举字符串字段**：
  ...
  - `ManufacturingItem` → `DisplayHelper.GetManufacturingItemText()`（代码值→中文映射：OrderFinishedProduct→订单成品、PreparedMaterial→备料成品、SurplusStock→余库料、IntermediateProduct→中间品、SpecialDeliveryStatus→特定交态成品）
...
- **成检到料（MaterialReceiveCheck）**：关联 ProductionBatch，单个批次唯一一条到料记录（UK约束）。菜单归属质量管理，后端实现仍在批次上下文（ProductionRecordService）
...
| 请求DTO | `MES.Core/DTOs/ProcessCardPrintRequest.cs` |
| 纸张 | A4横向（Landscape） |
| 字体 | SimSun，默认10pt，数据标题11pt，单元格9pt |
| 布局 | 5区块：批次基本信息(1行) → 质量要求(1行) → 仓库来源信息(1行) → 工单信息(2行) → 工序组(动态表格) |
| 区块样式 | 每个区块 = 灰底标题行(ColumnSpan) + 灰底表头行(字段名) + 数据行(数值) |
| 工序组 | #序号 + 动态列表格（列选择器控制），头部分离使用table.Header() |
| 列宽 | RelativeColumn 均分 |
| 列选择持久化 | localStorage (key: "process_card_print") |
| 隐藏字段默认值 | "-" |
...
| GET | `api/production-record/{batchId}/material-check` | 查询检验到料 |
| POST | `api/production-record/material-check` | 创建检验到料 |
...
| GET | `api/production-record/all/material-checks` | 跨批次查询所有检验到料（分页） |
| POST | `api/production-record/outsources/batch` | 批量创建工段委外 |
| POST | `api/production-record/recoveries/batch` | 批量创建委外回收 |
| POST | `api/production-record/material-checks/batch` | 批量创建检验到料 |
| PUT | `api/production-record/material-check/{id}` | 更新检验到料 |
| DELETE | `api/production-record/material-check/{id}` | 删除检验到料 |
| POST | `api/production-record/print-batch` | 批量打印生产记录 |
| POST | `api/production-record/print-all` | 全部打印生产记录 |
| POST | `api/production-record/material-check/print-batch` | 批量打印检验到料 |
| POST | `api/production-record/material-check/print-all` | 全部打印检验到料 |
| GET | `api/production-record/all-list` | 获取所有生产记录列表（无分页） |
| GET | `api/production-record/all/filter-contexts` | 生产记录筛选上下文 |
| GET | `api/production-record/material-check/filter-contexts` | 检验到料筛选上下文 |
| GET | `api/production-record/material-receive-checks/all-list` | 获取所有检验到料（无分页） |
