...
| 内联编辑 | _editingIds/_editCache 模式，支持 15 个可编辑字段（ExecDate/EquipmentName/Operator/Shift/Quantity/Weight/SolutionTemperature/SoakTime/FaceCutCount/CuttingMultiple/FinishedCutLength/PostCutQuantity/TagNo/PlantGrade/Remark），ProductStatus 为系统计算字段不可编辑 |
...
| G2 产出数据 | 2 | col-g4（橙） | 10 | EquipmentName, Operator, Shift, Quantity, Weight, ProductStatus, CuttingMultiple, FinishedCutLength, PostCutQuantity, FaceCutCount |
...
| 列分组 | 前端通过 initGroupHeaders JS 函数实现：G1(批次信息: BatchNo/ProcessName/ManufacturingSpec) / G2(入缸信息: InDate/SectionName/EquipmentName/Operator/Shift/Quantity/Weight/ProductStatus/TagNo/PlantGrade) / G3(完工信息: Status/CompleteDate/Remark) / G4(操作: CreatedTime/UpdatedTime 默认隐藏) |
...
- 每个工序组必须填写工序名称
- 工序名称：来自 `ProcessNames.All` 常量数组（荒管处理/在制修检/60冷轧/50冷轧/30冷轧/20冷轧/三辊冷轧/冷拔/附加成检），前端 MudSelect 选项由此生成
