...
| 版本 | 日期 | 变更说明 |
|------|------|----------|
| **V5.9** | **2026-08-27** | **合并投料明细改快照（脱离实时 JOIN）**：§6.3 ProductionBatchInventory 新增 7 个来源快照字段（BatchNo/HeatNo/PlantGrade/Specification/MaterialType/SourceName/WarehouseName），创建/编辑批次时从关联库存批次复制，**合并投料明细展示走快照，不再实时 JOIN 仓库**；保留 InventoryBatchId/OutboundRecordId 用于删除保护与追溯；历史存量由迁移 `20260827100105_AddProductionBatchInventorySnapshotFields` 回填，详情 DTO 快照为空时兜底回退实时 JOIN |
