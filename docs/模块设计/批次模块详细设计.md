...
| 列选择器 | ColumnDisplaySelect + 重置按钮，列偏好通过 ColumnPrefsService 持久化到 localStorage（key: `section-outsources`，2026-09-09 并入 `ColumnPrefsVersion="v2"` → `col_prefs_section-outsources_v2` 强制新默认显隐） |
| 排序列头 | 带排序箭头（▲▼），`ToggleSort(col.SortKey)` 切换升降序，默认按创建时间降序 |
...
| 默认显隐（2026-09-09 v2） | 委外信息 组 新增默认隐 **要求收回日期/紧急**（工单号/执行序号/备注/数据来源/更新时间 此前已隐）；回收信息 组 默认隐 **非正常回收(支)/非正常回收(重)/回收备注**。隐藏列均可经列选择器开启 |
