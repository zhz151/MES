...
- **过程检验（4值）**：`ExecDate=最近检验日`（跨日多次 `ExecDateStart=最早`）；`Operator=检验员聚合`；**合格支/重** = `QualifiedQuantity/QualifiedWeight` 合计；**次品支** = 返整+入库+报废支数之和（DefectRework+DefectWarehouse+DefectScrap，跨多条检验记录累加）；**次品重** = 三项理论重之和（`TheoreticalRework+TheoreticalWarehouse+TheoreticalScrap`，跨记录累加）
...
- **4 值只统计正式成检记录**：`QualifiedQuantity/QualifiedWeight`=正式合格支/重累计、`DefectQuantity/DefectWeight`=正式 返整+入库+报废 支/重累计；**预检记录不并入数量**（2026-09-06 拍板）
