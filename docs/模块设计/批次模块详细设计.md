...
| 列分组 | 新增 **G4「成品切割跟踪」** 分组（GroupKey=4）：CutRequirement/CutExecution/CutQuantity/CutDoubt + 中文显示字段（是/否/略/疑问-数量/疑问-缺少/正常），MudChip 渲染（疑问-数量=红/疑问-缺少=橙/正常=绿）。计算规则详见 §4.15 批次成切跟踪 |
...
| 查看模式 | 字段只读，显示"编辑""打印"按钮。**9个区块**：批次基本信息 → 跟踪可视化 → 执行信息 → 质量要求 → 投料信息 → 工单信息 → 工序组列表 → 操作日志。其中「区块二b 成切跟踪」位于「执行信息 → 成切跟踪 → 批次执行进度」。工单信息区块显示当前快照值，字段为空或默认值时自动显示「-」保护（采用 FmtDate/FmtStr 辅助方法） |
...
- 四字段语义（与上述跟踪字段同步计算，`ComputeCutTracking` 在 `ComputeTheoreticalOutput` 之后执行）：
  ...
  - **CutDoubt（成切存疑）**：`CutDoubtType` 枚举字符串（`QuantityMismatch`/`MissingRecords`/`Normal`，null=略），判定分三种情形——
    - 无需求（需求=否）→ `null`（略）
    - 有需求无断切记录（执行=否）：批次已到成检/完成 且 非强制完成 → `MissingRecords`（疑问-缺少，缺失成品切割记录）；否则 → `null`（略）
...
- 不计算 `HasInputChange`（因算法需使用各自上下文中的 InputQuantity/CurrentValidQty）；跟踪计算链为三步：`HasInputChange` → `ComputeTheoreticalOutput` → `ComputeCutTracking`
