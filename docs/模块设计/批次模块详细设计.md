...
| 列选择器 | ColumnDisplaySelect + 重置，列偏好 key `outsource-vendors` v1，PageState 同 key |
| 列定义 | 编码(VendorCode，默认隐藏) → 委外单位名 → 委外工段(中文下拉/筛选项) → 本厂/外协(IsWorkshop boolean) → 联系人 → 联系电话 → 备注 → 状态(IsActive：启用/停用 chip) |
...
| 关联口径 | 委外单文本不存档案 Id（保持 OutsourceVendor 字符串），**历史 239 行委外单文本不动**；委外单四元组重复/导入导出复合键/按单位汇总均不回卷；本档案非外协合同单，无金额统计 |
