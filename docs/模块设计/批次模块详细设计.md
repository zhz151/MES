...
> 版本：V5.13（2026-08-28）
> 状态：**已实施完成**。批次创建（工单 30 字段快照冗余 + 可用库存批次合并投料/非仓库模式列表输入）、工序组管理（复制上个批次/按批次号调取）、批次列表全字段排序筛选 + G1-G11 列分组与派生列（执行匹配/流转判定/成切跟踪/成检附加/过程检字段/产品单支量等）+「批次-错疑执行」4 类统计折叠卡片联动筛选、批次详情/编辑（5 区块仿新建布局）、批次及其子记录打印（工艺流转卡/生产记录/工段委外/委外回收/去油酸洗入缸与完工）、批次跟踪可视化与派生字段计算（生产记录/工段委外/过程检验/成检到料/仓库入库五类记录综合）。操作日志统一由基础设施 `OperationLog`（Module="Batch"）记录。
...
| 跟踪可视化 | 第2区块，显示8个卡片展示批次各执行阶段状态（待执行/执行中/已完成/不涉及）；各工段名称下方显示执行日期(MM-dd)（从生产记录/过程检验/检验到料[检验工段匹配]/工段委外/检验到料[末段回退]五类记录中按优先级获取）；显示"现有效原料投料量"；进度百分比排除"入库"工段 |
| 返回按钮 | "返回批次列表" → /batches |
