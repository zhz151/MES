...
| 排序 | BatchNo/Status/WorkOrderNo/CreatedTime 等全字段，`CurrentSectionCompleted` 按 `bool?` 排序（null→false→true），`HasInputChange` 不可排序（SortKey=null），`RemainingWorkDays` 按 int 值排序；**`FlowJudgment`（V5.10 起可排序）按 疑问→正常 序（疑问=0 在前，与 ExecutionMatch 同向），升/降序档位方向一致、同档按 Id 稳定**；`ScheduleStage`/`ExecutionMatch`/`FlowJudgment` 三派生字段均为后端拦截内存/子查询排序，非 SQL 反射排序 |
...
| 列分组 | **G1「批次基本信息」**（GroupKey=1）；**G2「关联工单状态」**（GroupKey=2）：工单关注（ScheduleStage 0-4 档/略/无此工单）+ 执行匹配（错误/正常，错误=MudChip Error 红章）；**G3「现执行状态」**（GroupKey=3）：状态/强制完成 + **成检附加（InspectionStage，预检/终检/空）**；**G4「流转判定」**（GroupKey=4，2026-08-28 新增）：FlowJudgment 正常/疑问，疑问=MudChip Error 红章，**V5.10 起可排序（疑问在前）与列头多选筛选（正常/疑问）**，判定规则见 §4.15「流转判定」；**G5「有效投料变更」**（GroupKey=5）含 **6 个过程检字段**（过程检合格支/合格量/理论成品支/需调整/缺陷-返整量/缺陷-纯次品量）+ HasInputChange + 理论三字段 + 现有效支数/重量；**G6「成品切割跟踪」**（GroupKey=6）：CutRequirement/CutExecution/CutQuantity/CutDoubt + 中文显示字段（是/否/略/疑问-数量/疑问-缺少/正常），MudChip 渲染（疑问-数量=红/疑问-缺少=橙/正常=绿）；**G7「生产执行」**（GroupKey=7）：截止执行日/当前工序/当前工段/工段完工/当前设备/当前委外/当前规格/下一工段/对应规格/下一工序/全工量/剩余工量；**G8「原始投料信息」**（GroupKey=8）；**G9「工单信息」**（GroupKey=9）；**G10「产品要求」**（GroupKey=10）新增 **产品单支量（ProductUnitWeight）**；**G11「质量要求」**（GroupKey=11）。计算规则详见 §4.15 |
| 操作 | 打印选中 + 打印全部 + 查看详情 + 工艺卡打印 |
| 通知轮询 | 30秒定时器自动轮询加载在产改制计划通知、在产主工单计划通知、工单号核查结果、工单变更通知、**成检到料强制完成通知（2026-08-17 起为 2 分钟轮询）**。通过 `StartNotificationPollingAsync()` + `CancellationTokenSource` 实现，页面销毁时通过 `IAsyncDisposable.DisposeAsync()` 取消轮询 |
...
**「批次-错疑执行」折叠卡片（V5.13 新增）：**

| 项目 | 说明 |
|------|------|
| 入口 | CardHeader 右上侧折叠按钮「批次-错疑执行」（展开态 Color=Warning + ExpandLess 图标，折叠态 Color=Default + ExpandMore 图标） |
| 位置 | 展开后显示在表格上方右侧（`d-flex justify-end` 右对齐，MudCard `summary-card` 宽 420px，`flex-shrink:0`） |
| 数据源 | `GET api/batch/doubt-execution-summary`（后端 `GetDoubtExecutionSummaryAsync`，复用 `GetAllBatchListAsync` 全量批次派生字段统计，4 项固定顺序） |
| 加载 | **默认折叠**，首次展开时懒加载（`_doubtExecutionItems == null && !_doubtExecutionLoading`），加载中显示「正在加载...」 |
| 卡片结构 | 两列表格（表头：错疑类别/批次数量）+ 底部提示「点击数据值联动筛选批次列表」 |
| 4 类统计口径 | ①**匹配工单**（`ExecutionMatch = "错误"`）②**工段流转**（`FlowJudgment = "疑问"`）③**有效投料**（`ProcessInspectionNeedAdjust = true`）④**成品切割**（`CutDoubt ∈ {QuantityMismatch, MissingRecords}`）——与列表列筛选口径完全一致 |
| 批次数量显示 | `N批/SKg`（Kg=`InputWeight` 领料重量合计，`ToString("G29")` 去零）；N=0 时显示 `-` |
| 点击联动 | 点击「批次数量」数据值 → 按对应条件写入列筛选 `_columnFilters` → `OnColumnFilterChanged(fieldKey, values)` → 表格 `ReloadServerData()` 刷新。对应关系：匹配工单→`ExecutionMatch=["错误"]`、工段流转→`FlowJudgment=["疑问"]`、有效投料→`ProcessInspectionNeedAdjust=["True"]`、成品切割→`CutDoubt=["QuantityMismatch","MissingRecords"]`。与列头 ExcelFilter 同通道，可手动清除 |
| 取消筛选按钮 | 卡片标题右侧，**仅当卡片联动筛选生效时显示**（`HasActiveDoubtFilter`=联动字段仍在 `_columnFilters` 中）；点击 `CancelDoubtExecutionFilterAsync` 仅清除**本卡片触发的全部列筛选**（`_doubtActiveFilterFields` 记录，不误伤列头手动筛选）→ 刷新列表。联动字段被列头手动清空时标记同步失效、按钮自动隐藏 |
| 样式 | 可点击数据值 `estimate-cell` class + `cursor:pointer; font-weight:600; color:#c62828;`；N=0 时置灰 `color:#bdbdbd` 不可点击 |
...
| 授权 | 查看：BatchViewer/BatchEditor/BatchFull/ReportViewer/ReportEditor/ReportFull/Admin；编辑：BatchViewer/BatchEditor/BatchFull/ReportViewer/ReportEditor/ReportFull/Admin（按钮级权限控制）/BatchEditor/BatchFull/Admin |
...
> 执行序号（SequenceNumber）已从页面移除，由服务端根据 ProcessGroupId + SectionName 自动计算。**V5.11**：一旦定位工序组即**强制对齐**工段步骤号（忽略前端传入值，仅工段不在工序组时保留前端值兜底），Update 时复用已加载工序组列表重对齐（纠正工序组编辑后的序号漂移）。
...
#### BatchReportDialog（已删除 2026-08-24）

| 项目 | 说明 |
|------|------|
| 文件 | `MES.Blazor/Shared/BatchReportDialog.razor`（已删除） |
| 用途 | 批次详情页内联的批量生产记录录入对话框（**已删除**，生产记录统一改走独立页 `/production-records` 录入，批次详情页 `BatchDetail.razor` 的 `ProdRecordService` 仅用于跟踪可视化） |
...
- 执行序号 SequenceNumber 由服务端自动解析：ProcessGroupId + SectionName → SequenceNumber（来自工序组中该工段的顺序值）。**V5.11 强制对齐**：创建/批量创建一旦定位工序组即强制取工段步骤号，忽略前端传入值；Update 复用已加载工序组列表重对齐（仅工段不在工序组时保留原值兜底）
...
- **执行序号强制对齐（V5.11）**：创建/批量创建一旦定位工序组即强制取工段步骤号（忽略前端传入值），Update 复用已加载工序组列表重对齐（仅工段不在工序组时保留原值兜底）
...
- 五类记录按其 SequenceNumber 统一排序，取最大值（overallMaxSeq）对应的记录填充当前跟踪字段：
  ...
  - **工段委外最大**：取 ProcessName/SectionName/OutsourceVendor；**委外日期口径（A，2026-08-28）**：已完工（有回收记录 RecoveryCount>0）取最大回收日 MaxRecoveryDate，未完工取发出日 SendOutDate
...
- **全局 CurrentExecDate（截止执行日，2026-08-28 统一口径）**：生产/委外/过程检验/到料/入缸 五路日期取最大（`new[]{ ExecDate, 委外A口径日期, InspectionDate, 到料日期, 酸洗A口径日期 }.Max()`）；委外/酸洗去油按 A 口径（已完工取最大回收日/完工日，未完工取发出日/入缸日），与「流转判定」6 表全量 max 口径一致
...
- 判定规则：
  ...
  - **规则2（序列连续性，V5.12 放宽）**：最大执行序 N 存在时，N-1/N-2 **至少一个存在**即正常（允许一个无值，容忍中间隔一道不产生记录的工序），两者均缺失 → 疑问；补位 ≤0 跳过；未产（6 表均无记录）→ 正常
  - **特判**：已入库 → 位置日期 = InboundDate（入库为终点覆盖）
  - 两规则均符合 → 正常
- 前提说明：SequenceNumber 批次全局连续（BatchEdit 强制 1 起连续且不重复），规则2 依赖该前提；「截止执行日」同口径（见上文全局 CurrentExecDate）。**V5.11**：存量缺号/错位由数据工具 `FixAllSystemFields` 修复——`FixProcessGroupSectionNumbersAsync` 按批次将工序组全部非空工段步骤号重排为 1..N 连续（仅补缺号、不改相对顺序），随后 5 类记录按 `(ProcessGroupId, SectionName)` 对齐工段步骤号（入缸/成检到料按 ProcessGroupId 直接定位）
- **排序/筛选（V5.10 起）**：FlowJudgment 非实体字段且依赖 6 表复杂计算无法 SQL 下推，采用**内存路径**——仅当 `SortBy=FlowJudgment` 或 Filters 含 FlowJudgment 时触发（GetPagedAsync 先拦截移除通用 Filter），全量加载候选批次 → `BuildFlowJudgmentMapAsync` 批量计算 → 内存筛选（in 正常/疑问）→ 内存排序（疑问=0 在前，升/降序档位方向一致，同档按 Id 稳定）→ 内存分页；未命中时保持 SQL 分页路径不变。前端列定义 SortKey=FlowJudgment + FilterType=enum（正常/疑问）。数据量（592 批次）下内存路径可行，疑问批次集中在前便于人工核查
...
- **执行序号解析**：SequenceNumber 由服务端自动通过 ProcessGroupId+SectionName 解析。**V5.11 强制对齐**：创建/批量创建一旦定位工序组即强制取工段步骤号（忽略前端传入值），Update 复用已加载工序组列表重对齐；解析值为0则返回 BusinessException 拒绝提交
...
- **成检到料（MaterialReceiveCheck）**：关联 ProductionBatch + ProcessGroup，每道检验工序组唯一一条到料记录（UK_MaterialReceiveCheck_ProcessGroup），新增 ProcessGroupId/ProcessName/SequenceNumber 字段用于多工序组支持。执行序号=检验工段步骤号（pg.Inspection），创建/批量创建强制对齐（V5.11），Update 重选工序组时联动刷新。菜单归属质量管理，后端独立实现在质量上下文（MaterialReceiveCheckService）
...
- **来源快照（2026-08-27）**：创建/编辑批次时从关联库存批次复制 7 个快照字段（BatchNo/HeatNo/PlantGrade/Specification/MaterialType/SourceName/WarehouseName）到 ProductionBatchInventory，**合并投料明细展示走快照，不再实时 JOIN 仓库**；保留 InventoryBatchId/OutboundRecordId 用于出库/批次删除保护与领料追溯；历史存量由迁移 `20260827100105_AddProductionBatchInventorySnapshotFields` 回填，详情 DTO 快照为空时兜底回退实时 JOIN
...
| POST | `api/batch/print-all-file` | 全部批次打印（PDF 文件直接返回） |
...
| 版本 | 日期 | 变更说明 |
|------|------|----------|
| **V5.13** | **2026-08-28** | **批次首页新增「批次-错疑执行」折叠卡片**：①后端新增 `GetDoubtExecutionSummaryAsync`（复用 `GetAllBatchListAsync` 全量批次派生字段统计）+ `BatchDoubtExecutionItemDto`（枚举 `BatchDoubtExecutionType` 4 类 + DoubtTypeText + BatchCount + InputWeight）+ 端点 `GET api/batch/doubt-execution-summary`；②§3.1 卡片：CardHeader 右上折叠按钮，展开后表格上方右侧 420px `summary-card`，两列（错疑类别/批次数量）4 行，4 类统计口径（匹配工单=`ExecutionMatch="错误"`、工段流转=`FlowJudgment="疑问"`、有效投料=`ProcessInspectionNeedAdjust=true`、成品切割=`CutDoubt∈{QuantityMismatch,MissingRecords}`），批次数量显示 `N批/SKg`（Kg=`InputWeight` 领料重量合计），**点击数据值写入列筛选联动刷新批次列表**（与列头 ExcelFilter 同通道），默认折叠懒加载；③**联动取消按钮**：卡片标题右侧「取消筛选」（仅联动筛选生效时显示，`_doubtActiveFilterFields` 记录联动字段仅清本卡片触发筛选不误伤列头手动，字段被手动清空则标记失效自动隐藏）；④前端 Blazor Service 加 GetDoubtExecutionSummaryAsync + Batches.razor 卡片渲染；⑤测试新增 1 用例（5 批次 4 类各命中 count/weight 断言），全量 3545 通过 0 警告 |
| **V5.12** | **2026-08-28** | **流转判定规则2放宽（读法A）**：`ComputeFlowJudgment` 规则2 由「N-1/N-2/N-3 任一缺失→疑问」改为「N-1/N-2 **至少一个存在**即正常（允许一个无值），两者均缺失→疑问」（去掉 N-3 检查；补位≤0 跳过、未产正常不变）。理由：6 表记录类型混排下中间工段可不产生记录（委外未回收/未报工/检验无记录），原前三档全连续将正常批次误报为疑问。§4.15 判定规则同步；测试用例更新（N-1 缺失但 N-2 存在→正常，新增 N=3/N=4 两档全缺失→疑问边界），全量 3544 通过 0 警告 |
| **V5.11** | **2026-08-28** | **记录增改序号强制对齐 + 数据工具工序组连续化**：①**语义A（序号=工段步骤号）落地**——生产记录/工段委外/入缸酸洗/过程检验/成检到料 5 类记录 Create/BatchCreate 一旦定位工序组即**强制对齐** `pg.GetSectionSequence(SectionName)`（成检到料=`pg.Inspection`），忽略前端传入值（仅工段不在工序组时保留前端值兜底）；Update 复用已加载工序组列表重对齐，纠正工序组编辑后的序号漂移；②**数据工具扩展**——DataFixService 新增 `FixProcessGroupSectionNumbersAsync`（按批次将工序组全部非空工段步骤号重排为 1..N 连续，仅补缺号不改相对顺序，先连续化再记录对齐），`FixSequenceNumbersAsync` 覆盖入缸（按 ProcessGroupId 定位）与成检到料（=pg.Inspection），DataFixReport 新增 ProcessGroupSectionNumbersFixed 字段，`fix-all-system-fields` 端点返回文案同步；③ProcessGroupExtensions 新增 `SetSectionNumber` 逆操作；④§3.1/§3.9/§3.11/§3.12/§4.6/§4.7/§4.9/§4.13/§4.15/§4.18/§6.1 序号语义描述同步；⑤测试增补 5 个用例（生产记录创建对齐/更新重对齐、成检到料强制检验深度、工序组连续化压缩/不改变），全量 3540 通过 0 警告 |
| **V5.10** | **2026-08-28** | **流转判定排序/筛选（内存路径）**：①FlowJudgment 支持列头排序（疑问=0 在前，与 ExecutionMatch 同向，同档按 Id 稳定）与列头多选筛选（正常/疑问）；②因依赖 6 表复杂计算无法 SQL 下推，GetPagedAsync 采用**内存路径**——仅当 SortBy=FlowJudgment 或 Filters 含 FlowJudgment 时触发（先拦截移除通用 Filter），全量加载候选批次 → BuildFlowJudgmentMapAsync 批量计算 → 内存筛选/排序/分页，未命中时保持 SQL 分页不变；③抽取 `MapBatchList` 私有方法（GetPaged/GetAll 两处内联映射合一）；④前端列定义加 SortKey/FilterType/EnumOptions，§3.1 排序/列分组行更新；⑤测试增补 2 个集成用例（筛选只返回匹配判定 / 排序疑问在前），全量 3535 通过 0 警告 |
| **V5.9** | **2026-08-28** | **流转判定字段 + 委外/酸洗完工日期口径（A）**：①批次列表新增「流转判定」组（G4，GroupKey=4）单字段 FlowJudgment（正常/疑问，疑问=MudChip Error 红章），6 表记录派生（生产/委外/去油酸洗/过程检验/成检到料/仓库入库），规则1=现最大执行序位置日期≠6表全量max、规则2=N-1/N-2/N-3 任一缺失→疑问，入库=终点覆盖（位置日期=InboundDate）、未产=正常，列表/导出/打印三路径同源（BuildFlowJudgmentMapAsync 按批次批量加载，Chunk 分批防 IN 上限，A 口径同款）；②§4.15 委外/去油酸洗日期口径（A）：已完工取最大回收日/完工日，未完工取发出日/入缸日，CurrentExecDate 同步为五路日期取最大（生产/委外/过程检验/到料/入缸）；③DTO 加 FlowJudgment 派生字段，§3.1 列分组更新（G1-G11 全分组重排）；④测试 9 用例（正常/疑问 Theory，反射调用 ComputeFlowJudgment） |
| **V5.8** | **2026-08-21** | **生产量汇总显示优化**：§3.5 生产记录列表页补「近日生产量数据」「月度生产量数据」折叠卡片说明（行=26 工段+合计，统计生产量 t，口径与批次计划页一致）；重量单元格 0 值留空（FormatT）+ **整行全 0 的工段默认隐藏（合计行始终保留）**（Service `GetSummaryAsync`/`GetMonthlySummaryAsync` return 前 RemoveAll，两页三表一次生效） |
| **V5.7** | **2026-08-17** | **成检到料强制完成闭环通知**：批次首页新增「成检到料强制完成」通知栏（§3.1 通知轮询补 `LoadForcedCompletedInspectionBatchesAsync`，调用 BatchService.GetForcedCompletedInspectionBatchesAsync，2 分钟轮询）——判定=成检到料 IsForceCompleted=true 且批次仍处成检（Status=InFinalInspection）的异常完成批次，通知批次号 MudLink 跳详情页；闭环=详情页将批次强制完成为「完成」后 Status 脱离 InFinalInspection → 通知自动消失 + 成检计划候选自动脱离。本模块强制完成/IsForceCompleted 字段（批次表、成检到料表）沿用既有文档，无数据库变更 |
