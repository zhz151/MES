...
| 列分组 | 新增 **G4「成品切割跟踪」** 分组（GroupKey=4）：CutRequirement/CutExecution/CutQuantity/CutDoubt + 中文显示字段（是/否/略/疑问/正常），MudChip 渲染。计算规则详见 §4.15 批次成切跟踪 |
...
- 四字段语义（与上述跟踪字段同步计算，`ComputeCutTracking` 在 `ComputeTheoreticalOutput` 之后执行）：
  ...
  - **CutDoubt（成切疑问）**：需求=否 或 执行=否 → `null`；`|成切支数−理论成品支|/理论成品支 > 5%` → `true`（疑问）；否则 `false`（正常）；理论成品支不可得 → `null`
