...
| 可持续天数 | SustainableDays | 整数显示，decimal?，默认隐藏 |
| 总况判定 | StatusJudgment | 字符串，对应 MudChip 颜色：偏少=Error，过多=Warning，正常=Success |
| 计划重点批流转量 | PlanFlowQuantity | 整数显示，decimal?，批次计划中流转=是的重量汇总（吨）；（2026-08-21 原"计划流转量"改名） |
| 计划流转判定 | PlanFlowJudgment | 计划重点批流转量 > 日流转设定 → "加速"（Warning），否则 "-" |
| 特急批重量 | PlanKeyWeight | 整数显示，decimal?，流转=是且等级=急+的重量汇总（吨）；（2026-08-21 原"重点批重量"改名） |
...
- 统计量 = 成品检验记录检验重量 Weight(kg) /1000 显示 t；预成检/正式成检合并统计；**0 值留空 + 整行全 0 的检验项目默认隐藏（合计行始终保留）**
