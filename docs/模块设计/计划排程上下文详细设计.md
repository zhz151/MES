...
> 版本：V5.53（2026-09-16）
> 版本变更（V5.53 2026-09-16）：**文档与代码对账（纯文档修订，第二轮；零代码/迁移/数据改动）**——① 概述状态行「工单总览（19 行产能负荷聚合）」更正为 **「12 + 机台组数 行（4 机台组种子 = 16 行）」**（源 `ProductionOverviewService` 行构建：原料锁定 4 + 荒管抛光 1 + 机台组 N + 在产汇总 1 + 成检 2 + 整体完工预计 1 + 订单交期负荷 3 = 12 + N，与 PO-001 口径对齐）；② §4.6 工单排程列分组补漏：G1 基础数据补 `EndCustomer`（最终客户，默认隐藏，`Pages/Scheduling/WorkOrderSchedules.razor.cs` 实有 22 列）、**G14 在产节点待量**补 `MainNoAttentionProcess`（主号-关注工序）与 `MaxBatchRemainingWorkDays`（最大剩余工量(天)）两列（实有 13 列）；③ §4.7 生产计划顶部工具栏更正为 **4 个按钮**（近日生产量数据 / **实时委外在产** / 段落流转分析 / 计划安排）——原记「显示类汇总」按钮**全仓无页面实现**（`ProductionSummaryHelper.SummaryAllSectionTabs` 仅被后端 `BatchPlanService` 汇总/段落端点消费），并新增「实时委外在产」折叠卡片说明（`GET api/batch-plan/outsource-pending`，行=在产单位含合计行、列=工段(t)+合计(t)，单元格「总量/[流转]/[*特急]」，可打印）；同页列定义总数 **76 → 77**；§2.8「Tab 汇总」更正为**前端内存计算的 6 指标**（原「通过 `PagedResult.Extras` 返回后端聚合结果」已过时）；④ §4.8 冷轧排程：工段筛选 Tab 由固定列表改**配置驱动**（`GET api/process-definition/cold-roll-options`，仅 `IsEnabled=true`，加载失败降级为仅「全部」）、顶部按钮补「排程建议[自动]」「排程汇总」「机台数配置」「最后更新」且「编辑排程」正名为**「编辑排程[手动]」**、Tab 汇总**仅「规格数 + 总重量(kg)」**（原记含重点批次数/重量为过时）、补齐四个折叠区（排机估算 / 机台数配置 / 冷轧排程建议 / 排程汇总含近2·4·6天切换）、分页汇总列 **12 → 16 列**；⑤ §4.8 **删除已废止的「待轧序（RollOrder）」全部描述**——该字段已由迁移 `20260610061116_DropRollOrderFromColdRollSpecSchedule` 从 `ColdRollSpecSchedule` 删除，全仓（Entity/DTO/Service/前端）**零引用**；同步更正 §一概述第 3 条字段枚举、§2.9 冷轧排程流程图（用户编辑列 + 保存动作）、§4.8 数据分组 G6（**5 列 → 4 列**：CompletionType/RollType/SchedMachineNo/DailyOutput，列标签补齐为「在轧要求/待轧要求/待轧单位或设备/单机单日量(kg/天)」）、§4.8「排程编辑模式」（**5 个排程设置列 → 4 个**）、§2.9「排程编辑操作」步骤 2/5（原「验证规则：待轧要求非无计划时待轧序必须 > 0」**源码无此校验**，改为实有的「保存时合并未在当前视图内的现有排程记录原值回写」）；⑥ §4.8 数据分组 G2/G3/G5 补齐列标签（G2 = 在轧单位或设备/在轧/在轧(急+)/在轧(急)/在轧(急-)、G3 = 近日待轧/近日待轧(急+)/近日待轧(急)/近日待轧(急-)、G5 = 工艺总量），原记「急+/特急」「急/特急-」等档位串已过时；⑦ §4.9 成检计划两处更正：分页选项 **10/25/50/100 → 10/20/50/100**（源 `MudTablePager PageSizeOptions`）、紧急程度 MudChip 颜色更正为 **`APlusUrgent`→Error(红) / `AUrgent`→Warning(黄) / `BOrder`→Info(蓝) / 其余→Default**（原文误把 A急写作红、B顺写作黄）；⑧ §4.3 日期桶边界补注**配置表驱动**（`DateBucket` / `Bucket1~Bucket5`，默认 7/15/30/45/60，源 `ProductionOverviewService.GetConfigAsync`）。
> 历史变更（V5.52 2026-09-16）：**文档与代码对账（纯文档修订）**——① §2.1 / §4.1 工单需求调整页路由更正 `/order-demand-adjustment` → **`/workorders-demand-adjustment`**（源 `Pages/WorkOrders/OrderDemandAdjustment.razor` 的 `@page`），页面名称「工单需求调整页面」保持不变；② §4.1 导航位置「工单管理 → 工单需求调整」更正为「工单管理 → 需求调整」（`AppMenu.cs` 二级菜单文案）；③ §4.2 原锁计划页导航位置原记「计划排程 → 原锁计划」**不符实**，改为「当前无菜单入口」（计划排程组实为 5 项，本页未挂载、全仓无跳转引用）；④ §4.3 标题「负载总览页面」更正为「订单负荷总量页面」、导航位置与权限同步更正（`SchedulingView` 系列，源 `Pages/Scheduling/PlanOverview.razor:2`）；⑤ 四页「前端 Service」路径补 `Scheduling/` 子目录前缀（607/688/789/880 行，源码实位于 `MES.Blazor/Services/Scheduling/`）；**⑥ 勘误**——其中 §4.6 工单排程页的 `WorkOrderScheduleService` **不在 `Services/Scheduling/`**，实际位于 `MES.Blazor/Services/WorkOrder/WorkOrderScheduleService.cs`（其余三页 `BatchPlanService` / `ColdRollPlanService` / `FinalInspectionPlanService` 确在 `Services/Scheduling/`），故 §4.6 的前端 Service 路径更正为 `MES.Blazor/Services/WorkOrder/WorkOrderScheduleService.cs`。
> 历史变更（V5.50 2026-09-11）：**成检计划 G6「检验的数量信息」组随成品检不合格品去向五档化加列**——G6 列序 = 检验支数 / 理论合格支 / 返整支数 / **入在制库支** / 入次品库支 / 退货支数 / 可入备库支（新增 `DefectInProcessWarehouseQuantity`，B33 分页汇总同步 9 列）；`QualifiedQuantity` 口径改「检验支数 − 五档次品汇总」。本页组名「检验的数量信息」不属「不合格处理」更名范围，保持原名。后端 `FinalInspectionPlanDto`/Service 口径同步见《计划排程上下文接口设计》V5.47、《质量管理模块详细设计》V8.13。
> 历史变更（V5.49 2026-09-10）：**批次计划 / 成检计划「生产编号」可点开「批次执行进度」弹窗**（批三十二；**纯 WASM，后端/DB 零改动**，数据源复用既有 `GET api/batch/{id}/tracking`）——① 两页 `RenderCell` 的 `BatchNo` 分支改渲染 `<span class="cell-link">`（新增全局类，蓝字 + 悬停下划线），点击调 `OpenBatchProgressAsync(BatchId / ProductionBatchId, BatchNo)` → `DialogService.ShowAsync<BatchProgressDialog>`（`MaxWidth.ExtraLarge` + `FullWidth`），弹窗内为批次详情页区块三同款卡片（`Shared/BatchProgressCard.razor`），**不跳转批次详情页**（弹窗内亦不提供跳转入口）；② **权限不对称 → 按角色降级**：两页页级策略为 `SchedulingView`、追踪端点策略为 `BatchView`，故在 `OnInitializedAsync` 内判定 `_canViewBatchProgress`，⚠️ `Policies.BatchView` 是**逗号分隔多角色串**（`ClaimsPrincipal.IsInRole` 只认单角色名）必须 `Split(',').Any(user.IsInRole)` 逐角色展开；无批次查看权者「生产编号」保持纯文本、无可点样式。验证 `dotnet build MES.Blazor --no-incremental` 0 错 0 警 + `MES.Tests` Components 163 例全绿。
> 状态：**已实施完成**。计划排程上下文提供多层宏观到微观调度视图：工单总览（**12 + 机台组数 行**产能负荷聚合，4 组种子 = 16 行；+ 7 时间桶）、原锁计划（LEFT JOIN 实时查询 + RawMaterialLockPreExecution）、工单排程（WorkOrderExecutionSummary + WorkOrderPlan 薄表 COALESCE 覆盖）、冷轧排程（ColdRollSpecSchedule 独立实体全量同步 + 排程建议引擎 + 机台组/产能/机台数三配置表驱动）、生产计划（全量加载 Items 模式 + BatchPlanSchedule 薄表三规则计划安排 + 工段 Tab 配置驱动）、成检计划（四档看板 + 待检批支重/近日成检量汇总）。工单需求调整与冷轧/流转分析相关页面已迁至工单上下文或整套删除，流转分析体系收敛为 3 类配置驱动的段落流转分析。
...
5. **生产计划** — 在产明细计划实时视图，ProductionBatch LEFT JOIN WorkOrderExecutionSummary + WorkOrderPlan + BatchPlanSchedule 薄表，G4 工单计划采用 COALESCE（Plan优先），G10 工单需求调整直接从实体读取（无需 JOIN OrderDemandAdjustment），展示批次级的生产执行状态与重点关注；含工段 Tab 汇总 + 计划安排（三规则生成薄表）+ G13 薄表内联编辑；ProductionFlowProperty 兜底：无 Plan 覆盖且无 Summary 数据时，非工单批次→"略"，其他→"疑问"
...
- **冷轧排程（ColdRollSpecSchedule）是独立实体（非读模型），存储排程员在冷轧排程页面上设置的排程决策（完成类型+待轧类型+待轧单位或设备+单机单日量），按 (ProcessType, BilletSpec, RollingSpec, IsFinished) 唯一约束，全量同步模式**（原「待轧序 RollOrder」字段已由迁移 `20260610061116_DropRollOrderFromColdRollSpecSchedule` 删除）
- **生产计划和冷轧排程的 G4 采用 WorkOrderPlan COALESCE 模式**（与工单排程统一），IsKeyBatch 计算使用 COALESCE 后的最终值，判定逻辑为 `MainNoAttentionProcess 非空 && UrgencyLevel in [APlusUrgent, AUrgent] && ProductionFlowProperty=="正常"` + 序号比较（生产收尾直接重点、未产批次执行序视为 0）
- **ScheduleStage 默认值**：无 WorkOrderExecutionSummary 记录的批次，其 ScheduleStage 默认为 `-1`（"存错-无此工单"，红色 MudChip 警示），生产计划和成检计划两处同口径
...
- **生产计划（BatchPlan）：在产明细计划实时视图，ProductionBatch LEFT JOIN WorkOrderExecutionSummary + WorkOrderPlan + BatchPlanSchedule 薄表，G4 工单计划采用 COALESCE（Plan优先），G10 工单需求调整直接从实体读取（无需 JOIN OrderDemandAdjustment），含 Tab 汇总 + 计划安排（三规则生成）+ G13 薄表内联编辑**
...
工单需求调整 UI/API 已迁移至 **工单上下文**（工单需求调整页面 `/workorders-demand-adjustment`）。`OrderDemandAdjustment` 实体位于 `MES.Data.Entities.WorkOrder`，原锁计划和工单排程的 G13 字段通过 RefreshAllAsync 同步写入 WorkOrderExecutionSummary，无需 LEFT JOIN 引用。
...
flowchart LR
    ...
    subgraph 原料锁行 R8-R11
        A2[WorkOrderExecutionSummary<br/>ScheduleStage=2] -->|完善用料计划待投料量| R8[原料锁定·完善用料-原料类]
        A2 -->|执行用料计划待投料量| R9[原料锁定·执行用料-原料类]
        A2 -->|成品计划量-已到货量<br/>仅 ExecutePlan 工单| R10[原料锁定·执行用料-成购类]
        R8 --> R11[原料锁定·汇总]
        R9 --> R11
        R10 --> R11
    end
...
> 注意：在制修检、附加成检两道工序无设备分类映射，不参与生产行分配。**行序与序号（2026-09-08 取消 2026-08-23「订单交期负荷置顶」）按自然构建序输出**：原料（1-1~1-3 + 汇总）→ 投料-在产（2-* + 汇总）→ 投料-成检（3-1 + 汇总）→ 整体完工预计（**4-0**）→ 订单交期负荷 3 行（订单延期-原料/在产/成检，**5-1/5-2/5-3**，日期桶仅显副值）。**生产行数随机台组配置动态**（1 荒管抛光 + N 机台组，完全遍历 `ColdRollMachineGroupConfig`）。
...
- 重点批次逻辑为**流转量分析看板专用算法**（与生产计划 IsKeyBatch 分道）：`ScheduleStage==3（生产执行）或 2（原料锁定）` 且 `UrgencyLevel in [APlusUrgent, AUrgent]`，且工序为荒管处理或 == MainNoAttentionProcess（非冷轧类工序，或工段为冷轧拔）；ScheduleStage==2 额外要求 IsUrging/IsBatchDelivery
- 结果通过 `KeyBatchCount`（int）和 `KeyBatchWeight`（decimal?）字段返回

**依赖关系：**
- 段落配置来源 `SectionParagraphConfig`（「系统参数」菜单下维护，原「参数表」2026-09-16 更名；仅参数可编辑）
...
**ProductionFlowProperty 在生产计划中的兜底逻辑：**
生产计划的 G4 字段 `ProductionFlowProperty` 在无 WorkOrderPlan 覆盖且无 WorkOrderExecutionSummary 数据时兜底为：
...
- **G11（关联冷轧排程）/G12（执行反馈）**：实时计算；**G13（生产计划）**：BatchPlanSchedule 薄表 LEFT JOIN
...
- **仅 G4 工单计划概念（生产计划列显示用）**；冷轧排程逻辑改用三档分类器（Model B），不再使用 IsKeyBatch
- Tab 汇总（计划重点批次数/重量）按 PlanFlowLevel==1（急+）统计

**Tab 汇总（6 个指标，前端内存计算 `UpdateTabSummary()`）：**
- 批次数、总重量(t，÷1000)、**计划流转批次数 / 计划流转批次重量(t)**（`PlanIsFlow == true`）、**计划重点批次数 / 计划重点批次重量(t)**（`PlanFlowLevel == 1` 即急+）
- 取值基于 `_allItems`（已按当前工段 Tab 服务端过滤后的全量加载结果），**非** `PagedResult.Extras` 后端聚合（该写法已过时）
...
flowchart LR
    ...
    subgraph 排程编辑
        C -->|工具栏"编辑排程"| D[进入排程模式]
        D -->|RowTemplate<br/>MudSelect/MudNumericField| E[用户编辑<br/>CompletionType/RollType<br/>MachineNo/DailyOutput]
        E -->|保存| F[合并视图外<br/>现有排程记录]
...
**排程编辑操作：**
1. 点击工具栏"编辑排程[手动]"按钮进入排程模式（同组出现"保存排程"/"取消"）
2. 排程设置列（在轧要求/待轧要求/待轧单位或设备/单机单日量）变为 MudSelect/MudSelect/MudTextField/MudNumericField 内联编辑
3. 编辑完成后点击"保存排程"提交到后端
4. 简化视图模式下保存会自动展开到明细级别（`_rawAllItems` 匹配）
5. 保存时把「未在当前视图内的现有排程记录」原值合并回写，避免 Tab 筛选导致数据丢失
...
- 端点 `GET /api/cold-roll-machine-group-config/all`、`/list`、`POST /save`、`POST /delete/{id}`；配置页 `/cold-roll-machine-group-configs`（「系统参数」菜单，原「参数表」2026-09-16 更名）；**保存/删除后失效三缓存键**：`MachineGroupCacheKey` + `MachineEstimateCacheKey` + `ScheduleSuggestionCacheKey`
...
flowchart LR
    subgraph 自动计算
        A[生产计划<br/>"计划安排"按钮] --> B[BatchPlanScheduleService<br/>PlanAllAsync]
    ...
    subgraph 手工调整
        E[生产计划<br/>内联编辑] -->|仅抢单/备注| D
        F[生产计划组字段<br/>PlanIsFlow/PlanFlowLevel/...] -->|前端展示| G[BatchPlanDto]
        D --> F
    end
```

**数据来源：**
- 生产计划数据存储在 `BatchPlanSchedule` 表，通过 `BatchPlanScheduleService.GetAllAsync()` 加载全部记录
- 在 `GetAllAsync()`（生产计划）中通过 LEFT JOIN 映射到 BatchPlanDto 的 `Plan*` 属性
- 8 个字段由 `PlanAllAsync()` 按三规则自动计算，支持手工覆盖（仅抢单/备注内联编辑）

**计划安排流程：**
1. 排程员在生产计划点击"计划安排"按钮
...
| SC-004 | 工单排程为 LEFT JOIN 实时查询（WorkOrderExecutionSummary + WorkOrderPlan），过滤 `ProductionFlowProperty != "略"`；WorkOrderPlan 为薄表（仅含 WorkOrderId + 4 个手工覆盖字段），通过 COALESCE 模式 `DisplayXxx = PlanXxx ?? SystemXxx` 展示；支持保存单行 Plan 覆盖值和"全量计划"批量重置；无物化表。生产计划和冷轧计划的 G4（工单计划）字段在 Service 投影时同样采用 COALESCE 模式（WorkOrderPlan.字段 ?? WorkOrderExecutionSummary.字段），与工单排程统一 |
...
| 规则ID | 规则说明 |
|--------|----------|
| PO-001 | 工单总览为纯查询聚合，无独立实体，不涉及任何数据写入。行序按自然构建序输出；**生产行数随机台组配置动态**（总行数 = 12 + 机台组数，4 组种子 = 16 行；含 110 组 = 17 行） |
...
| PO-003 | **展示顺序（2026-09-08 起取消 2026-08-23「订单交期负荷置顶」，改自然构建序 + 分级序号）**：原料锁定 4 行（完善用料-原料类 1-1/执行用料-原料类 1-2/执行用料-成购类 1-3/汇总留空）→ 投料-在产（[累]荒管抛光固定首行 2-1 + 冷轧机台组动态遍历 2-*，行序=机台组 DisplayOrder，完全遍历含 110）+ 汇总 → 投料-成检（3-1 + 汇总）→ 整体完工预计（**4-0**）→ 订单交期负荷 3 行（订单延期-原料/在产/成检，**5-1/5-2/5-3**）。**行名显示**：荒管抛光行走 DictValueDefinitions 配置表（DictKey=ProductionOverviewRowKey，优先 → 规范中文兜底）；机台组行走组 DisplayName（组显示名联动）；[累] 为固定口径前缀 |
...
| PO-006 | 原料锁定-完善用料-原料类 = 原锁计划待投料量汇总 完善用料计划（RawMaterialLockRemark=ImprovePlan）待投料量（落在「未编制计划」列）；执行用料-原料类 = ExecutePlan 待投料量（**落在「待落实量」列**，2026-09-10 搬列）；执行用料-成购类 = **仅 ExecutePlan 工单**的成品计划量 − 已到货量（FinishPlanWeight − FinishInWeight 缺口，外购由供应商生产、本厂不投料；2026-09-10 收窄口径，排除质量补料/生产返整补足/完善用料计划） |
| PO-007 | 原料锁定-汇总 = 未编制计划取完善用料-原料类；待落实量 = 执行用料-原料类 + 执行用料-成购类；日期桶逐桶求和（IsSummary，总量守恒） |
| PO-007b | **待投料口径统一排除「单一成品采购」工单**（成品采购计划量 > 0 且其余 6 类计划量全 ≤ 0 的纯成购单）：行 1-1/1-2 与 `RawMaterialLockPlanAndExecutionService.IsSingleFinishPurchase` 完全对齐（2026-09-10 用户决策「两表对齐」）；行 1-3 成购**不排除**（纯成购单正是成购本体），故 1-2 与 1-3 天然互斥不重复。`row1Remaining`（整体完工预计的原料待投料量）同步走该排除口径 |
...
| G13 | 生产计划（列组名，原「批次计划」） | PlanIsFlow(流转), PlanFlowLevel(等级,五档 1急+/2急/3急-/4一般/5略), PlanFlowCRType(目标工序), PlanFlowTarget(流转位,六档 成检/完工冷轧/冷轧/荒管检/在制检/成品检验), PlanOuterDiameterSpan(外径跨度), PlanFlowExecSpec(执行规格), PlanExecutionSequence(执行序), PlanTargetSequence(目标序), IsGrabOrder(抢单), PlanRemark(计划备注) — 对应 BatchPlanSchedule 表 |
| G12 | 执行反馈（生产计划） | OriginalDiff(原差=PlanTargetSeq-PlanExecutionSeq, 未产执行序视为0), CurrentDiff(现差=PlanTargetSeq-ExecutionSequence, 未产视为0), CurrentFlowDiff(G7实时工量差), IsExecuted(执行=原差≠现差), IsCompliant(达标/未达标, 流转=否→null) |
...
工单需求调整页面已迁移至 **工单上下文**：路由 `/workorders-demand-adjustment`，路径 `MES.Blazor/Pages/WorkOrders/OrderDemandAdjustment.razor`，导航位置「工单管理 → 需求调整」，权限 `Roles.Policies.WorkOrderView`。

### 4.2 原锁计划页面

- 路由：`/raw-material-lock-plan`
- 路径：`MES.Blazor/Pages/Scheduling/RawMaterialLockPlanAndExecution.razor`
- 导航位置：**当前无菜单入口**（`AppMenu.cs` 计划排程组仅 5 项：订单负荷总量/工单排程/冷轧排程/生产计划/成检计划，本页未挂载，全仓亦无跳转引用 → 属可直达 URL 的未挂菜单页）
...
- **汇总数据卡片**（MudPaper 卡片风格，默认折叠）：
  - **顶部行**：工单总数(X批) | 待投原料(Xt)，存在成购缺口工单时追加「+成购：Xt」
  - **待投原料矩阵**：行=原料锁定备注，列=主号计划性，单元格=单数+待投料重量，含合计行/列
  - **成购矩阵**：仅当存在成购缺口工单时显示；**只渲染「执行用料计划」一行**（2026-09-10 用户决策：成购只是该档的一个分支，其余 3 档恒 0 不渲染，避免误读为「成购横跨 4 档」；底部「合计」行因与唯一数据行同值一并删除），单元格=成购单数/重量（计数为 0 显 `-`）。三处同源实现：本页 + 报表 Tab3（`ReportOverview.razor`）+ 工单上下文用料计划页（`MaterialPlanOverview.razor`，带行/列/格点击下钻联动）
  - **理论待投料截日**：原锁类别（完善用料-原料类/执行用料-原料类/执行用料-成购类/合计）× 日期桶（按理论截止投料日归桶，空→远日量；桶边界与订单负荷总量页同源），含「全期合计」列
- **汇总计算公式：**
  - 成购（外购成品）= `Sum(Max(0, FinishPlanWeight - FinishInWeight))`，**仅限备注=执行用料计划（ExecutePlan）的工单**（2026-09-10 决策：成购属执行用料计划子类，排除质量补料/生产返整补足/完善用料计划；真库此三类缺口恒为 0，收紧不改数值）
  - 待投料 = 分档口径（均扣除外购成品，与订单总览 R1 同口径）：
    - **质量补料（QualityReplenish）**：`Max(0, (TotalWeight - 成购) × 1.1 × (1 - FlowOutputRatio/100))`（投料已满足但产出不足，补料按流转比缺口折算）
    - **执行用料计划（ExecutePlan）/完善用料计划（ImprovePlan）等**：`Max(0, (TotalWeight - 成购) × 1.1 - InputWeight)`（扣除外购部分，加10%耗损，减已投料）
  - 理论待投料截日：完善用料-原料类/执行用料-原料类分别归入 ImprovePlan/ExecutePlan 的 `PendingCalc(item)`，执行用料-成购类归 ExecutePlan 的 `PurchaseCalc(item)`，逐桶求和
...
- 导航位置：计划排程 → 订单负荷总量（第一项）
- 权限：`Roles.Policies.SchedulingView`（SchedulingViewer/SchedulingEditor/SchedulingFull/ReportViewer/ReportEditor/ReportFull/Admin）

**页面布局：**
- 顶部工具栏：仅标题
- 数据表格：MudTable 客户端模式（只读无分页），Dense/Hover/Bordered
- 无搜索、无排序、无分页、无筛选（纯展示）
- 组件化：本表 = `WorkOrderLoadOverview`（`MES.Blazor/Pages/Scheduling/WorkOrderLoadOverview.razor`），同时被报表系统「现订单负荷总量」Tab（ReportOverview Tab2）以 `Title="现订单负荷总量"` 复用；**行序/序号（含 4-0/5-1~5-3）两页共享后端 `ProductionOverviewService` 同步生效**

**表格列结构：**

| 列 | 说明 | 数据来源 |
|----|------|---------|
| 序号 | 层级序号（大类-行，如 1-1/2-3）；汇总行留空；2026-09-08 起 整体完工预计=**4-0**、订单交期负荷 3 行=**5-1/5-2/5-3** | Seq + CategoryNo/RowNo |
| 类别 | 原料锁定/投料-在产/投料-成检/整体完工预计/订单交期负荷（行按自然构建序输出） | Category |
| 工段 | 各工段名称（原料锁定行=完善用料-原料类/执行用料-原料类/执行用料-成购类） | Section |
| 未编制计划(吨) | 仅原料锁定-完善用料-原料类行（及汇总行） | PendingPlanTons |
| 待落实量(吨) | 原料锁定-执行用料-原料类 + 执行用料-成购类（汇总行两者之和） | InProcurementTons |
...
**日期桶说明**（边界配置表驱动：`DateBucket` / `Bucket1~Bucket5`，默认 7/15/30/45/60）。
- 第一桶（交期截止-今日）：`MinValue ~ 今天`，交期在今天及以前的工单/批次
- 后续桶：今日+7 / 今日+15 / 今日+30 / 今日+45 / 今日+60（各桶区间互斥不累加）
- 远日桶（远日量）：今日+60 以后
- 原料锁定行（完善用料-原料类/执行用料-原料类/执行用料-成购类）：按工单交期分布
...
- 前端 Service：`MES.Blazor/Services/WorkOrder/WorkOrderScheduleService.cs`
- 导航位置：计划排程 → 工单排程
- 权限：SchedulingViewer/SchedulingEditor/SchedulingFull/ReportViewer/ReportEditor/ReportFull/Admin
...
- 列显隐默认布局（2026-09-08）：基础数据组「最终客户 EndCustomer」默认隐藏（ColumnPrefs 键并入 `ColumnPrefsVersion="v1"` → `col_prefs_workorderschedules_v1`，升版使已持久化旧布局失效一次；可经列选择器切回）
...
| 分组 | 组名 | 包含字段 |
|------|------|----------|
| G1 | 基础数据 | WorkOrderNo, Salesman, CustomerName, **EndCustomer**, SignDate, DeliveryDate, DelayPenalty, SettlementMethod, SalesOrderNo, ProductionMainNo, ProductionSubNo, MaterialName, DeliveryState, PlantGrade, Specification, LengthStatus, MinLength, MaxLength, TotalItemCount, TotalQuantity, TotalMeters, TotalWeight（共 22 列，列序同 `GetAllColumnDefs()`） |
| G7 | 有效流转 | FlowOutputRatio, FlowStatus, MainNoFlowOutputRatio, MainNoFlowStatus, FlowTotalBatchCount, FlowIncompleteBatchCount, FlowMaxRemainingWorkDays |
| G12 | 实时关注 | ScheduleStage, TotalRemainingWorkDays, CapacityWorkDays, UrgencyLevel, EstimatedProcessCompletionDate, DaysDiffFromDelivery, RawMaterialLockRemark |
...
| G14 | 在产节点待量 | MainNoAttentionProcess, PendingSectionRoughTube, PendingSectionWarehouseFix, PendingSection60Roll, PendingSection50Roll, PendingSection30Roll, PendingSection20Roll, PendingSectionThreeRoll, PendingSectionDrawBench, DeformedProcessCompleted, ProductionAttentionProcess, ProductionFlowProperty, MaxBatchRemainingWorkDays（共 13 列） |
...
- 前端 Service：`MES.Blazor/Services/Scheduling/BatchPlanService.cs`
- 导航位置：计划排程 → 生产计划
- 权限：SchedulingViewer/SchedulingEditor/SchedulingFull/ReportViewer/ReportEditor/ReportFull/Admin

**页面布局：**
- 顶部：标题"生产计划" + 右上角按钮组（`gap:8px`）＝**三个折叠卡片按钮「近日生产量数据」「实时委外在产」「段落流转分析」+「计划安排」**（`BatchPlans.razor:32-59`，折叠按钮 Outlined、展开时 Primary 色 + ExpandLess）+ 工段筛选 MudButton 选项卡 + Tab 汇总数据（批次数/总重量(t)/计划流转批次/计划流转重量(t)/计划重点批次数/计划重点批次重量(t)）
- 搜索框：模糊搜索（全 string 字段 + 中文显示文本列：计划状态/等级/布尔 是·否，见下"搜索与筛选"）
- 数据表格：MudTable **全量加载 Items 模式**（客户端过滤/排序/分页）+ 列分组标题栏 + 列显隐切换 + ExcelFilter 列筛选
- **「生产编号」可点开「批次执行进度」弹窗**（2026-09-10 V5.49）：`RenderCell` 的 `BatchNo` 分支在 `_canViewBatchProgress`（`BatchView` 角色，`OnInitializedAsync` 逐角色展开判定）为真且 `BatchId>0` 时渲染 `<span class="cell-link">`，点击 `OpenBatchProgressAsync(item.BatchId, item.BatchNo)` → `DialogService.ShowAsync<BatchProgressDialog>`，弹窗内容为批次详情页区块三同款卡片；**不跳转批次详情页**；无权限者该列仍为纯文本
- 列分组标题栏（col-group-header-bar）顺序：冷轧排程(本层/下层/下下层/本层匹配/下层匹配)→工单需求调整→批次基础信息→生产计划→状态跟踪→执行反馈；**工单计划/关联冷轧排程组默认隐藏（列显隐可切换打开）**，**冷轧排程/工单需求调整组永久隐藏（标题栏不显示）**，生产计划(执行序/目标序/暂停/抢单)/状态跟踪(现执行序)/执行反馈(原差) 单列默认隐藏
...
**两个折叠查询卡片（原菜单项移入生产计划页内嵌：「实时委外在产」+「段落流转分析」）：**
- **实时委外在产**（折叠、懒加载 `GET api/batch-plan/outsource-pending`，返回 `BatchPlanOutsourcePendingDto`）：
  - 二维表：行 = 在产单位（含末行「合计」），列 = 在产单位 / 各工段(t)（动态列，取 `Sections`）/ 合计(t)（暖色底 #fff8e1，合计行冷色底 #eef3fa）
  - 口径：**状态为在产/未产 且 当前委外单位非空**的批次「有效投料重量」，按（在产单位, 当前工段）统计
  - 单元格格式「总量/[流转]/[*特急]」= 总量、其中生产计划实时流转（IsFlow=是）重量、其中生产计划等级急+（特急批）重量（`*` 标红）；0 值留空，单位 t
  - 可打印（前端 DOM 打印，表格 id `batch-plan-outsource-pending-table`）
- **段落流转分析**（折叠、懒加载 `GET api/section-paragraph-flow-analysis/analysis`）：纯 MudTable 表格（无"可持续天数"列），列：生产段落/待在产重量/总况判定/计划重点批流转量/计划流转判定/特急批重量，可打印；段落由 3 类配置驱动自动生成（冷轧拔按机台组/普通工段按启用工段/检验固定荒管检·在制检），按 3 类规则聚合（Cold=工序组命中机台组内任一工序、Section=工段命中该段落工段且不属于任何冷轧拔机台组、Fixed=荒管检→RoughTube/在制检→InProgress）

**数据分组**（全量加载 Items 模式，共 **77 列**，其中 26 列永久隐藏）：

| 分组 | 组名 | 包含字段 |
|------|------|----------|
| G1 | 批次基础信息（批次信息+关联工单合并） | BatchNo(生产编号), PlantGrade(原料钢号), CurrentValidWeight(重量), ManufacturingStatus(制造状态, **批次真实制造状态字段**), SalesOrderNo(订单号), ProductionMainNo(主号), Specification(成品规格)；DeliveryState(交货状态)/LengthStatus(长度状态)/TagNo(挂牌号)/WorkOrderNo(关联工单号)/DeliveryDate(交货日期)/ProductionType(生产类型)/ManufacturingItem(制造物品)/Salesman(业务员)/EndCustomer(最终用户) 默认隐藏（列显隐可切换打开；2026-09-08：制造状态换源转默认显示、交货状态/长度状态转默认隐藏、重量(kg)→重量、列偏好键 v2→v3）；MinLength(最小长度)/MaxLength(最大长度) 保留定义供搜索/筛选，**永久隐藏**（列显隐选择器不显示） |
| G3 | 状态跟踪 | CurrentExecDate(执行截止日), PendingProcess(执行工序/计算字段), PendingSectionName(待在产工段/计算字段), PendingSpec(执行规格/计算字段), PendingUnit(在产单位/计算字段), PendingEquipment(在产设备/计算字段), ExecutionSequence(现执行序) |
...
| G5 | 冷轧排程（维度明细+匹配结果，**全部永久隐藏**） | 本层 CurrentCR_ProcessType/BilletSpec/RollingSpec/IsFinished/**DeformedSeqCompleted(变形序完成)**（5 列）、下层 NextCR_*（4 列）、下下层 NextNextCR_*（4 列）、实时 RealTimeCR_*（4 列），匹配结果 CR_CompletionType(在轧要求)/CR_RollType(待轧要求)/CR_SchedMachineNo(待轧设备号)（**永久隐藏**；冷轧信息展示统一走 G11 判定结果 + G13 生产计划） |
...
| G12 | 执行反馈 | OriginalDiff(原差), CurrentDiff(现差), IsExecuted(执行), IsCompliant(达标) |
| G13 | 生产计划（持久化，仅 IsGrabOrder/PlanRemark 内联编辑，其余只读） | PlanIsFlow(流转), PlanFlowLevel(等级，显示 PlanFlowLevelDisplay **五档** 1急+/2急/3急-/4一般/5略，特急A/B 手工档已删除), PlanFlowCRType(目标工序), PlanFlowTarget(流转位，**六档**：成检/完工冷轧/冷轧/荒管检/在制检/成品检验), PlanOuterDiameterSpan(外径跨度), PlanFlowExecSpec(执行规格), PlanExecutionSequence(执行序), PlanTargetSequence(目标序), PlanIsPaused(暂停), IsGrabOrder(抢单), PlanRemark(计划备注)；PlanIsPaused(暂停)/IsGrabOrder(抢单) 2026-09-08 起默认隐藏（列显隐可切回） |
...
- 前端 Service：`MES.Blazor/Services/Scheduling/ColdRollPlanService.cs`
- 导航位置：计划排程 → 冷轧排程
- 权限：SchedulingViewer/SchedulingEditor/SchedulingFull/ReportViewer/ReportEditor/ReportFull/Admin

**页面布局（`ColdRollPlans.razor`）：**
- 顶部工具栏：标题 + 右上角按钮组（`gap:8px`，`crp-print-hide`）＝**「排程建议[自动]」**（展开时 Secondary 色 + ExpandLess）→ **「编辑排程[手动]」**（Filled/Primary，进入后紧随其后追加「保存排程」+「取消」）→ **「排程汇总」**（展开后变关闭 ✕ 图标按钮）→ **「排机估算」** → **「机台数配置」**（展开时 Secondary 色）→ 有排程更新时间时追加「最后更新：yyyy-MM-dd HH:mm」
- 工段筛选 MudButton 选项卡（**配置驱动**）：`GET api/process-definition/cold-roll-options`（`Where((IsColdRoll||IsColdDraw) && IsEnabled)` 按 DisplayOrder）→ 首项固定「全部」+ 各启用冷轧/冷拔工序（Key=ProcessKey、显示走 `ProcessDisplayHelper.GetProcessNameText`）；**接口失败降级为仅「全部」**；恢复持久化 Tab 时若 Key 已悬空则置回「全部」
- 明细/简化视图 MudSwitch（Label"简化视图"）+ 搜索栏（搜索规格/类型…，Immediate + 500ms 防抖）
- Tab 汇总数据：**仅「规格数」+「总重量(kg)」**（`_tabSpecCount` / `_tabTotalWeight`，无重点批次两项）
- 数据表格：MudTable 全量加载后客户端分页 + 列分组标题栏（规格信息/近日在轧/近日待轧/待轧分布/合计/排程设置）+ ExcelFilter 列筛选 + MudTablePager(10/25/50/100) + ColumnDisplaySelect 列显隐 + 重置

**四个折叠区（均在标题行内按钮切换显隐，前三者打印隐藏）：**
- **排机估算**（懒加载 `GET /api/cold-roll-plan/machine-estimate`）见下节
- **冷轧机台数配置**（`ColdRollMachineConfig`，随页内联维护）：列 = 机型 / 本厂数量 / 最小机台数 / 最大机台数 / 估算单机日产(kg/天) / 备注 / 操作；机型下拉为配置驱动的启用冷轧/冷拔工序；行内编辑（✓ 保存 / ✕ 取消）+ 删除按钮按 `SchedulingEdit` / `SchedulingDelete` 分档显示，新建按钮按 `SchedulingEdit` 显隐；保存后自动刷新排机估算与排程建议
- **冷轧排程建议**（懒加载 `GET /api/cold-roll-plan/suggestion`）：按机台类型分卡片，显示「可流转量 / 本次计划流转量 / 计划机台数 / 流转档」，有下游链时追加「后流转（目标组量、预算机台）」，`Conflicts` 以 MudAlert(Warning) 逐条列出（矛盾 A/A'/B）；右上角「一键采用建议」转既有 `POST /save-all` 全量同步通道；建议明细表已按用户要求隐藏，仅保留组级基础数据行
- **排程汇总**（`GetScheduleSummaryAsync`）：按（冷轧类型）分组展示近 **2 天 / 4 天 / 6 天** 切换（默认近 6 天），可打印；排机估算固定近 6 天、不随本切换变化
...
| G2 | 近日在轧 | MachineNo(在轧单位或设备), WeightProd(在轧), WeightProdUrgent(在轧(急+)), WeightProdUrgentSub(在轧(急)), WeightProdUrgentOther(在轧(急-)) |
| G3 | 近日待轧 | WeightWaitNear(近日待轧), WeightWaitNearUrgent(近日待轧(急+)), WeightWaitNearBackUrgent(近日待轧(急)), WeightWaitNearOtherUrgent(近日待轧(急-)) |
| G4 | 待轧分布 | WeightToday, WeightTomorrow, WeightDayAfter, WeightExt3, WeightExt4, WeightExt5, WeightDistant |
| G5 | 合计 | WeightTotal(工艺总量) |
| G6 | 排程设置 | CompletionType(在轧要求), RollType(待轧要求), SchedMachineNo(待轧单位或设备), DailyOutput(单机单日量(kg/天)) |

**排程编辑模式：**
- 4 个排程设置列（在轧要求/待轧要求/待轧单位或设备/单机单日量）始终显示
- 工具栏"编辑排程[手动]"按钮进入编辑模式，排程设置列变为内联编辑控件（同组「保存排程」「取消」按钮仅在编辑模式下显示）
- 在轧要求（CompletionType）：MudSelect 选项（`DisplayHelper.GetCompletionTypeOptions()`：CrOnly=急+ / Urgent=急+/急 / Partial2=急+/急/急- / Partial3=急+/急/急-/顺 / All=全量 / None=-）
- 待轧要求（RollType）：MudSelect 选项（显示名同上，`GetRollTypeOptions`）
- 待轧单位或设备（SchedMachineNo）：MudTextField（绑定 `_edit.MachineNo`）
- 单机单日量（DailyOutput）：MudNumericField T="decimal?" + HideSpinButtons + Format="G29"，紧邻待轧单位或设备，排机估算机台需求数依据（空/≤0 不计入）
- 保存到 `ColdRollSpecSchedule` 表，`ScheduleSvc.GetAllAsync()` 加载已有数据
- 简化视图保存时展开到明细级别（`_rawAllItems` 匹配明细行）
- 保存时合并「未在当前视图内的现有排程记录」原值回写，避免 Tab 筛选导致数据丢失
...
**分页汇总行（B33 规范）：**
- FooterContent 中显示当前页所有重量列（**16 列**：）小计
...
- 前端 Service：`MES.Blazor/Services/Scheduling/FinalInspectionPlanService.cs`
...
- 数据表格：MudTable 全量加载后客户端分页 + 列分组标题栏 G1-G6 + ExcelFilter 列筛选 + MudTablePager(10/20/50/100)
- **「生产编号」可点开「批次执行进度」弹窗**（2026-09-10 V5.49）：`RenderCell` 的 `BatchNo` 分支在 `_canViewBatchProgress`（`BatchView` 角色，`OnInitializedAsync` 逐角色展开判定）为真且 `ProductionBatchId>0` 时渲染 `<span class="cell-link">`，点击 `OpenBatchProgressAsync(item.ProductionBatchId, item.BatchNo)` → `DialogService.ShowAsync<BatchProgressDialog>`，弹窗内容为批次详情页区块三同款卡片；**不跳转批次详情页**；无权限者该列仍为纯文本
- B33 分页汇总：CurrentValidWeight, InspectionCount, TotalQuantity, QualifiedQuantity, DefectReworkQuantity, DefectInProcessWarehouseQuantity, DefectScrapQuantity, DefectReturnQuantity, DefectWarehouseQuantity
...
| G6 | 检验的数量信息 | TotalQuantity, QualifiedQuantity, DefectReworkQuantity, DefectInProcessWarehouseQuantity, DefectScrapQuantity, DefectReturnQuantity, DefectWarehouseQuantity |

**列默认显隐（2026-09-08 调整）：**
- G1 批次信息默认显示：生产编号 BatchNo / 生产类型 ProductionType / 制造状态 ManufacturingStatus / 工厂牌号 PlantGrade / 规格 Specification / 长度状态 LengthStatus / 支数 ProductionCutQuantity / 重量 ProductionWeight / 订单号 SalesOrderNo / 主号 ProductionMainNo / 业务员 Salesman
- G1 批次信息默认隐藏（列选择器可随时打开）：成检类型 InspectionType / 是否交付态 IsDeliveryStatus / 制造物品 ManufacturingItem / 交货状态 DeliveryState / 炉号 SourceHeatNo / 来料单位 SourceName / 工单号 WorkOrderNo / 最终用户 EndCustomer
- G3 成检状态默认显示：成检阶段 KanbanStage / 到料日期 ReceiveDate；**最晚检验 MaxInspectionDate 默认隐藏**
- **G5 各项检验的日期 + G6 检验的数量信息 两组整组默认隐藏**（列选择器可随时打开）；G2 排程信息、G4 技术要求检验项默认全显
- 列标签（2026-09-08 简化，仅显示名、Key/排序/公式不变）：G1 生产支数→**支数**、生产重量(kg)→**重量**；G4 必检项数→**必检项**、PMI检验→**PMI**、水下气压→**气压**、端口着色→**着色**
- 默认显隐定义在 `GetAllColumnDefs()`（各 ColumnDef 的 `Visible` 默认值）；「重置」按钮恢复代码默认显隐（非全显）
- ⚠️ 页面状态一体存于 PageState（含列显隐清单），调整默认显隐后须递增 `ColumnLayoutVersion`（并入存储键 `page_state_final-inspection-plan-v3`）使旧持久化失效一次

**紧急程度 MudChip 颜色渲染（按 `UrgencyLevel` 值分支）：**
- `APlusUrgent`（A+急）→ Color.Error（红色）
- `AUrgent`（A急）→ Color.Warning（黄色）
- `BOrder`（B顺）→ Color.Info（蓝色）
- `CSlow` / `DSlow` / 其它 → Color.Default
