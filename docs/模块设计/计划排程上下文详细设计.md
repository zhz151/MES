...
5. **批次计划** — 在产明细计划实时视图，ProductionBatch LEFT JOIN WorkOrderExecutionSummary + WorkOrderPlan，G4 采用 COALESCE（Plan优先），G6 直接从实体读取（无需 JOIN OrderDemandAdjustment），展示批次级的生产执行状态与重点关注；内含产量目标（BatchPlanTarget）设定；ProductionFlowProperty 兜底：无 Plan 覆盖且无 Summary 数据时，非工单批次→"略"，其他→"疑问"
...
- **批次计划和冷轧计划的 G4 采用 WorkOrderPlan COALESCE 模式**（与工单排程统一），IsKeyBatch 计算使用 COALESCE 后的最终值，判定逻辑为 `UrgencyLevel in [A+急,A急] && ProductionFlowProperty=="正常"` + 序号比较（V5.11 统一，不再引用 ScheduleStage/pending 条件）
...
- **批次计划薄表（BatchPlanSchedule）是独立实体（非读模型），存储批次级的计划安排决策（IsFlow/FlowLevel/FlowTarget/FlowCRType/PlanOuterDiameterSpan/FlowExecSpec/ExecutionSequence/TargetSequence + 抢单/备注），通过"计划安排"按钮自动计算 8 个字段的快照，支持排程员手工调整排程计划**
- **批次计划产量目标（BatchPlanTarget）是独立实体（非读模型），存储每个工段在本次计划周期的日产目标(吨)，在批次计划页面 Tab 栏下方以内联编辑框展示，全量覆盖保存，默认值 0 表示未设目标**
- **冷轧排程流转汇总报表**：在冷轧计划页面按"排程汇总"按钮展示，基于批次计划组（PlanIsFlow + PlanFlowCRType + OriginalDiff）聚合的流转重量报表，完全独立于实时计算的批次流转组。流转等级分三档：特急A档（FlowLevel=1 + FlowCRType==MainNoAttentionProcess）、特急B档（FlowLevel=1 + FlowCRType!=MainNoAttentionProcess）、非特急（FlowLevel≥2）
...
- **工单排程（WorkOrderSchedule）：LEFT JOIN 实时查询（WorkOrderExecutionSummary + WorkOrderPlan 薄表），过滤 `ProductionFlowProperty != "略"`，支持 G15 内联编辑 + ConsistencyStatus（四值：一致/进度调整/值存疑/错误）+ 全量计划 + 进度保留计划（V4.5 重构，V5.12 增强）**
...
- 重点批次逻辑与批次计划页面一致（`UrgencyLevel in [A+急,A急] && ProductionFlowProperty=="正常"`；冷轧类 `ExecutionSequence < AttentionProcessSectionSequence+1`，其他 `< AttentionProcessSectionSequence`）
...
- `ConsistencyStatus = 四值计算字段`：Service 端 `ApplyConsistencyStatus()` 方法对每行逐字段比较：
  - `"一致"`：全部 4 对字段完全一致
  - `"进度调整"`：仅 PlanProductionAttentionProcess 不同（其他 3 对一致）
  - `"值存疑"`：其他 3 对（Stage/Urgency/Flow）任一不一致
  - `"错误"`：跨主号 Plan 值冲突（同一 SalesOrderNo+ProductionMainNo 组的工单 Plan 值不一致）
...
**"进度保留计划"按钮（V5.12 新增）：**
- 将当前查询条件发送到后端，对匹配的工单部分重置 WorkOrderPlan 记录
- 仅重置 PlanScheduleStage、PlanUrgencyLevel、PlanProductionFlowProperty 三个字段为系统值
- **保留 PlanProductionAttentionProcess 不变**（用户手工设置的生产关注工序不被覆盖）
- 用于排程员只想更新进度相关字段但不影响已设置的生产关注工序的场景
...
**重点批次（IsKeyBatch）逻辑（V5.11 统一，V5.13 新增 1A/1B 分档）：**
- UrgencyLevel in ["A+急","A急"] && ProductionFlowProperty=="正常" && MainNoAttentionProcess非空
- **冷轧类（含三辊冷轧/冷拔）：** ExecutionSequence < AttentionProcessSectionSequence + 1
- **其他（荒管处理/在制修检/附加成检）：** ExecutionSequence < AttentionProcessSectionSequence
- **冷轧计划同步采用相同逻辑**，工序匹配限定冷轧类（`ProcessNames.IsColdRollOrDraw`），序号直接用 `currentGlobalSeq < ColdRollDraw + 1`
- Tab 汇总因无 ProcessGroups 数据，简化为前两个条件（略过序号比较）
- **等级分档：** 等级1拆分为 **1A**（FlowCRType == MainNoAttentionProcess）和 **1B**（FlowCRType != MainNoAttentionProcess），G7 和 G13 同步更新显示
...
flowchart LR
    subgraph 自动计算
        ...
        B --> C[计算 8 个字段快照<br/>IsFlow/FlowLevel/FlowTarget<br/>FlowCRType/外径跨度/FlowExecSpec<br/>ExecutionSequence/TargetSequence]
...
- 8 个字段由 `PlanAllAsync()` 自动计算（基于 `_trigger` 逻辑），支持手工覆盖

**计划安排流程：**
1. 排程员在批次计划点击"计划安排"按钮
2. 后端遍历所有活跃批次（Status=None/InProgress），按 Flow 判定逻辑计算 8 个字段
...
| G13 | 批次计划（批次计划） | PlanIsFlow(流转), PlanFlowLevel(等级,1拆分为1A/1B), PlanFlowTarget(流转目标), PlanFlowCRType(冷轧类型), PlanOuterDiameterSpan(外径跨度), PlanFlowExecSpec(执行规格), PlanExecutionSequence(执行序), PlanTargetSequence(目标序), IsGrabOrder(抢单), PlanRemark(计划备注) — 对应 BatchPlanSchedule 表 |
| G12 | 执行反馈（批次计划） | OriginalDiff(原工量差, 计算: PlanTargetSeq-PlanExecutionSeq), CurrentDiff(现工量差), CurrentFlowDiff(G7实时工量差), IsExecuted(是否执行), IsCompliant(达标/半达标/未达标) |
...
**页面布局：**
- 顶部工具栏：标题 + "全量计划"按钮 + "进度保留计划"按钮（右上角）+ 总记录数
- 搜索框：模糊搜索（工单号/订单号/业务员/客户/牌号/规格/主号）
...
- 4 个覆盖字段均支持内联编辑：
  ...
  - **PlanUrgencyLevel**（紧急性覆盖）：MudSelect 选项（A+急/A急/B急/C急/B顺/普通），空选项=使用系统值（V5.12 选项更新）
...
- 每行逐字段比较 4 对值（Plan* vs 系统值），输出四值之一：
  - **"一致"**（绿色 MudChip）：全部 4 对字段完全一致
  - **"进度调整"**（蓝色 MudChip）：仅 PlanProductionAttentionProcess 不同，其他 3 对一致
  - **"值存疑"**（橙色 MudChip）：Stage/Urgency/Flow 任一不一致
  - **"错误"**（红色 MudChip）：跨主号 Plan 值冲突（同一 SalesOrderNo+ProductionMainNo 组的工单 Plan 值不一致）
- 用于快速识别手工覆盖过的行及跨主号冲突
...
| G4 | 批次关注（COALESCE） | UrgencyLevel, ScheduleStage, ProductionAttentionProcess, AttentionProcessSectionSequence(计算字段), ProductionFlowProperty（Service 投影时 COALESCE：WorkOrderPlan.字段 ?? WorkOrderExecutionSummary.字段；无数据时非工单→"略"，其他→"疑问"）, IsKeyBatch(计算字段) |
...
| G11 | 批次流转（实时计算） | IsFlow, FlowLevel(显示 FlowLevelDisplay：等级1 按 FlowCRType 匹配 1A/1B), IsFlowLevel1A, IsFlowLevel1B, FlowTarget, FlowCRType, FlowExecSpec, TargetSequence |
| G12 | 执行反馈 | OriginalDiff(原工量差), CurrentDiff(现工量差), IsExecuted(是否执行), IsCompliant(达标) |
| G13 | 批次计划（持久化） | PlanIsFlow(流转), PlanFlowLevel(等级，显示 PlanFlowLevelDisplay：PlanFlowLevel=1 时按 PlanFlowCRType 匹配 1A/1B), PlanFlowTarget(流转目标), PlanFlowCRType(冷轧类型), PlanFlowExecSpec(执行规格), PlanExecutionSequence(执行序), PlanTargetSequence(目标序), IsGrabOrder(抢单), PlanRemark(计划备注) |
...
- `IsKeyBatch`（V5.11 统一算法，与 §2.8 一致）：
  - UrgencyLevel in ["A+急","A急"] && ProductionFlowProperty=="正常" && ProductionAttentionProcess 非空
  - **冷轧类（含三辊冷轧/冷拔）：** ExecutionSequence < AttentionProcessSectionSequence + 1
  - **其他（荒管处理/在制修检/附加成检）：** ExecutionSequence < AttentionProcessSectionSequence
...
- 后端 `PlanAllAsync()` 遍历当前工段 Tab 筛选的活跃批次，按 Flow 判定逻辑自动计算 8 个字段（IsFlow/FlowLevel/FlowTarget/FlowCRType/外径跨度/FlowExecSpec/TargetSequence/ExecutionSequence），其中等级1按 FlowCRType 与 MainNoAttentionProcess 匹配情况自动标记 1A/1B
...
- **IsKeyBatch 逻辑**（与批次计划完全一致，V5.11 统一算法）：
  - UrgencyLevel in ["A+急","A急"] && ProductionFlowProperty=="正常" && ProductionAttentionProcess 非空
  - **冷轧类（含三辊冷轧/冷拔）：** ExecutionSequence < AttentionProcessSectionSequence + 1
  - **其他（荒管处理/在制修检/附加成检）：** ExecutionSequence < AttentionProcessSectionSequence
...
| G2 | 近日在轧 | MachineNo, WeightProd, WeightProdUrgent(特急管), WeightProdUrgentOther(急管) |
| G3 | 近日待轧 | WeightWaitNear, WeightWaitNearUrgent(特急管), WeightWaitNearBackUrgent(后特急), WeightWaitNearOtherUrgent(其它急管) |
...
- 在轧要求（CompletionType）：MudSelect 选项（无计划/全量/特急单/急单/含B顺）
- 待轧要求（RollType）：MudSelect 选项（无计划/全量/特急单/急单/含B顺）
...
- CompletionType/RollType 内部值：`Urgent`=特急单, `Partial2`=急单, `Partial3`=含B顺, `All`=全量, `None`=无计划
...
**冷轧排程流转汇总报表（V4.7 新增，V5.12 增强，V5.13 三档分级）：**
...
- **筛选机制**：
  ...
  - 原工量差筛选：用户可选近2天/近4天/全部，对应 `OriginalDiff <= 2/4` 或无限制
- **聚合维度**：`(PlanFlowCRType, 外径跨度)` — PlanFlowCRType 来自批次计划组，外径跨度来自 ProcessGroups 实时推导
- **展示方式**：按冷轧类型（20冷轧/30冷轧/50冷轧等）分组，每组为一个 MudCard 列表
- **分级统计**：在轧和待轧各分三档 — **特急A档**（FlowLevel==1 + FlowCRType==MainNoAttentionProcess）、**特急B档**（FlowLevel==1 + FlowCRType!=MainNoAttentionProcess）、**非特急**（FlowLevel>=2），合计汇总重量
