...
- **批次计划和冷轧计划的 G4 采用 WorkOrderPlan COALESCE 模式**（与工单排程统一），IsKeyBatch 计算使用 COALESCE 后的最终值，判定逻辑为 `UrgencyLevel in [APlusUrgent, AUrgent] && ProductionFlowProperty=="正常"` + 序号比较（V5.11 统一，不再引用 ScheduleStage/pending 条件）
...
- 重点批次逻辑与批次计划页面一致（`UrgencyLevel in [APlusUrgent, AUrgent] && ProductionFlowProperty=="正常"`；冷轧类 `ExecutionSequence < AttentionProcessSectionSequence+1`，其他 `< AttentionProcessSectionSequence`）
...
- UrgencyLevel in [APlusUrgent, AUrgent] && ProductionFlowProperty=="正常" && MainNoAttentionProcess非空
...
- `IsKeyBatch`（V5.11 统一算法，与 §2.8 一致）：
  - UrgencyLevel in [APlusUrgent, AUrgent] && ProductionFlowProperty=="正常" && ProductionAttentionProcess 非空
  - **冷轧类（含三辊冷轧/冷拔）：** ExecutionSequence < AttentionProcessSectionSequence + 1
  - **其他（荒管处理/在制修检/附加成检）：** ExecutionSequence < AttentionProcessSectionSequence
