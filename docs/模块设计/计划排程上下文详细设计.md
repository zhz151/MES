...
flowchart LR
    ...
    subgraph 原料行 R8-R11
        A2[WorkOrderExecutionSummary<br/>ScheduleStage=2] -->|完善用料计划待投料量| R8[原料·完善用料计划]
        A2 -->|执行用料计划待投料量| R9[原料·执行用料计划]
...
| PO-006 | 原料-完善用料计划 = 原锁计划待投料量汇总 完善用料计划（RawMaterialLockRemark=ImprovePlan）待计划量；执行用料计划 = ExecutePlan 待投料量；外购成品 = 成品计划量 − 已到货量（FinishPlanWeight − FinishInWeight 缺口，外购由供应商生产、本厂不投料） |
| PO-007 | 原料-汇总 = 完善用料计划 + 执行用料计划 + 外购成品，日期桶逐桶求和（IsSummary） |
...
- **汇总数据卡片**（MudPaper 卡片风格，默认折叠）：
  ...
  - **理论待投料截日**：原锁类别（完善用料计划/执行用料计划/外购成品/合计）× 日期桶（按理论截止投料日归桶，空→远日量；桶边界与订单负荷总量页同源），含「待投料总量」列
- **汇总计算公式：**
  ...
  - 理论待投料截日：完善用料计划/执行用料计划归 `PendingCalc(item)`，外购成品归 `PurchaseCalc(item)`，逐桶求和
...
| 类别 | 原料/投料-在产/投料-成检/整体完工预计/订单交期负荷（行按自然构建序输出） | Category |
| 工段 | 各工段名称 | Section |
| 待计划量(吨) | 原料完善用料计划行的待计划量 | PendingPlanTons |
...
- 远日桶（远日量）：今日+60 以后
- 原料行（完善用料计划/执行用料计划/外购成品）：按工单交期分布
