...
5. **批次计划** — 在产明细计划实时视图，ProductionBatch LEFT JOIN WorkOrderExecutionSummary + WorkOrderPlan + BatchPlanSchedule 薄表，G4 工单计划采用 COALESCE（Plan优先），G10 工单需求调整直接从实体读取（无需 JOIN OrderDemandAdjustment），展示批次级的生产执行状态与重点关注；含工段 Tab 汇总 + 计划安排（三规则生成薄表）+ G13 薄表内联编辑（产量目标 BatchPlanTarget 已弃用）；ProductionFlowProperty 兜底：无 Plan 覆盖且无 Summary 数据时，非工单批次→"略"，其他→"疑问"
...
- **批次计划和冷轧计划的 G4 采用 WorkOrderPlan COALESCE 模式**（与工单排程统一），IsKeyBatch 计算使用 COALESCE 后的最终值，判定逻辑为 `MainNoAttentionProcess 非空 && UrgencyLevel in [APlusUrgent, AUrgent] && ProductionFlowProperty=="正常"` + 序号比较（V5.11 统一，2026-08-13 修订：生产收尾直接重点、未产批次执行序视为 0）
...
- ~~**批次计划产量目标（BatchPlanTarget）是独立实体（非读模型），存储每个工段在本次计划周期的日产目标(吨)，在批次计划页面 Tab 栏下方以内联编辑框展示，全量覆盖保存，默认值 0 表示未设目标~~（V5.28 已弃用：页面移除产量目标输入行 + 成品检验 Tab，后端 Controller/Service/实体保留未删，无消费）**~~
...
- **批次计划产量目标（BatchPlanTarget）：独立实体，每个工段一条日产目标(吨)，在批次计划页面 Tab 栏下方以内联编辑框展示，全量覆盖保存（V5.0 新增）；⚠️ 已弃用（V5.28）：页面已移除产量目标输入行 + 成品检验 Tab，后端 Controller/Service/实体保留未删（无消费）**
...
flowchart LR
    subgraph 原料行 R0-R2
        A1[WorkOrderExecutionSummary<br/>ScheduleStage >= 2] -->|PendingOutsourceFinishWeight| R0[成品在购]
        A1 -->|ScheduleStage=2<br/>(Total - Outsource)×1.1 - Input| R1[待投料]
...
> 注意：在制修检、附加成检两道工序无设备分类映射，不参与生产行分配。
...
flowchart LR
    subgraph 数据源
        ...
        C[CombinationGroups<br/>组合归类表] -->|(工序组,工段,产类)<br/>三维归属映射| ITEM[Category 归属行]
...
2. 从 `CombinationGroups`（组合归类表）加载每个类别下的归属行（`(工序组, 工段, 产类)` 三维唯一归属映射），工序组/工段支持"全部"通配，产类为 ProductStatuses 三态或 AllStatus 哨兵（不限定）；`SectionFlowCategoryItem` 已删除
...
| B | 内修磨 | (全部, 内修磨) 通配符 |
| C | 外点磨 | (全部, 外点磨) 通配符 |
| D | 荒管检 | (荒管处理, 检验, 荒管) 三维通配行（由组合归类表驱动） |
| E | 在制检 | (全部, 检验, 在制) 三维通配行（由组合归类表驱动） |
| F | 固溶 | (全部, 固溶) 通配符 |
| G | 矫直 | (全部, 矫直) 通配符 |
| H | 切割 | (全部, 切割) 通配符 |
| I | 去油 | (全部, 去油) 通配符 |
...
| L | 小轧 | (全部, 小轧) 通配符 |
| M | 冷拔 | (全部, 冷拔) 通配符 |
| N | 成品待检 | (全部, 检验, 成品) 三维通配行（由组合归类表驱动） |
...
**依赖关系：**
- 页面**数据源**来自 SectionFlowCategorySetting + CombinationGroups（组合归类表）预设表（在"参数表"菜单下管理）
...
- **仅 G4 工单计划概念（批次计划列显示用）**；冷轧排程逻辑不再使用（V5.25 Model B 改用三档分类器，见 §2.8 冷轧计划）
...
| PO-002 | 原料行（R0-R2）数据源为 `WorkOrderExecutionSummary`，限制 `ScheduleStage >= 2`（排除主号暂停/主号完成） |
...
| PO-005 | R1 待投料[含在购荒管] = ScheduleStage=2（原料锁定）的 SUM(TotalWeight × 1.1 - PendingOutsourceFinishWeight × 1.1 - InputWeight)，成品重量按 1.1 倍换算到原料重量 |
...
- 列默认显隐：G2（用料计划）、G5（物料执行）、G3（投料数据）、G7（有效流转）默认隐藏
...
- 导航位置：计划排程 → 生产工段待产量现况（**已从菜单栏移除**，仅经批次计划页内嵌折叠查询访问）
- 权限：WorkOrderRead
...
- 分页 25 条/页，FooterContent 汇总行（InProduction/PendingProduction/Total/PlanFlowQuantity/PlanKeyWeight）
...
| 工段 | SectionName | 字符串 |
| 产类 | ProductStatus | 字符串，显示产类中文（荒管/在制/成品） |
| 生产中 | InProduction | 整数显示，decimal? |
| 待产量 | PendingProduction | 整数显示，decimal? |
| 汇总量 | Total | 整数显示，decimal? |
| 计划流转量 | PlanFlowQuantity | 整数显示，decimal?，单位 kg |
| 重点批重量 | PlanKeyWeight | 整数显示，decimal?，单位 kg |
...
- 导航位置：计划排程 → 生产段流转量分析（**已从菜单栏移除**，仅经批次计划页内嵌折叠查询访问）
- 权限：WorkOrderRead

**页面布局：**
- 顶部工具栏：标题 + 记录数
- 搜索框：模糊搜索（流转类别名称、计划流转判定、总况判定）
- 数据表格：MudTable 客户端模式（全量加载，`Items` 绑定），Dense/Hover，Class="auto-table"
- 列显隐切换（ColumnDisplaySelect）+ 全字段排序
- 分页 25 条/页，FooterContent 汇总行（PendingTotal/PlanFlowQuantity/PlanKeyWeight）

**表格列（7列）：**

| 列 | 数据字段 | 说明 |
|---|---------|------|
| 流转类别 | Category | 显示 CategoryName |
| 待在产重量 | PendingTotal | 整数显示，decimal? |
| 可持续天数 | SustainableDays | 整数显示，decimal?，默认隐藏 |
| 总况判定 | StatusJudgment | 字符串，对应 MudChip 颜色：偏少=Error，过多=Warning，正常=Success |
| 计划流转量 | PlanFlowQuantity | 整数显示，decimal?，批次计划中流转=是的重量汇总（吨） |
| 计划流转判定 | PlanFlowJudgment | 计划流转量 > 日产量 → "加速"（Warning），否则 "-" |
| 重点批重量 | PlanKeyWeight | 整数显示，decimal?，流转=是且等级=急+的重量汇总（吨） |
...
### 4.5.1 生产段落流转量分析页面（2026-08-15 新增）

- 路由：`/section-paragraph-flow-analysis`
- 路径：`MES.Blazor/Pages/Scheduling/SectionParagraphFlowAnalysis.razor` + `.razor.cs`
- 前端 Service：`MES.Blazor/Services/SectionParagraphFlowAnalysisService.cs`
- 导航位置：计划排程 → 生产段落流转量分析（**已从菜单栏移除**，仅经批次计划页内嵌折叠查询访问）
- 权限：WorkOrderRead

**背景：**
- 生产段落（ParagraphName）由 `SectionParagraphConfig` 段落日产配置定义（ParagraphName 唯一），日流转设定/偏少天数/过多天数可编辑
- 数据源为 `SectionParagraphFlowAnalysisService`：组合归类表 `CombinationGroups` 的「归属段落」列将 (工序组,工段,产类) 三维行上卷到段落，聚合待产/计划流转等指标

**页面布局：**
- 顶部工具栏：标题 + 记录数
- 搜索框：模糊搜索（生产段落、计划流转判定、总况判定）
- 数据表格：MudTable 客户端模式（全量加载，`Items` 绑定），Dense/Hover，Class="auto-table"
- 列显隐切换（ColumnDisplaySelect）+ 全字段排序
- 分页 25 条/页，FooterContent 汇总行（PendingTotal/PlanFlowQuantity/PlanKeyWeight）
...
| 列 | 数据字段 | 说明 |
|---|---------|------|
| 生产段落 | Paragraph | 显示 ParagraphName |
| 待在产重量 | PendingTotal | 整数显示，decimal? |
| 可持续天数 | SustainableDays | 整数显示，decimal?，默认隐藏 |
| 总况判定 | StatusJudgment | 字符串，对应 MudChip 颜色：偏少=Error，过多=Warning，正常=Success |
| 计划流转量 | PlanFlowQuantity | 整数显示，decimal?，批次计划中流转=是的重量汇总（吨） |
| 计划流转判定 | PlanFlowJudgment | 计划流转量 > 日流转设定 → "加速"（Warning），否则 "-" |
| 重点批重量 | PlanKeyWeight | 整数显示，decimal?，流转=是且等级=急+的重量汇总（吨） |
...
**页面布局：**
- 顶部：标题"批次计划" + "计划安排"按钮 + **"显示类汇总"（原"汇总"按钮改名）+ "段落流转分析" + "工段流转分析"三个折叠查询按钮** + 工段筛选 MudButton 选项卡 + Tab 汇总数据（批次数/总重量/计划流转批次/计划流转重量/计划重点批次数/计划重点批次重量）
...
- **段落流转分析**：折叠查询按钮 → 纯 MudTable 表格（无"可持续天数"列），列：生产段落/待在产重量/总况判定/计划流转量/计划流转判定/重点批重量，可打印
- **工段流转分析**：折叠查询按钮 → 纯 MudTable 表格（无"可持续天数"列），列：流转类别/待在产重量/总况判定/计划流转量/计划流转判定/重点批重量，可打印

**数据分组（V5.28 列重组，全量加载 Items 模式，共 64 列，其中 22 列永久隐藏）：**
...
| G3 | 状态跟踪 | CurrentExecDate(执行截止日), PendingProcess(执行工序/计算字段), PendingSectionName(待在产执行工段/计算字段), PendingSpec(执行规格/计算字段), PendingEquipment(在产设备/计算字段), ExecutionSequence(现执行序) |
...
| G5 | 冷轧排程（维度明细+匹配结果） | 本层 CurrentCR_ProcessType/BilletSpec/RollingSpec/IsFinished（4 列）、下层 NextCR_*（4 列）、下下层 NextNextCR_*（4 列），匹配结果 CR_CompletionType(在轧要求)/CR_RollType(待轧要求)/CR_SchedMachineNo(待轧设备号)（**永久隐藏**） |
| G11 | 关联冷轧排程（实时计算） | IsFlow(流转，与冷轧计划「排程选中」口径一致，二值 是/否；Model B 档位（显示名 V5.26：急+=特急、急=特急-、急-=急）：All=全量、CrOnly=急+(正常流转∧关注==当前冷轧)、Urgent=急+/急(正常流转)、Partial2=急+·急·急-、Partial3=急+·急·急-·顺，不掺入关注状态/IsKeyBatch), FlowLevel(等级列，**显示 ScheduleTierDisplay 六档（V5.27 细化）**：急+/急/急-/顺/带/略，与 _trigger 同源：急+=正常流转∧关注==当前冷轧、急=正常流转∧关注≠当前冷轧、急-=非正常流转、顺=非急但流转(B顺)、带=All 档下非急非顺普通批次、略=不在排程内 IsFlow=false；筛选选项 GetScheduleTierOptions、客户端排序按 ScheduleTier；DTO 隐藏字段 ScheduleTier/ScheduleTierDisplay/AttentionMatchesCurrentCR), FlowTarget(流转目标), FlowCRType(冷轧类型), OuterDiameterSpan(外径跨度), FlowExecSpec(执行规格), TargetSequence(目标序) |
| G12 | 执行反馈 | OriginalDiff(原工量差), CurrentDiff(现工量差), IsExecuted(是否执行), IsCompliant(达标) |
| G13 | 批次计划（持久化，仅 IsGrabOrder/PlanRemark 内联编辑，其余只读） | PlanIsFlow(流转), PlanFlowLevel(等级，显示 PlanFlowLevelDisplay **五档** 1急+/2急/3急-/4一般/5略，特急A/B 手工档已删除), PlanFlowCRType(目标工序), PlanFlowTarget(流转位，**六档**：成检/完工冷轧/冷轧/荒管检/在制检/成品检验), PlanOuterDiameterSpan(外径跨度), PlanFlowExecSpec(执行规格), PlanExecutionSequence(执行序), PlanTargetSequence(目标序), IsGrabOrder(抢单), PlanRemark(计划备注) |

**永久隐藏列（22 列）**：冷轧排程三层明细 12 列 + 匹配结果 3 列 + 工单需求调整 4 列 + 业务员/最小长度/最大长度 3 列（`_permanentlyHiddenColumnKeys`，列显隐选择器不显示，定义保留供搜索/筛选）
...
| **V5.28** | **2026-08-14** | **批次计划页列重组 + 流转目标六档 + 全量 Items 模式**：① 批次信息+关联工单合并为「批次基础信息」（9 字段），状态跟踪组改名（执行工序/待在产执行工段/在产设备/现执行序）；永久隐藏 22 列（冷轧排程 5 组 15 列 + 工单需求调整 4 列 + 业务员/最小/最大长度 3 列）；② G13 批次计划组只读（仅抢单/计划备注内联编辑），统计栏改名（计划流转批次/计划重点批次，重点按 PlanFlowLevel==1 急+）；③ **流转目标六档**：FlowTargetKeys 3→6 值（新增荒管检/在制检/成品检验），薄表规则2 planFlowTarget=MapFlowTargetByCRType(MainNoAttentionProcess)（荒管处理→荒管检/在制修检→在制检/生产收尾→成品检验/空→null/剩余冷轧类→冷轧），规则1 保持排程 FlowTarget（含完工冷轧）；④ 前端全量 Items 模式客户端排序/筛选，模糊搜索补中文文本列（计划状态/等级/布尔是·否），日期筛选值 yyyy-MM-dd；⑤ 断切工段 Tab 筛选精确匹配；⑥ 移除产量目标输入行 + 成品检验 Tab（BatchPlanTarget 端点/实体保留未删）；⑦ IsKeyBatch 修订（2026-08-13）：生产收尾直接重点、未产批次执行序视为 0 |
