...
> 版本：V5.51（2026-09-11）
> 版本变更（V5.50 2026-09-11）：**成检计划 G6「检验的数量信息」组随成品检不合格品去向五档化加列**——G6 列序 = 检验支数 / 理论合格支 / 返整支数 / **入在制库支** / 入次品库支 / 退货支数 / 可入备库支（新增 `DefectInProcessWarehouseQuantity`，B33 分页汇总同步 9 列）；`QualifiedQuantity` 口径改「检验支数 − 五档次品汇总」。本页组名「检验的数量信息」不属「不合格处理」更名范围，保持原名。后端 `FinalInspectionPlanDto`/Service 口径同步见《计划排程上下文接口设计》V5.47、《质量管理模块详细设计》V8.13。
...
**页面布局：**
- 顶部：标题"生产计划" + "计划安排"按钮 + **"近日生产量数据"折叠卡片 + "显示类汇总" + "段落流转分析"两个折叠查询按钮** + 工段筛选 MudButton 选项卡 + Tab 汇总数据（批次数/总重量/计划流转批次/计划流转重量/计划重点批次数/计划重点批次重量）
