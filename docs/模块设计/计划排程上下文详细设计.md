...
- **段落流转分析（SectionParagraphFlowAnalysis）：段落日产配置 3 类驱动自动生成（冷轧拔按机台组/普通工段按启用工段/检验固定），分析待产总量与可持续天数（原生产段流转量分析 SectionFlowAnalysis 旧体系已整套删除）**
...
> 注意：在制修检、附加成检两道工序无设备分类映射，不参与生产行分配。订单交期负荷 6 行 + 整体完工预计置顶展示（2026-08-19 重排）。**生产行数随机台组配置动态**（1 荒管抛光 + N 机台组，2026-08-30 起完全遍历 `ColdRollMachineGroupConfig`）。
...
**工段筛选（Section Tab，配置驱动，2026-08-30 改造）：**
...
- **普通工段类**：按「工段工量天数」StandardWorkDays 启用工段生成 Tab（SectionKey 英文 + SectionName 中文），扣除 ColdRollDraw/Inspection/Warehouse 三固定工段；**内抛+内修磨不再合并为"内抛+内修磨"，各自独立 Tab**（2026-08-30）
...
- **产能平衡（统一语义，2026-08-30）**：minTarget = isDemander ? max(组 MinMachines, demandInflow.Total) : 组 MinMachines；沿阶梯双向调整——超上限向窄收（Urgent→CrOnly）、不足向宽放（Partial3→All）至 count∈[minTarget,max]；**凡配了 MaxMachines 的组（无论有无供需链）都走产能平衡**：无供需组（三辊/冷拔独立池、或取消供给目标组后的 5060）不再无条件固定「急+/急/急-」纯人工档，超上限标注矛盾 A'、不足标注矛盾 A；未配机台数上限的组级 no-op（suggestedTier 保持默认原始档，特急锁定在行级仍生效）
...
**冷轧机台组配置（ColdRollMachineGroupConfig，V5.42，组角色 V5.44 移除）：**
- 表 `ColdRollMachineGroupConfig`，归组由编译期 `ColdRollMachineGroupKeys.Groups`（4 组硬编码）改为**配置表驱动**，新增冷轧工序可经配置归入现有组或新建组，无需改代码即可在排机估算/排程建议中看到新机台类型
...
- **组角色（FlowRole）已废除（V5.44，2026-08-29）**：None/Supplier/Demander 三值概念删除——每组天然可同时为供给方与需求方，角色由配置推导而非显式字段：**供给方 = 配置了 `SupplyTargetGroupKey`**（驱动排机估算拆档取整 + 排程建议供给方两阶段流转平衡）、**需求方 = 被至少一个供给组指向**（`demandInflow` 驱动最小机台基准/矛盾 B）；多级链中间节点 isDemander ∧ isSupplier → flowState `Role=Both`；`ColdRollMachineGroupKeys.Groups` 五元组降四元组、`FlowRole*` 三常量删除（该常量现仅作种子/测试规范定义源）
- **供需链显式化（方案 A，V5.43，2026-08-29 取代「各恰 1」）**：供需链由 `SupplyTargetGroupKey` 显式表达（供给方组 → 下游需求组），**允许多条并行链、多级链**；强不变量从「全表 Supplier/Demander 各恰 1」放宽为**链合法性**（`ValidateChainInvariant`）：① 凡配置了 SupplyTargetGroupKey → 目标组必须存在（**去掉「目标非 None」约束**——2030→冷拔(None) 多级链末端合法，独立池/链末 = 不配目标即合法状态）；② 供给链无环（沿 SupplyTarget 链走，命中已访问组即抛）；删除被指向的需求组（供给目标悬空）也抛错。工序**全局唯一归属一组**（跨组重叠会导致引擎双重计数，服务层 `ToHashSet(OrdinalIgnoreCase)` 强校验）
...
- 端点 `GET /api/cold-roll-machine-group-config/all`、`/list`、`POST /save`、`POST /delete/{id}`；配置页 `/cold-roll-machine-group-configs`（参数表菜单）；**保存/删除后失效三缓存键**：`MachineGroupCacheKey`（新增）+ `MachineEstimateCacheKey` + `ScheduleSuggestionCacheKey`
- **机型选项联动（V5.42，2026-08-29 收紧仅启用工序）**：新增 `GET /api/process-definition/cold-roll-options`（`Where((IsColdRoll||IsColdDraw) && IsEnabled)` 按 DisplayOrder 排序返回 `List<ProcessInfoDto>`，**仅 IsEnabled=true 的启用工序**——禁用工序不参与机型下拉/工段 Tab/机台组归组）；机台数配置两处机型下拉（`ColdRollMachineConfigs`/`ColdRollPlans` 机台数卡片）与冷轧排程工段 Tab 由静态数组改为配置驱动（`ProcessDefinitionService.GetColdRollOptionsAsync`），新增启用冷轧工序自动出现在下拉与工段 Tab；`GetColdRollOrDrawKeysAsync`（引擎排程判定用）**仍不过滤 IsEnabled**（禁用工序存量批次仍需按冷轧处理，两者口径按用途拆分）
- **2026-08-29 Q1/Q2 修订（用户反馈「新增工序机型显示英文」+「禁用工序机台数配置残留」）**：
  ...
  - **禁用工序自动删除机台数配置（Q2）**：`ProcessDefinitionService.SaveAsync` 中当 `dto.IsEnabled == false` 时自动①删除 `ColdRollMachineConfigs` 中 `ProcessType == ProcessKey` 的记录②从所有 `ColdRollMachineGroupConfigs.ProcessKeys` 逗号串中移除该工序（组内仅该工序则置空）③失效 `MachineGroupCacheKey`+`MachineEstimateCacheKey`+`ScheduleSuggestionCacheKey` 三引擎缓存键；与「禁用工序不可归组」服务层校验闭环——禁用即从排程建议产能平衡/机台组归组彻底移除；存量批次仍按冷轧处理（`GetColdRollOrDrawKeysAsync` 口径）
...
| 规则ID | 规则说明 |
|--------|----------|
| PO-001 | 工单总览为纯查询聚合，无独立实体，不涉及任何数据写入。**2026-08-19 重构为 19 行**（原 8 行）；**2026-08-30 起生产行数随机台组配置动态**（总行数 = 19 + 机台组数 − 4） |
...
| PO-003 | **展示顺序（2026-08-23 行序重排）**：订单交期负荷 3 行置顶（订单延期-原料/在产/成检）→ 原料 4 行（完善计划/执行计划/外购成品/汇总）→ 投料-在产 N 行（[累]荒管抛光固定首行 + 冷轧机台组动态遍历，行序=机台组 DisplayOrder，2026-08-30 起完全遍历含 110）+汇总 → 成检+成检汇总 → 整体完工预计。**行名显示**：荒管抛光行走 DictValueDefinitions 配置表（DictKey=ProductionOverviewRowKey，优先 → 规范中文兜底）；机台组行走组 DisplayName（2026-08-30 起组显示名联动）；[累] 为固定口径前缀 |
...
| PO-010 | 荒管抛光：ProcessName = "荒管处理" 且 OuterPolish 有值；冷轧/冷拔机台组行：工序 Key ∈ 组 ProcessKeys（配置表 ColdRollMachineGroupConfig 驱动，2026-08-30 起无硬编码工序/组 Key 匹配） |
...
| PO-014 | 整体完工预计：EstDays = Max(生产工段天数) + CEILING(原料待投料量(吨) / 2030机台组日产) + **2 天成检**（2026-08-19 补齐成品检验环节）；EstDeadline = today + EstDays |
...
> - 新导航位置：工单管理 → 工单需求调整
> - 权限：使用工单上下文角色权限（SchedulingViewer/SchedulingEditor/SchedulingFull/ReportViewer/ReportEditor/ReportFull/Admin / SchedulingEditor/SchedulingFull/Admin）
...
> 2026-08-24 删除独立路由页 `SectionFlowAnalysis.razor`；**2026-08-31 整套删除**：后端 `SectionFlowAnalysisService`/`SectionFlowAnalysisController`/DTO/`SectionFlowAnalysisPrintRequest`、前端 `SectionFlowAnalysisService` 及批次计划页折叠卡片全部移除，不再消费 `GET api/section-flow-analysis`。
...
**页面布局：**
- 顶部：标题"批次计划" + "计划安排"按钮 + **"近日生产量数据"折叠卡片 + "显示类汇总"（原"汇总"按钮改名）+ "段落流转分析"两个折叠查询按钮** + 工段筛选 MudButton 选项卡 + Tab 汇总数据（批次数/总重量/计划流转批次/计划流转重量/计划重点批次数/计划重点批次重量）
...
**工段筛选 Tab（配置驱动，2026-08-30 改造，`GET api/batch-plan/section-tab-options` 动态加载）：**
...
**两个折叠查询（2026-08-15 原菜单项移入批次计划页内嵌，工段流转分析已随旧流转量分析体系删除）：**
- **显示类汇总**（原"汇总"按钮改名）：点击展开跨工段汇总卡片，按 `ProductionSummaryHelper.SummaryAllSectionTabs`（26 工段全量，内抛+内修磨合并）逐工段归桶统计，列：工段/批次数/总重量(t)/流转批次/流转重量(t)/重点批次/重点重量(t)/急+/急/急-/一般/略，含"合计"行（全量唯一批次），可打印
- **段落流转分析**：折叠查询按钮 → 纯 MudTable 表格（无"可持续天数"列），列：生产段落/待在产重量/总况判定/计划重点批流转量/计划流转判定/特急批重量（2026-08-21 表头改名），可打印；段落由 3 类配置驱动自动生成（冷轧拔按机台组/普通工段按启用工段/检验固定荒管检·在制检），按 3 类规则聚合（Cold=工序组命中机台组内任一工序、Section=工段命中该段落工段且不属于任何冷轧拔机台组、Fixed=荒管检→RoughTube/在制检→InProgress）
...
- 行数 = 冷轧机台组配置行数（2026-08-29 起配置驱动，DisplayName 显示组名）× 5 列（轧机类型/流转总量(kg)/在制品(kg)/成品(kg)/机台需求数）
...
| **V5.49** | **2026-08-30** | **批次计划工段筛选 Tab 配置驱动**：§2.8/§4.7 工段筛选 Tab 由编译期 `BatchPlanSectionTabs.All`（17 项）改为配置驱动——冷轧/冷拔类=「工序组定义」ProcessDefinitions 启用冷轧拔工序逐工序生成（新增工序自动出现）、普通工段=「工段工量天数」StandardWorkDays 启用工段（扣除 ColdRollDraw/Inspection/Warehouse，**内抛+内修磨拆独立 Tab**）、末尾固定荒管检/在制检；Tab 选项经 `GET api/batch-plan/section-tab-options` 动态加载（唯一规范源 `BuildSectionTabOptionsAsync`，与委外在产汇总列同源）；`BatchPlanSectionTabs` 删 All/InnerPolishGrind 仅留检验两常量；筛选匹配改 `crKeys.Contains(sectionTab)` 判定冷轧 + 普通工段精确匹配英文 Key（前端传英文 Key，后端 NormalizeSectionTab 原样兜底）；委外在产列 `ResolveSummaryTabName` 改配置显示名 + 内抛/内修磨拆分；汇总卡片（`ProductionSummaryHelper.SummaryAllSectionTabs` 26 工段全量，内抛+内修磨合并）保持解耦不动；持久化选中 Tab 恢复时悬空校验置"全部" |
| **V5.48** | **2026-08-30** | **生产总览运行时去兜底（产能单一数据源）**：`ProductionOverviewService` 3 处代码兜底移除（荒管抛光 `dailyPolish`/机台组行 `groupCapacity`/整体完工预计 `daily2030`）——日产能 100% 来自产能档案 `DailyProductionCapacities`，未配置组产能=0→预计天数空（显式信号去补录，不再静默用错误默认值，消除 ForGroup 代码值 11/9/3 与真库运营值 13/11/1.5 不一致）；`ProductionOverviewDefaults.ForGroup`/`Polish` 改注释为**仅供 DbInitializer 种子初始化新库**（种子保留默认产能，新库开箱即出预计天数），生产服务不再引用；测试 110 无产能断言改「预计天数空」 |
| **V5.47** | **2026-08-30** | **生产总览完全遍历机台组（含 110）**：§2.2/§3.4 工单总览冷轧/冷拔行由「5 行硬编码（Polish/Mill50_60/Mill20_30/ThreeRollMill/DrawBench）」改为「荒管抛光固定首行 + 完全遍历 `ColdRollMachineGroupConfig`（按 DisplayOrder 排序）」——PO-001 行数动态化（总行数=19+机台组数−4）、PO-003 展示顺序（行 Key=机台组 GroupKey、显示名=组 DisplayName）、PO-010 匹配（ClassifySection 用组 ProcessKeys 包含工序 Key，无硬编码工序/组 Key）、PO-012 动态工段数汇总、PO-014 整体完工预计 2030 机台组日产；**110 组纳入总览**（默认日产能 `ProductionOverviewDefaults.ForGroup`：110=3/5060=11/2030=9/ThreeRoll=0.5/Draw=3，新增组无默认落 0→预计天数空）；行序重排由硬编码 16 项数组改动态分类排序（延期置顶、整体完工预计置底、其余按 Seq，`nextSeq=5+机台组数`）；`DailyProductionCapacityService.SaveAsync` 校验改「仅 Polish 或已存在机台组 GroupKey」；真库存量迁移：产能档案键 4 条（Mill50_60→5060/Mill20_30→2030/ThreeRollMill→ThreeRoll/DrawBench→Draw）+ 新增 110=3 + 清理 DictValueDefinitions.ProductionOverviewRowKey 冷轧 4 条死条目；排机估算行数配置驱动（§4.9 冷轧排程）；前端 WorkOrderLoadOverview 纯渲染零改动；测试补 4 组种子保持既有行索引断言 + 新增 110 排最前测试 |
| **V5.46** | **2026-08-30** | **无供需组统一产能平衡（排程建议缺口修复）**：凡配了 MaxMachines 的组（无论有无供需链）都走产能平衡——无供需组（三辊/冷拔独立池、或取消供给目标组后的 5060）不再无条件固定「急+/急/急-」纯人工档（原分支跳过 `ChooseTierBidirectional`，配了上限的组取消供需链后机台数可超 MaxMachines 且无任何矛盾标注）；超上限向窄收 + 矛盾 A'、不足向宽放 + 矛盾 A；未配机台数上限保持默认原始档不变；测试：三辊配上限不足矛盾 A、5060 取消供给目标超上限矛盾 A' |
| **V5.45** | **2026-08-29** | **删除冷轧机台组组角色字段（FlowRole 移除）**：§2.9 冷轧机台组配置——组角色 None/Supplier/Demander 三值概念废除，供需链由 `SupplyTargetGroupKey` 显式表达，每组天然可同时为供给方与需求方（多级链中间节点 flowState Role=Both）；迁移 `20260829153516_RemoveFlowRoleFromColdRollMachineGroupConfig` DROP COLUMN FlowRole；`ColdRollMachineGroupKeys.Groups` 五元组降四元组、`FlowRole*` 三常量删除；引擎 `MachineGroupDef.FlowRole` 死字段移除（isSupplier=!IsNullOrEmpty(SupplyTargetGroupKey)、isDemander=demandInflow.ContainsKey，均不读组角色）；链合法性新语义——凡配供给目标组→目标存在 + 无环，去掉「目标非 None」约束（2030→冷拔(None) 多级链末端合法，独立池/链末=不配目标即合法）；配置页删「组角色」列（MudSelect 三值下拉）；测试 3 文件删 FlowRole + 链校验断言重写 |
| **V5.44** | **2026-08-29** | **供需链配置化（方案 A）**：§2.9 冷轧机台组配置升级——新增字段 `SupplyTargetGroupKey`（供给目标组 Key，可空）显式表达供需链，取代「全表 Supplier/Demander 各恰 1」强约束，允许多条并行链/多级链（中间节点 flowState Role=Both）；强不变量放宽为链合法性（供给方组必有目标/目标存在/无环，删除被指向需求组抛错——**「目标非 None」约束已随 V5.45 移除**）；引擎 `ComputeFlowDemand2030`→`ComputeFlowDemand` 按链对返回 (demandInflow, supplierOutflow) 两字典、isDemander/isSupplier 按 demandInflow/SupplyTargetGroupKey 判定、`CountAtTierFlowTo(supplierGroup, targetGroup)`；排机估算拆档判定改 `!IsNullOrEmpty(SupplyTargetGroupKey)`；迁移 `20260829142057_AddSupplyTargetGroupKey`（存量 5060 回填 2030）；配置页新增「供给目标组」列（MudSelect 选组 Key + 显示名映射）+ ApplySort 补排序；测试补多供给方并行链/多级链 Both 集成测试 |
| **V5.43** | **2026-08-29** | **冷轧机台组归组配置化 + 机型选项联动**：§2.9 新增「冷轧机台组配置」（ColdRollMachineGroupConfig）小节——归组由编译期 `ColdRollMachineGroupKeys.Groups` 改为配置表驱动；组角色 FlowRole 三值（Supplier 拆档取整+供给方 / Demander minMachines2030+需求方 / None 默认），强不变量 Supplier 与 Demander 各恰 1、工序全局唯一归属一组——**FlowRole 已随 V5.45 删除**；引擎改造（`LoadMachineGroupsAsync` 缓存 60s、FlowRole 定位替代硬编码、流转链 supplierKeys/demanderKeys 并集传入）；保存/删除后失效 MachineGroupCacheKey+MachineEstimateCacheKey+ScheduleSuggestionCacheKey 三缓存键；机型选项联动：`cold-roll-options` 端点 + 机台数两处下拉与冷轧排程工段 Tab 由静态数组改配置驱动；配置页 `/cold-roll-machine-group-configs`（参数表菜单） |
