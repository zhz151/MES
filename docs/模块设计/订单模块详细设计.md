...
| Controller | 路由前缀 | 文件 |
|-----------|---------|------|
| GradeMappingController | `api/grade-mapping` | `MES.Api/Controllers/StandardRegister/GradeMappingController.cs`（生产标准上下文） |

| 方法 | 路由 | 说明 |
|------|------|------|
| GET | `api/grade-mapping/list` | 分页查询牌号对照列表 |
| GET | `api/grade-mapping/all` | 获取所有牌号对照（下拉框） |
| GET | `api/grade-mapping/{id}` | 获取牌号对照详情 |
| POST | `api/grade-mapping` | 创建牌号对照 |
| PUT | `api/grade-mapping/{id}` | 更新牌号对照 |
| DELETE | `api/grade-mapping/{id}` | 删除牌号对照 |
