...
| OrderDemandAdjustment | 工单需求调整（催单/分批交货/暂停标记，API/UI 属工单上下文，数据库实体在计划排程上下文） |
...
| 页面 | 并行加载的数据 | 说明 |
|------|---------------|------|
| Orders/OrderCreate.razor | CustomerService.GetPagedAsync + StandardRegisterService.GetAllAsync + GradeMappingService.GetAllAsync | 客户、标准号、牌号对照三者无依赖 |
| Orders/OrderDetail.razor | StandardRegisterService.GetAllAsync + GradeMappingService.GetAllAsync + OrderService.GetByIdAsync | 标准号、牌号对照、订单详情三者无依赖 |
...
| GET | `api/grade-mapping/{id}/print` | 打印牌号对照 |
...
- 四个手工字段：IsUrging（催单）、IsBatchDelivery（分批交货）、IsPaused（工单暂停）、AdjustmentRemark（调整备注）
- 数据库实体 `OrderDemandAdjustment` 保留在计划排程上下文，其 G13 字段通过 RefreshAllAsync 同步写入 WorkOrderExecutionSummary 实体供查询使用
...
- 默认收起，点击"展开"加载
- 通过 `WorkOrderService.GetOperationLogsAsync(Id)` 调用 `IOperationLogService.GetLogsAsync("Order", entityId)`
