...
| OrderDemandAdjustment | 工单需求调整（催单/分批交货/主号暂停标记，API/UI 与数据库实体均属工单上下文，`MES.Data.Entities.WorkOrder`） |
...
订单首页（`/orders`）的「④ 订单执行」组（GroupKey=4，2026-08-07 由「工单执行」改名）三字段从 `WorkOrderExecutionSummary` 聚合写入 `OrderListSummary` 读模型（`OrderService.RefreshByOrderIdAsync`）：
...
| POST | `api/order-demand-adjustment/save` | 保存工单需求调整（催单/分批交货/暂停/备注） |
| GET | `api/order-demand-adjustment/filter-contexts` | 获取筛选上下文 |

**业务说明：**
- 工单需求调整（原销售催单）记录对生产排程的催货要求
- 每个工单最多一条记录（WorkOrderId 唯一）
- 四个手工字段：IsUrging（催单）、IsBatchDelivery（分批交货）、IsPaused（主号暂停，联动连带同主号未入库完结工单）、AdjustmentRemark（调整备注）
- 数据库实体 `OrderDemandAdjustment` 现位于工单上下文（`MES.Data.Entities.WorkOrder`）；其 IsPaused 在工单执行状况刷新（RefreshAllAsync / RefreshByWorkOrderNosAsync）时同步写入 `WorkOrderExecutionSummary.IsPaused`，作为「主号关注」档0（主号暂停）判定依据
