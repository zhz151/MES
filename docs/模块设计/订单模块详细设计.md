...
订单首页（`/orders`）工具栏「订单接单·出库及现负荷汇总」按钮切换的折叠卡片（2026-08-18 新增），样式仿「计划排程」→「原锁计划」→「待投料量汇总」卡片。表格结构 = 5 指标行 × 12 月列（本年 2026年1月~12月）：
...
> **2026-08-26 新增**：自仓库上下文整体迁入（原「待发货项」，更名「订单成品(实时库存)」），路由 `/orders/pending-delivery`，API 路径保持 `api/pending-delivery` 不变。服务位于 `MES.Services/Order/PendingDeliveryQueryService.cs`（命名空间 `MES.Services.Order`，接口 `MES.Core.Interfaces.Order.IPendingDeliveryQueryService`），DTO 位于 `MES.Core/DTOs/Order/`。
...
> 页面更名为"工单需求调整"，权限改为 WorkOrderViewer/WorkOrderEditor/WorkOrderFull/ReportViewer/ReportEditor/ReportFull/Admin/WorkOrderEditor/WorkOrderFull/Admin。
> 详情请参见工单上下文相关设计文档。
...
| POST | `api/workorder-demand-adjustment/save` | 保存工单需求调整（催单/分批交货/暂停/强制完成/备注） |
| GET | `api/workorder-demand-adjustment/filter-contexts` | 获取筛选上下文 |
...
- 前端页面路由：`/workorders-demand-adjustment`，位于工单管理菜单下
- 权限：`WorkOrderViewer/WorkOrderEditor/WorkOrderFull/ReportViewer/ReportEditor/ReportFull/Admin`（读）/ `WorkOrderEditor/WorkOrderFull/Admin`（写）

### 6.6 订单成品(实时库存) API（PendingDeliveryController）

> **2026-08-26 迁入**：原仓库上下文「待发货项」API，随功能整体迁入订单上下文。API 路径保持 `api/pending-delivery` 不变。
...
| 方法 | 路由 | 说明 | 权限 |
|------|------|------|------|
| GET | `api/pending-delivery/list` | 待发货列表（无分页，供质保书创建页引用） | `PendingDeliveryView` |
...
| POST | `api/pending-delivery/print-all-file` | 打印全部（按当前筛选条件） | `OrderView` |

**权限说明**：`all`/`filter-contexts`/`print-file`/`print-all-file` 跟随订单角色（`Roles.Policies.OrderView` = 订单+报表三档+Admin）；`list`/`header-options` 保留 `Roles.Policies.PendingDeliveryView`（**订单 + 质量**数据源并集，供质保书创建页 Quality 角色调用，2026-08-26 自「仓库+质量」调整）。
...
> **跨域组合策略**：`PendingDeliveryView` = **订单 + 质量**数据源并集（2026-08-26 起；原「仓库+质量」）。该策略仅用于订单成品(实时库存)的 `list`/`header-options` 端点，供质保书创建页 Quality 角色调用；其余端点跟随 `OrderView`。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| **V1.19** | **2026-08-26** | **待发货项迁入订单上下文**：原仓库「待发货项」整体迁入，更名「订单成品(实时库存)」，路由 `/orders/pending-delivery`，页面/控制器/服务/DTO 全部迁至 Orders 命名空间（`MES.Blazor/Pages/Orders/PendingDelivery.razor`、`MES.Api/Controllers/Order/PendingDeliveryController.cs`、`MES.Services/Order/PendingDeliveryQueryService.cs`），API 路径保持 `api/pending-delivery`；权限跟随订单角色 `OrderView`，`list`/`header-options` 保留 `PendingDeliveryView`（订单+质量并集）；新增 §5.16（查询服务设计：5 表 JOIN/Hybrid/3 层缓存/质保书集成）与 §6.6（API 端点表）；§八 补跨域策略说明；菜单入口位于客户管理之下 |
| **V1.18** | **2026-08-18** | **新增订单接单·出库及现负荷汇总**：新增 §5.14.2 折叠卡片（5 指标行 × 12 月列，接单量/出库量全列 + 成品库存(完工/未完工)/订单负荷量(实时)仅当前月列，0 显「-」）+ §6.1 API 端点 `GET api/order/in-out-summary?year=`；卡片标题右侧「打印」按钮走前端 printRawHtml |
...
| **V1.14** | **2026-08-01** | **文档审查修复**：§5.11 理算重量 ≤ 0 时跳过对比校验（不阻止提交）；§5.12 创建页并行加载 CustomerService.GetPagedAsync → GetAllAsync()，详情页补充第 4 个数据源 CustomerService.GetAllAsync()；§6.3 牌号对照打印端点改为 POST api/grade-mapping/print-single（**2026-08-24 死端点清理：grade-mapping print-single 已删除，保留 print-batch/print-all 系列**） |
