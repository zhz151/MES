...
> 版本：V1.39（2026-09-16）
> 最新变更（V1.39，2026-09-16）：**历史条目状态补注（纯文档修订）**——V1.36 条末尾原「⚠️ 本批尚未打包上线」已失效，就地补注为「已于 2026-09-15 随 `mes-deploy-20260915_1539.zip` 上线」（原文保留，依据 `docs/本机与线上发布差异待办.md`「当前状态」）。**零正文改动、零代码 / 零迁移 / 零数据变更**；同轮对账见 V1.38（§2.1 ERD 弱关联）与 V1.37（第六章 API 清单）。
> 历史变更（V1.38，2026-09-16）：**文档与代码对账（§2.1 实体关系图）：删除 `SalesOrder → CustomerProfile` 外键连线**（**纯文档修订，零代码 / 零迁移 / 零数据变更**）：`MES.Data/AppDbContext.Order.cs:25` 已注明「**CustomerId FK 已移除**」——订单不再维护与客户档案的外键关系，二者之间**无数据库级约束**，仅以冗余快照字段（`CustomerName` / `Salesman` / `EndCustomer` 等）弱关联；故 ERD 不再绘制 `belongs_to` 连线，并在图下补注说明弱关联语义。
> 历史变更（V1.37，2026-09-16）：**文档与代码对账（第六章 API 清单）：删除 3 个已下线端点、补 4 个漏记端点**（**纯文档修订，零代码 / 零迁移 / 零数据变更**）：① **§6.1 订单 API 删除** `POST api/order/refresh`（读模型手动刷新端点已下线）、`POST api/order/{id}/items` 与 `PUT api/order/{id}/items/{itemId}`（项次增改已并入 `save-all` 单事务，二者已下线）；**补齐** `DELETE api/order/{id}/items/{itemId}`（单条项次物理删除）与 `GET api/order/{id}/operation-logs`（订单操作日志）。② **§6.2 客户 API 删除** `GET api/customer/select-list`（客户下拉列表已下线）；**补齐** `POST api/customer/print-list-file`（按可见列打印选中客户列表）。③ **§6.4 产品要求 API 补齐** `GET ~/api/order/{orderId}/requirements/defaults`（按标准号取新建技术要求默认值）与 `GET ~/api/order/requirements/quality-remark`（按销售订单号 + 项次序号列表取质量备注），并把三条 `~/` 前缀路由与「无记录返回空 DTO」语义写入表格。对账依据：`MES.Api/Controllers/Order/{OrderController,CustomerController,ProductRequirementController}.cs` + 交叉核对《订单上下文接口设计》中「已下线」注记。
> 历史变更（V1.36，2026-09-15）：**订单进度树：叶下新增「生产批次名单」（在产 / 在途分段）+ 生产执行分支口径由快照改实时重算**（**无 EF 迁移 / 无数据变更 / 无权限变更 / 零新端点**）：① **生产执行分支改实时**——原取 `WorkOrderExecutionSummary.PendingSection*` 快照，现改由新建共享计算器 `MES.Services/Helpers/ProductionPendingNodeHelper` 对 `ProductionBatches.Include(ProcessGroups)` 实时计算（按该订单各 `WorkOrderNo` 收窄、按 `ProductionMainNo` 分组，主号级 = 主号全部工单批次并集一次算完）；**节点 Key / 中文 Label / 叶子顺序与数值口径逐字不变**，仍「>0 才建叶」。② **叶下批次名单**——`MainProgressLeafDto` 新增 `BatchSegments`（`List<LeafBatchSegmentDto>`）+ 两个新 DTO（`LeafBatchSegmentDto` / `LeafBatchItemDto`）；生产执行叶 = **在产 / 在途两段**（`batchCurrentSeq == targetSeq` → 在产；`< targetSeq` → 在途；`> targetSeq` / 无此工序组 / 工序组无目标工段 / 成检与完成态 → 不计），**不变式 `Σ在产 + Σ在途 == 叶 WeightKg`**；成品检验叶 = **单段**（每批恰落一档，无在途概念）。段序固定「在产 → 在途」、段内按 `BatchNo` Ordinal 升序。③ **前端 `Shared/OrderProgressTree`**（首页「订单进度查询」卡与 `/orders/progress` 页共用）——名单**默认折叠**（折叠态 `【在产 14 批 · 在途 41 批】`）、展开后批号可点 → 弹「批次执行进度」（`MaxWidth.Large`，不跳详情页）；节点标签**【待产】不变**（用户拍板）。④ **口径分叉声明**——本分支实时值与本页其余快照字段（`DeformedProcessCompleted` / `ProductionAttentionProcess`）及「工单执行状况」页快照**会长期分叉**，属设计取舍非 bug。⑤ 验证：`dotnet build MES.Api` / `MES.Blazor` **各 0 错 0 警**；定向单测 `ProductionPendingNodeHelperTests`（新建 16 例）+ `OrderProgressQueryServiceTests` **21 过** + 工单执行相关合计 **102 过**；**真库 SQL 核验**（`D26Z2159001` / `X03`）荒管处理 **19281.000**（14 批，名单逐字吻合）/ 50冷轧 **37433.000** / 30冷轧 **98973.000**，与 2026-09-15 快照值一致。详见 §5.17.6。**⚠️ 本批已于 2026-09-15 随 `mes-deploy-20260915_1539.zip` 上线**（原文为「尚未打包上线」，2026-09-16 对账补注）。
...
| StandardGradeMapping | 牌号对照 ⚠️ **实归属于产品标准上下文管理** |
...
> ⚠️ **`SalesOrder → CustomerProfile` 外键关系已删除**（2026-09-16 对账更正）：`MES.Data/AppDbContext.Order.cs:25` 注释明示「CustomerId FK 已移除」——订单不再维护与客户档案的外键，仅以冗余快照字段（`CustomerName` / `Salesman` / `EndCustomer` 等）弱关联；二者之间**无数据库级约束**，故 ERD 不再绘制连线。
...
| DELETE | `api/order/{id}/items/{itemId}` | 删除单个项次（物理删除） |
| GET | `api/order/{id}/operation-logs` | 订单操作日志 |
...
| DELETE | `api/customer/{id}` | 删除客户（物理删除） |
| GET | `api/customer/filter-contexts` | 筛选上下文 |
| POST | `api/customer/print-batch-file` | 批量打印客户（PDF文件，直接返回） |
| POST | `api/customer/print-list-file` | 按可见列打印选中客户列表（PDF文件，直接返回） |

### 6.3 牌号对照 API

牌号对照（GradeMapping）API 归属**产品标准上下文**（`MES.Api/Controllers/StandardRegister/GradeMappingController.cs`，路由 `api/grade-mapping`），本模块不再提供端点，详见《产品标准上下文详细设计》。
...
| 方法 | 路由 | 说明 |
|------|------|------|
| GET | `api/order/{orderId}/items/{itemId}/requirement` | 获取指定项次的产品要求（无记录时返回空 DTO，非 null） |
| POST | `api/order/{orderId}/items/{itemId}/requirement` | 创建/更新项次的产品要求 |
| GET | `~/api/order/{orderId}/requirements` | 获取订单下所有项次的产品要求列表（含项次号） |
| GET | `~/api/order/{orderId}/requirements/defaults` | 按标准号取新建技术要求默认值（工厂检验项要求含「必检」→ true） |
| GET | `~/api/order/requirements/quality-remark` | 按销售订单号 + 项次序号列表（逗号分隔）取质量备注（各项次「其他要求」拼接） |
...
| 方法 | 路由 | 说明 | 权限 |
|------|------|------|------|
| GET | `api/order/progress?salesOrderNo=` | 获取指定订单号的四级进度树；无该订单工单/订单号为空 → 返回 `null`（`ApiResponse.Ok(null, ...)`） | 仅登录（`[Authorize]`，2026-09-14 为首页「订单进度查询」卡放开） |
