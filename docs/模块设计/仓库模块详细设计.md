...
> **2026-08-26 迁移说明：** PendingDeliveryController 已迁至 `MES.Api/Controllers/Order/PendingDeliveryController.cs`，API 路径保持 `api/pending-delivery` 不变。权限调整：`all`/`filter-contexts`/`print-file` → `Roles.Policies.OrderView`（订单+报表+Admin）；`header-options` → `PendingDeliveryView`（订单+质量数据源并集，供质保书创建页 Quality 角色调用）。
