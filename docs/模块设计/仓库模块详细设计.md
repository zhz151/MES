...
> 版本：V7.21（2026-09-16）
> 版本变更（V7.21 2026-09-16）：**§4.5 打印功能与 `InventoryController` 对齐** —— 删除 3 条不存在端点的描述 `POST api/inventory/print-stock-all-file` / `print-inbound-all-file` / `print-outbound-all-file`（「打印全部」入口从未实现，实际只有「打印选中」一条路径），同步删除已不存在的 `OnlyWithStock=true` 参数说明；保留 3 条真实端点（`print-stock-selected-file` / `print-inbound-selected-file` / `print-outbound-selected-file`，前端调用点：`WarehouseInventory.razor.cs:1063`、`InboundHistory.razor.cs:1641`、`OutboundHistory.razor.cs:857`）。
> 版本变更（V7.20 2026-09-16）：**文档与代码对账（纯文档修订，无接口 / 数据库 / 迁移变更）** —— **§7.1 库存 API 清单与 `InventoryController` 逐条对齐**：① **删 1 条已下线端点** `GET api/inventory/validate-workorder-nos/{warehouseId}`（校验工单号列表合法性，无对应 Action），改表下「已下线端点」注记（校验改走 `validate-source-order` / `validate-production-batch`）；② **补 3 条漏登记端点** `GET api/inventory/mismatched-batches`（到料实投不一致批次）、`GET api/inventory/source-order-changed-batches`（来源单号变更批次）、`GET api/inventory/resolve-linked-work-order`（反查关联工单）。两处依据：《仓库上下文接口设计》V3.29 已下线注记 + `InventoryController.cs` 实际端点清单。
> 版本变更（V7.19 2026-09-09）：**物料进出存报表（§4.10，inout 前端改造，无后端变更）**——每月格去月末结存改彩带「入x/出y」（入=浅绿底深绿字、出=浅红底深红字，内联样式屏显+打印一致），0 侧省略；末尾单列拆两列 =「进出汇总」（该行全年 入/出，同彩带格式）+「实时库存」（只显示当前库存量 ClosingWeight，单值 t，原「实时数据」更名）；入库/出库/库存报表单值展示不变。验证 `dotnet build MES.Blazor` 0 错 0 警。
> 状态：**已实施完成**。仓库模块以批量操作与筛选为核心：4 库（原料/成品/次品/在制品）合并单实体，前端按仓库 Code 切分展示与字段可见性；批量入库（内联编辑 + 列选择器 + 公共字段预填 + 方向键导航 + 定尺核查）、表头筛选多选批量出库独立页（部分出库 + 委外-穿孔号校验）、批次物理删除（OperationLog 统一日志 + 入库状态回退）、出库工单号两级匹配联动批次用料计划完成、物料进出存透视表（/warehouse/monthly-stock，同源 4 报表切换）。移库/冻结功能已废弃；「待发货项」已整体迁至订单上下文（订单成品(实时库存)），§4.9/§7.3 保留迁移说明、§8 查询设计归 Orders 后仍成立。
...
- 物理删除，记录到统一 OperationLog 表（InventoryBatchDeleteLog 已并入统一 OperationLog）
...
| INV-008 | **长度验证（仅 FG 成品库进库适用，2026-09-08 起收窄；其它仓库进库不强制）**：定尺时最小长度=最大长度且>0，范围尺时最大长度>最小长度且>0 | ✅ 已实现 |
| INV-009 | **次品库（DEFECT）额外验证**：次品原因必填、责任类型必填 | ✅ 已实现 |
| INV-010 | **4库通用验证规则**：支数/重量/物料类型/钢种/名义规格/来源类型/工厂牌号/入库日期必填；物料类型允许范围校验；重量>0；支数>0 | ✅ 已实现 |
| INV-011 | **入库完成推进**：任何仓库的首笔入库均触发 `TryCompleteProductionBatchAsync`（InFinalInspection→Completed 或 InProgress+余材→Completed），不再仅限 FG 仓库 | ✅ 已实现 |
| INV-012 | **入库删除状态回退**：删除最后一条入库记录后，若 ProductionBatch 状态为 Completed 且无其他入库，自动回退状态（RefreshBatchTrackingFieldsAsync 重算） | ✅ 已实现 |
| INV-013 | **定尺切割长度匹配标识（CutLengthMatchType）**：仅成品库（FG）+ 订单类成品（OrderFinished/CriticalFinished/SpecialDeliveryStatus）+ 定尺核查；关联解析生产批号为主、工单号兜底（ResolveAssociationAsync）；标识 = 命中本工单号定尺集 → FullMatch（完全匹配）/ 命中「订单号+主号」定尺集 → MainNoMatch（主号匹配）/ 其余 → null（不适用）；其他库房即使有生产批号关联也不核查（库房维度为准）；备料成品 Finished 无工单号不参与 | ✅ 已实现 |
| INV-014 | **定尺切割长度硬校验**：仅自动填充第2种「检验入库」按生产批号时（前端 EnforceCutLengthMatch=true）启用，需同时满足 FG 成品库 + 检验入库 + 订单类成品 + 定尺 + 生产批号非空 + 工单号非空，入库长度必须命中「订单号+主号」定尺长度集合，否则 SaveChanges 前抛 BusinessException 禁止入库（采购/委外与手动填写不受影响） | ✅ 已实现 |
| INV-015 | **内联编辑核查条件**：入库批次内联编辑时，仅当生产批次 + 工单号都非空才进行定尺切割长度核查；任一缺失则 CutLengthMatchType 标识清空；主号在工单号/生产批号任一非空时按权威关联推导 | ✅ 已实现 |
...
**打印按钮（工具栏）：** 三个页面均**只有「打印选中」一种入口**（无「打印全部」端点，三张表各自勾选后打印）。

| 按钮 | 端点 | 说明 |
|------|------|------|
| 库存台账打印选中 | POST `/api/inventory/print-stock-selected-file` | 携带选中批次 ID + 可见列定义 |
| 入库历史打印选中 | POST `/api/inventory/print-inbound-selected-file` | 同上 |
| 出库历史打印选中 | POST `/api/inventory/print-outbound-selected-file` | 同上 |

**前端处理：** 后端直接返回 PDF 二进制流 → 前端 `openPdfFromApi()` 通过 fetch 获取后以 Blob URL + iframe 覆盖层展示（与工单打印方案一致）
...
**验证规则对齐：** 入库历史页的 SaveRow 内联编辑验证与入库操作页（WarehouseInbound）完全一致，包括：工厂牌号必填、物料类型允许范围校验、入库来源必填、重量/支数>0。长度验证仅 FG 成品库进库强制（2026-09-08 起收窄：原 FG/WIP → 仅 FG；其它仓库进库定尺/范围尺不强填最小/最大长度；定尺/范围尺逻辑与入库页相同）。DEFECT 次品库额外验证：次品原因必填、责任类型必填。
...
> **迁移说明：**「待发货项」已自仓库上下文整体迁移至**订单管理**上下文，更名为「订单成品(实时库存)」，路由 `/warehouse/pending-delivery` → `/orders/pending-delivery`，页面/控制器/服务/DTO 全部迁至 Orders 命名空间，权限跟随订单角色。
> 详细设计请参见《订单模块详细设计》§5.15 / §6.6。本节原内容已移除。

### 4.10 物料进出存报表

**页面路由：** `/warehouse/monthly-stock`（仓库管理，自 /reports/monthly-stock 迁入；旧路由已废弃）

**页面文件：** `MES.Blazor/Pages/Reports/MonthlyStock.razor` / `.razor.cs`

**功能说明：** 物料进出存透视表，行=库房×物料类型，列=期初 + 12 月 + 末尾合计列。**同一数据集支撑 4 报表切换**（入库报表/出库报表/库存报表/物料进出存报表），仅展示列不同；**入库/出库报表额外按 来源/出库类型 展开粒度**。**当前月份之后的月份尚未发生，单元格留空**（前端按 `_currentMonthIndex` 判断）。**物料进出存报表（inout，2026-09-09 用户 UX 拍板 V7.19）**：每月格去掉月末结存、改**彩带入x/出y**；末尾由单列拆为两列 = **进出汇总**（全年 入/出）+ **实时库存**（只显示当前库存量，原「实时数据」更名）——见下方「单位显示」口径。
...
- **4 报表切换**：入库报表仅展示每月「入」+「来源汇总(t)」（列=库房×物料类型×入库来源，来源固定顺序=外购→委外→生产入库→检验入库→其它，全 0 来源行隐藏）；出库报表仅展示「出」+「出库类型汇总(t)」（列=库房×物料类型×出库类型，类型固定顺序=生产领用→销售出库→退货出库→委外出库→其它出库，全 0 类型行隐藏）；库存报表仅展示「结」+期初+实时结存；**物料进出存报表（V7.19 前端改造）展示每月彩带「入x/出y」+期初 + 末尾「进出汇总」（全年 入/出）+「实时库存」两列**
...
- **单位显示**：kg/1000 → t，F1，0 值留空；物料进出存报表每月格=**彩带「入x/出y」**——「入」浅绿底深绿字 `#1e7e34/#e6f4ea`、「出」浅红底深红字 `#c62828/#fdecea`（内联样式，打印 getTableHtml 抓 DOM 同步保色），0 侧省略、两侧皆 0 留空；**月末结存不再逐月显示**（用户 2026-09-09 决策），由末尾「实时库存」列集中给当前库存；「进出汇总」列=该行全年 入/出（TotalIn/TotalOut，同彩带格式），「实时库存」列=只显示当前库存量（ClosingWeight=真实全口径结存，单值 t）；入库/出库/库存报表仍是单值，库存报表末尾「实时结存」=ClosingWeight
...
> 移库/冻结功能已废弃（移库规则 ⏳ 待实现，TransferIn/TransferOut 枚举已删除；冻结 IsFrozen 字段已移除），不纳入权限列。

> 实际使用 `Roles.Menus.WarehouseViewer` / `Roles.Menus.WarehouseEditor` / `Roles.Menus.WarehouseFull` / `Roles.Admin` 角色，储存在 `MES.Shared.Constants.Roles` 中。
...
- 通过主号（ProductionMainNo）关联订单，实际查询键为「订单号+主号」（项次 OrderItemIds 已弃用）
...
| GET | `api/inventory/mismatched-batches` | 到料实投不一致批次 |
| GET | `api/inventory/source-order-changed-batches` | 来源单号变更批次 |
| GET | `api/inventory/resolve-linked-work-order` | 反查关联工单 |
| GET | `api/inventory/outbound-filter-contexts` | 出库记录筛选上下文 |
| GET | `api/inventory/inventory-filter-contexts` | 库存批次筛选上下文 |
| GET | `api/inventory/monthly-stock-summary` | 物料进出存汇总（一次返回 Rows 全口径 + InboundSourceRows 入库来源粒度 + OutboundTypeRows 出库类型粒度；DTO 数据=期初+12月×入/出/结+ClosingWeight，4 报表共用数据源，无合计行；物料进出存报表前端展示列见 §4.10 V7.19） |

> **已下线端点**：`GET api/inventory/validate-workorder-nos/{warehouseId}`（校验工单号列表合法性）**已下线**，无对应 Action —— 校验改走 `POST api/inventory/validate-source-order`（来源单号）与 `POST api/inventory/validate-production-batch`（生产批号）。
...
### 7.3 待发货查询 API（已迁移至订单上下文）

> **迁移说明：** PendingDeliveryController 已迁至 `MES.Api/Controllers/Order/PendingDeliveryController.cs`，API 路径保持 `api/pending-delivery` 不变。权限调整：`all`/`filter-contexts`/`print-file` → `Roles.Policies.OrderView`（订单+报表+Admin）；`header-options` → `PendingDeliveryView`（订单+质量数据源并集，供质保书创建页 Quality 角色调用）。
> 详细设计请参见《订单模块详细设计》§6.6。

---

## 八、待发货项查询（PendingDeliveryQueryService）

> **迁移说明：** 本章所述 PendingDeliveryQueryService 已随「待发货项」整体迁至**订单上下文**（`MES.Services/Order/PendingDeliveryQueryService.cs`，命名空间 `MES.Services.Order`，接口 `MES.Core.Interfaces.Order.IPendingDeliveryQueryService`）。下文的查询逻辑/缓存策略/与质保书集成等设计在当前上下文仍然成立，仅归属调整；菜单入口现为**订单管理 → 订单成品(实时库存)**。详细设计请参见《订单模块详细设计》§5.15 / §6.6。
...
**用途**：
- 订单管理 → 订单成品(实时库存)列表页（服务端分页+筛选+排序+打印，自仓库管理迁入）
...
> **项次仅内部解析**：`OrderItemIds` 仅作内部解析「产品标准/标准等级/交货状态」的关联键（订单号|序列 复合键），查询/显示/Keyword/筛选已改用主号（`ProductionMainNo`），`PendingDeliveryItemDto` 不再暴露 `OrderItemIds` 字段。
...
**失效策略：**
- 出库/入库操作时，`InventoryService` 注入 `IPendingDeliveryQueryService` 调用 `InvalidateCachesAsync()` 清空全部 3 层缓存（含静态 `ConcurrentDictionary` 跟踪的 C2 动态键），下次请求完整重建
