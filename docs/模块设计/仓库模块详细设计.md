...
> **2026-08-26 迁移说明：** 「待发货项」已自仓库上下文整体迁移至**订单管理**上下文，更名为「订单成品(实时库存)」，路由 `/warehouse/pending-delivery` → `/orders/pending-delivery`，页面/控制器/服务/DTO 全部迁至 Orders 命名空间，权限跟随订单角色。
...
> **2026-08-26 迁移说明：** PendingDeliveryController 已迁至 `MES.Api/Controllers/Order/PendingDeliveryController.cs`，API 路径保持 `api/pending-delivery` 不变。权限调整：`all`/`filter-contexts`/`print-file`/`print-all-file` → `Roles.Policies.OrderView`（订单+报表+Admin）；`list`/`header-options` → `PendingDeliveryView`（订单+质量数据源并集，供质保书创建页 Quality 角色调用）。
...
> **2026-08-26 迁移说明：** 本章所述 PendingDeliveryQueryService 已随「待发货项」整体迁至**订单上下文**（`MES.Services/Order/PendingDeliveryQueryService.cs`，命名空间 `MES.Services.Order`，接口 `MES.Core.Interfaces.Order.IPendingDeliveryQueryService`）。下文的查询逻辑/缓存策略/与质保书集成等设计在当前上下文仍然成立，仅归属调整；菜单入口现为**订单管理 → 订单成品(实时库存)**。详细设计请参见《订单模块详细设计》§5.15 / §6.6。
...
**用途**：
- 订单管理 → 订单成品(实时库存)列表页（服务端分页+筛选+排序+打印，2026-08-26 自仓库管理迁入）
...
| 版本 | 日期 | 说明 |
|------|------|------|
| **V7.18** | **2026-08-26** | **待发货项迁出至订单上下文**：「待发货项」整体迁移至订单管理（更名「订单成品(实时库存)」，路由 `/orders/pending-delivery`），页面/控制器/服务/DTO 全部迁至 Orders 命名空间（`MES.Blazor/Pages/Orders/PendingDelivery.razor`、`MES.Api/Controllers/Order/PendingDeliveryController.cs`、`MES.Services/Order/PendingDeliveryQueryService.cs`），权限跟随订单角色（OrderView），API 路径保持 `api/pending-delivery`；§4.9/§7.3 改为迁移说明，§8 标注迁移归属并指向《订单模块详细设计》§5.15/§6.6 |
