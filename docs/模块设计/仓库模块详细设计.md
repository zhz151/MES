...
**功能说明：** 物料进出存透视表，行=库房×物料类型，列=期初 + 12 月×（入/出/结）+ 实时数据/实时结存。**同一数据集支撑 4 报表切换**（入库报表/出库报表/库存报表/物料进出存报表），仅展示列不同；**入库/出库报表额外按 来源/出库类型 展开粒度**。**当前月份之后的月份尚未发生，单元格留空**（前端按 `_currentMonthIndex` 判断）。
