...
> **V2.1 变更：** RunningStatus/InspectionStatus/MaintStatus 现为 Equipment 实体物化存储字段，由 `EquipmentStatusCalculator` 统一管理。点检/保养/维修的写操作（创建/更新/删除）后自动触发对应状态的重算和持久化。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| **V2.4** | **2026-08-15** | **过时描述清理**：全文档核对当前代码（物化状态计算逻辑、扫码报修/维修端点、故障类型、DataExchange 注册、文件清单），无过时内容需要删除，仅更新版本/日期 |
| **V2.3** | **2026-08-09** | **文档审查修复**：§4.9 报修人自动填充由"从 JWT 读取"修正为"从 localStorage 的 userFullName/userName 读取"；§六 补充枚举中英/显示参数化说明（后端存储英文枚举名，前端中文显示，枚举走 EnumHelper.GetDisplayName，字典走 DictValueDisplayHelper） |
| **V2.1** | **2026-07-09** | **设备物化状态同步**：RunningStatus/InspectionStatus/MaintStatus 从动态计算改为 Equipment 实体物化存储，由 EquipmentStatusCalculator 统一管理；更新 §2.3 状态联动图，新增点检/保养物化状态同步链路；修正 EQ-004~EQ-006 业务规则描述 |
| **V2.0** | **2026-06-16** | **新增扫码报修子模块：EquipmentRepair 扫码报修页（3 步流程） + 设备二维码打印功能；更新文件清单** |
| V1.0 | 2026-05-16 | 初始版本，定义设备上下文4个子模块的完整详细设计 |
| **V2.2** | **2026-08-01** | **文档审查修复**：EQ-005 未到执行时间→Pending 修正为当前周期未开始（today<起始日）→Normal；RO-009/§4.10 其它维修人由追加到 RepairPerson 字段改为存入独立 OtherRepairPersons 字段；§7.3 前端服务路径修正（Equipment/*.cs 4 个服务 + Common/ScanService.cs） |
