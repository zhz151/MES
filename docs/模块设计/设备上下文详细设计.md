...
| EQ-005 | 点检状况（InspectionStatus）为 Equipment 实体物化存储字段，由 EquipmentStatusCalculator 根据点检周期和实际执行日期自动计算：NeedInspection=false→NotApplicable；当前周期内有记录→Normal；已逾期→Overdue；未到执行时间→Pending |
...
| RO-009 | 多人协作：其它维修人追加到 RepairPerson 字段（逗号分隔，自动去重） |
...
| 填写 | Form | 显示设备摘要信息 → 自动填充报修人（从 JWT 读取）→ 选择故障类型 → 可选填写故障描述 → 提交 |
...
**多人协作：** 在"其它维修人"输入框输入姓名，以 MudChip 展示，支持删除。完成维修时追加到 RepairPerson 字段（逗号分隔）。
...
| MES.Blazor/Services/EquipmentService.cs | 设备前端服务 |
| MES.Blazor/Services/ScanService.cs | 扫码解析服务（含 `ResolveEquipmentAsync` 设备扫码解析） |
| MES.Blazor/Services/InspectionRecordService.cs | 点检前端服务 |
| MES.Blazor/Services/MaintenanceOrderService.cs | 保养前端服务 |
| MES.Blazor/Services/RepairOrderService.cs | 维修前端服务 |
