...
> ⚠️ 「物料读/物料写」两列已移除（物料档案 MaterialController 随主档删除，2026-08-18）；MaterialViewer/MaterialEditor/MaterialFull/ReportViewer/ReportEditor/ReportFull/Admin 角色保留，供采购/委外/供应商页面授权。
...
| **V3.7** | **2026-08-26** | **字段引用修正**：§五 跨上下文查询链路 WorkOrder.OrderNo → WorkOrder.SalesOrderNo（源订单号）、WorkOrder.ItemSequence → WorkOrder.OrderItemIds（项次序号列表，逗号分隔）——WorkOrder 实体无 OrderNo/ItemSequence 字段；MAT-010 同步 |
