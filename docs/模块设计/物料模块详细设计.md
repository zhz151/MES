...
- **二、执行状态（G2，6列）：** 执行状态（4 档 MudChip）、截止回收日、回收支数、回收重量(kg)、退货量（X支/Ykg 或 "-"）、属强制完成（是/-）

**加载方式：** 全量加载+内存筛选（跨表字段 OrderNo/SupplierName/OrderDate/RequiredArrivalDate=ReturnDeadline 等导航属性字段通过 DTO 投影预解析；退货量按委外单号→退货-原仓库批反查原仓库批 SourceOrderNo==委外单号实时汇总；筛选/排序均在内存 List 中完成）
