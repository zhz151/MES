...
**表格列（支持排序，共32列，支持列选择器显隐/排序）：** 委外单号(MudLink)、下单日期、加工类型、物料分类、工厂牌号、规格、发出支数、发出重量、收回期限、供应商、状态、已回收支数、已回收重量、来源工单号、订单号(Wo)、主号(Wo)、次号(Wo)、签订日期(Wo)、业务员(Wo)、最终用户(Wo)、交货日期(Wo)、延期罚款(Wo)、结算方式(Wo)、工厂牌号(Wo)、成品规格(Wo)、长度状态(Wo)、最大长度(Wo)、总支数(Wo)、总重量(Wo)、交货状态(Wo)、含项次数(Wo)、操作
