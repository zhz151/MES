...
> 版本：V7.11（2026-09-16）
> 最新变更（V7.11，2026-09-16）：**历史条目状态补注（纯文档修订）**——V7.6 条末尾原「⚠️ 本批尚未打包上线」已失效，就地补注为「已于 2026-09-15 随 `mes-deploy-20260915_1539.zip` 上线」（原文保留，依据 `docs/本机与线上发布差异待办.md`「当前状态」）。**零正文改动、零代码 / 零迁移 / 零数据变更**。
> 历史变更（V7.10，2026-09-16）：**文档与代码对账（纯文档修订，§5.17 编号 + §10 用料计划路由/操作列）**——① **§5.17「API 控制器清单」的 7 个子节标题编号修正**：原为 `#### 5.7.1 ~ 5.7.8`（沿用已废弃的 §5.7 编号且缺 5.7.5，与父节 §5.17 不符），改为 **`#### 5.17.1 ~ 5.17.7`**（小节相对顺序不变，闭合缺号）；父节「共 7 个 API 控制器」与 `MES.Api/Controllers/WorkOrder/*.cs` 实际 7 个文件（WorkOrderController / WorkOrderExecutionController / MaterialPlanController / NotificationController / MaterialPlanProcessGroupController / OrderDemandAdjustmentController / FixedLengthWorkOrderController）**逐一对上**；V7.9 变更说明中的 §5.7.x 引用即对应现 §5.17.x。② **§10.1 Tab1 圆棒穿孔补编辑路由** —— 除 `/piercing-plan/create` 外另有 `/workorders/{WorkOrderId:int}/piercing-plan/edit/{Id:int}`（独立文件 `WorkOrderPiercingPlanEdit.razor`，⚠️ 路由参数名是 `Id` 而非 `PlanId`）。③ **§10.1 Tab6 在产改制删除「（编辑 `…/edit/{PlanId:int}`）」** —— 该计划**无编辑路由**（`WorkOrderMaterialPlan.razor` Tab6 行内仅「打印」+「取消」；全仓无 `in-process-rework-plan/edit` 路由）。④ **§10.3「操作列」更正** —— 原文「仅保留『打印』+『取消』（已移除『编辑』+『刷新状态』）」对 Tab1~Tab5 不成立：实际 **Tab1~Tab5（圆棒穿孔/荒管采购/成品采购/库存使用/库料改制）= 「编辑」+「打印」+「取消」**（取消包 `AuthorizeView Roles="@Roles.Policies.WorkOrderDelete"`）、**Tab6 在产改制 / Tab7 在产主工单 = 「打印」+「取消」**；「刷新状态」按钮确已移除（`POST refresh-status/{workOrderId}` 已下线）。
> 历史变更（V7.9，2026-09-16）：**文档与代码对账（纯文档修订，§5.7 API 清单）**：以 `MES.Api/Controllers/WorkOrder/*.cs` 实际端点 + 《工单上下文接口设计》「已下线」注记为准，**删 7 补 22**：① **§5.7.1 工单 API** —— 删 4 个已下线端点 `GET api/workorder/order-status`、`GET api/workorder/order-status-all`、`POST api/workorder/check-all-order-change`（原「订单状态监控」三端点，改由 `GET api/workorder/list` 的 `Status` 承载）与 `POST api/workorder/refresh-material-plan-readmodel`（用料计划读模型改为写路径自动失效重建），改为表下「已下线端点」注记；补 `GET api/workorder/list-with-plans`（含用料计划聚合的工单列表，读 `WorkOrderListSummary`）与 `GET api/workorder/{id}/operation-logs`。② **§5.7.2 工单执行状况 API** —— 补 `dashboard-summary` / `error-doubt-inputs` / `in-production-inspection-doubt-items` 三端点。③ **§5.7.3 用料计划 API** —— 补 14 条漏记端点（`PUT semi/{id}`、`PUT finished/{id}`、`GET inventory/plan/{id}`、`PUT inventory/{id}`、`GET in-process-batches/{workOrderId}`、`GET pending-inprocess-rework`、`GET/POST/PUT/DELETE in-main-work-order*`、`GET main-work-order-batches/{workOrderId}`、`GET pending-in-main-work-order`、`POST print/in-main-work-order/{id}/file`、`GET pending-batches/{warehouseId}`），并把 `POST refresh-status/{workOrderId}` 标注**已下线**（状态为查询时派生）。④ **§5.7.4 通知 API** —— 删已下线 `GET unread-count` 与 `POST read-all`，改表下注记（未读数改由 `by-type/{type}` 列表长度承载、全部已读改逐类型 `mark-type/{type}/read`）。⑤ **§10.1 7 类计划体系表** —— 「路由」列原写的 `/workorders/{id}/piercing-plan`、`/finish-plan`、`/inventory-plan`、`/rework-plan` **四个列表页路由在代码中已不存在**（全仓仅存各 `…/create` 与 `…/edit/{PlanId}`）；改表为「页面位置（Tab）+ 创建页路由」两列，明确 7 类计划统一挂合并页 `/workorders/{WorkOrderId:int}/material-plan` 的 Tab1~Tab7（顺序与 `WorkOrderMaterialPlan.razor` 的 `MudTabPanel` 一致）；同步删 §10.3 下方「旧路由保留兼容」的过时注记，改为「原独立列表页已删除、旧路由不再保留」。⑥ §5.7.7 需求调整 / §5.7.8 定尺工单两表核对后与代码一致，未改。
> 历史变更（V7.8，2026-09-16）：**文档与代码对账（纯文档修订）**——V7.7 改名已落地代码但本文仍留旧名，本次补齐 5 处：① §1 概述「定尺工单定尺」→「**查询定尺工单**」（源 `Pages/WorkOrders/FixedLengthWorkOrderView.razor:17` 页头）；② §5.18 功能说明同改；③ §5.8 顶栏标题「工单需求调整」→「**需求调整**」（源 `OrderDemandAdjustment.razor:22`）；④ §5.9 标题「用料计划页」→「**工单用料页**」、§5.9.1「用料投料核查页」→「**用投料核查页**」（源 `MaterialPlanOverview.razor:26` / `MaterialInputConsistency.razor:24` 页头）；⑤ §10 页面清单「用料计划 / 用料投料核查」两行页面名同步为「工单用料 / 用投料核查」。**API 端点名与实体名零改动**（`api/fixed-length-work-order/*`、`api/workorder-demand-adjustment/*` 等保持原名）。
> 历史变更（V7.7，2026-09-15）：**「工单管理」菜单拍平 + 6 项改名重排**（纯前端菜单树与页面标题，**后端 / API / DB / 迁移 / 权限 / 路由零改动**）：① 原「工单操作 / 工单查询」两个三级分组**取消**，其下 6 项上提为**二级**并列（`MES.Blazor/Shared/AppMenu.cs`）；② 改名（菜单名 ↔ 页面标题同步）——工单生成（不变）、用料计划 → **工单用料**、用料投料核查 → **用投料核查**、工单需求调整 → **需求调整**、工单执行状况 → **查询工单执行**、定尺工单定尺 → **查询定尺工单**；③ 新序：**工单生成 → 需求调整 → 工单用料 → 用投料核查 → 查询工单执行 → 查询定尺工单**；④ 首页常用入口 `AppShortcuts`「用料计划」→「工单用料」（Label 必须与菜单叶子一致，`AppShortcutsTests` 断言）；⑤ `AppMenuTests` 断言改为「六项并列二级_无子分组」，`AppMenuTests` + `AppShortcutsTests` **14 例全过**；⑥ 6 项权限仍继承分组 `WorkOrderMenu`，**可见性零变更**；无 CSS 变更 → 不抬 `index.html` 版本串。详见前端页面结构参考 V144。
> 历史变更（V7.6，2026-09-15）：**「在产节点待量」G17 抽为共享计算器 + 删私有重复 `GetSectionSequence`**（**无 EF 迁移 / 无数据库变更 / 无权限变更 / 无 API 变更**）：① **新建 `MES.Services/Helpers/ProductionPendingNodeHelper.cs`**（静态类、纯内存、不依赖 `DbContext`）——承载 8 节点单源表 `NodeDefs`（`(Key, Label, ProcessName, SectionName)`，**顺序即原 `nodeDefs` 顺序，是「生产关注工序」tie-break 依据，禁止重排**）、`ActiveBatches`（排除成检/完成两态）、`Compute`（原 G17 循环逐行搬入 + 新增「在产 / 在途」分桶）、`Totals`、`ApplyTo`（写 8 个 `PendingSection*`，`>0 ? value : null`）。**验收铁律**：8 个 `PendingSection*` 数值逐字段不变，由定向单测 `WorkOrderExecutionServiceTests.RefreshAllAsync_在产待量八字段_与公共计算器逐字段一致` 锁死。② **`WorkOrderExecutionService` 瘦身**——G17 内联块（原 1944~2066 行）替换为 `Compute` + `ApplyTo` + `Totals` 三行调用；`group14Batches` 仍留服务内（「关注工序」块也用），其后「变形工序完成三档 + 生产关注工序」块**逻辑一字不改**，仅把 `nodeDefs` 改读 `ProductionPendingNodeHelper.NodeDefs`。③ **删私有重复实现** `WorkOrderExecutionService.GetSectionSequence`（与 `ProcessGroupExtensions.GetSectionSequence` 等价），补 `using MES.Services.Extensions;`；同仓等价实现由三份降为两份（`ProductionOverviewService` 那份属另一层，**未动**）。④ **共用方新增第二处**——`OrderProgressQueryService` 的「生产执行」分支改由本计算器**实时**计算（原读快照），并顺带带出各节点批次名单，详见 `订单模块详细设计.md` §5.17.6；两份 `nodeDefs` 定义（原本服务局部数组 + 原 `OrderProgressQueryService.ProductionNodeDefs`）由此收敛为一份。⑤ 验证：`dotnet build MES.Api` / `MES.Blazor` **各 0 错 0 警**；定向单测 `ProductionPendingNodeHelperTests`（新建 16 例，含节点表顺序锁、分段合计不变式、工段级到达判定、段内稳定序、`ApplyTo` 字段错位护栏）+ `OrderProgressQueryServiceTests` **21 过** + 本服务测试合计 **102 过**；真库 SQL 核验（`D26Z2159001`/`X03`）荒管处理 19281 / 50冷轧 37433 / 30冷轧 98973 与快照值一致。**⚠️ 本批已于 2026-09-15 随 `mes-deploy-20260915_1539.zip` 上线**（原文为「尚未打包上线」，2026-09-16 对账补注）。详见 §5.5 在产节点待量注。
> 状态：**已实施完成**。工单由订单项次手动生成（首次/覆盖/待修正/更新修改四种模式），订单项次 CRUD 时增量触发待修正检测；工单执行状况读模型按 18 组列展示并增量刷新（主号级实时关注五档、到料实投一致性七态、返整执行、入库状态、在产节点待量等）；含定尺工单联通视图、工单需求调整手工字段表；用料计划 7 类体系（圆棒穿孔/荒管采购/成品采购/库存使用/库料改制/在产改制/在产主工单）三级聚合、工序组子实体与批量打印；操作日志统一由基础设施 OperationLog（Module="WorkOrder"）记录。**2026-09-08：原用料计划总览页两页拆分**——用料计划（规划工作台，实时关注收 3 字段，默认列 20 项 + 加载默认 原料锁定）/ 用料投料核查（实时关注列组 + 两错疑卡联动），数据层/读模型零改动（见 §5.9、§10.3）。
...
- **定尺工单联通视图**：定尺工单（FixedLengthWorkOrder）冗余表随工单生成/编辑/删除自动重建（BuildFixedLengthWorkOrders 按 OrderItem.MaxLength 分组，LengthStatus==Fixed，冗余字段从 WorkOrder 填充无漂移风险），页面 `/fixed-length-work-order-view`「查询定尺工单」，端点 `GET api/fixed-length-work-order/list`
- **在产改制计划**：从可用在产批次中选取进行改制计划
- **在产主工单计划**：从可用在产批次中选取，按主工单分配支数/重量（第7类）
- **工单需求调整**：手工标记催单/分批交货/主号暂停，管理排程优先级
...
**待生成工单区域：**
- 位置：主表格上方
- 显示条件：存在订单状态为 `Confirmed` 且未生成任何工单的订单（`GET api/workorder/pending-orders`）
- 显示内容：订单号、签订日期、业务员、最终客户 + 「生成工单」按钮
- 操作：点击「生成工单」进入首次生成页面；无此类订单时不显示
- 权限：WorkOrderViewer/WorkOrderEditor/WorkOrderFull/ReportViewer/ReportEditor/ReportFull/Admin
...
> **定尺模式**：MaxLength 取合并项次中所有 MaxLength 的最大值，不强制覆盖为 MinLength（避免多个定尺项次合并到同一工单时最大长度丢失）。

**更新修改模式聚合重算规则：**

当使用"更新修改"模式时，无论项次是否有变化，始终重新计算聚合字段（MinLength、MaxLength、TotalQuantity、TotalWeight 等），以修复原始生成时的错误值。
...
**检测触发方式：** 不再依赖定时任务，改为在订单项次 CRUD 时自动增量触发（由 `OrderService.UpdateItemAsync` / `DeleteItemAsync` 在项次变更后自动标记关联工单为 Pending）。
...
> **精准消除原理：** Type A 场景下，多个分工单可能共享同一源批次的余量。若使用批量消除，所有分工单的待处理计划会被一并消除，导致未创建批次的分工单丢失通知。精准消除方法 `DismissInMainWorkOrderPlanByBatchAndWorkOrderAsync` 和 `DismissInProcessReworkPlanByBatchAndWorkOrderAsync` 通过 `ProductionBatchId` + `WorkOrderNo` 双重条件匹配，仅消除当前分工单对应的计划记录。
...
- **第一块：待生成工单区域**：订单已确认但尚未生成工单的订单以信息卡片网格展示（订单号、签订日期、业务员、最终客户 + 「生成工单」按钮，点击进入首次生成页），无此类订单时不显示（`GET api/workorder/pending-orders`）
...
> **列默认显隐（2026-09-08）**：工单列表「次号」「最终客户」「钢管制造」默认隐藏（列显隐可切换打开）；列偏好 ColumnPrefs 持久化键并入 `ColumnPrefsVersion="v1"`（`col_prefs_workorders_v1`，原版本传 null），旧持久化偏好失效一次、新默认立即生效。
...
**导航位置：** 工单管理 → 查询工单执行（V7.7 改名，原「工单查询 → 工单执行状况」）
...
> **列默认显隐（2026-09-08）**：基础数据「最终客户」「订单日期」默认隐藏（列显隐可切换打开）；本页整页状态（排序/筛选/列显隐）一体存于 PageState，本次并入存储键 `page_state_workorderexecution-v1`（`ColumnLayoutVersion="v1"`），旧持久化状态失效一次。

**工单需求调整（5列）：** 催单、分批交货、暂停、强制完成、调整备注

**实时关注（7列，主号级）：** 主号-关注（五档：0=主号暂停/1=主号完成/2=原料锁定/3=生产执行/4=成品检验）、主号-计划性（A+急/A急/B顺/C缓/D缓，存储英文 Key）、主号-预计完成日、主号-交期相差天数、主号-剩余总工量(天)、主号-产能工量(天)、主号-原锁备注（仅原料锁定有值：质量补料/生产返整补足/执行用料计划/完善用料计划，2026-09-07 去 A/B/C/D 前缀更名）
...
> **批次作用域原则：** 所有涉及批次数据的 Group，在计算时使用 **当前工单自身的批次集合**（`woBatches`），而非同主号下所有兄弟工单的批次集合（`batches`），避免跨工单投料量串扰。
...
> **入库状态口径：** 工单级（WoWarehousingStatus）独立判定不聚合：定尺按支数（=需求→完结、>需求→超额）、非定尺按重量容差（≥需求×0.95 且 ≥需求−100→完结、>需求×1.05→超额）。主号级（MainNoWarehousingStatus）按主号聚合入库量 vs 主号总需求直接比较：定尺按支数（=需求→完结、>需求→超额）、非定尺按重量容差（同工单级容差，>需求→超额）。订单级（OrderWarehousingStatus）从主号状态上卷：全0→无入库、全达成(2/3)→入库完结、否则→入库部分。
...
>
> **在产节点待量计算（2026-09-15 抽为共享计算器）**：8 节点待量（荒管处理/在制修检/60冷轧/50冷轧/30冷轧/20冷轧/三辊冷轧/冷拔）的计算逻辑**由本服务内联块 + `OrderProgressQueryService` 的 `ProductionNodeDefs` 两份重复定义，收敛为单一来源 `MES.Services/Helpers/ProductionPendingNodeHelper.cs`**（静态类、纯内存、不依赖 `DbContext`；目录风格参照同目录 `SubcontractHelper.cs`）。共用方：① **本页读模型**——`WorkOrderExecutionService.ComputeSummary` 的 G17 块调 `ActiveBatches` + `Compute` + `ApplyTo`，把结果写入 `WorkOrderExecutionSummary` 的 8 个 `PendingSection*` 字段（**快照口径**）；② **订单进度树**——`OrderProgressQueryService` 对批次实时 `Compute`（**实时口径**，并顺带带出各节点「在产 / 在途」两段批次名单，详见 `订单模块详细设计.md` §5.17.6）。⚠️ 两侧**共用同一份算法**，但**取值时机不同 → 数值会长期分叉**（本页未点「即时刷新」时显示旧快照），属设计取舍非 bug。
>
> `ProductionPendingNodeHelper` 内容要点：a. **`NodeDefs`** —— 8 节点**单源表** `(Key, Label, ProcessName, SectionName)`，**顺序即原 `nodeDefs` 顺序，同时是「生产关注工序」取最小序号工序时的 tie-break 依据，禁止重排**（顺序漂移会让关注工序漂移，已由单测 `NodeDefs_顺序与键集锁定` 锁死）；b. `BatchRef` / `NodePending` 两个 record；c. `ActiveBatches`（排除 `BatchStatus.InFinalInspection` / `Completed` 两态，即原 `group14Batches` 原义）；d. `Compute` —— 原 G17 循环**逐行搬入**（`ProcessGroups` 为空 → 跳过、目标工序组不存在 → 跳过、`CurrentValidWeight ?? 0m` 三条原样保留），**唯一新增**是「命中的同时按 `batchCurrentSeq` vs `targetSeq` 落在产 / 在途桶」；e. `Totals`（`OrdinalIgnoreCase`，供「生产关注工序」块沿用旧写法）；f. `ApplyTo`（写 8 个 `PendingSection*`，`> 0 ? value : null`）。**⚠️ 前置条件**：入参批次的 `ProcessGroups` 必须已 `Include` 加载——未加载时集合为空 → 全部批次判定为「不适用」→ 8 节点待量**恒为 0 且不抛异常**（静默口径错误），已在 helper 的 XML 注释写死该前置条件。
>
> **同批清理**：`WorkOrderExecutionService` 原私有 `GetSectionSequence`（与 `ProcessGroupExtensions.GetSectionSequence` 等价的重复实现）已删除，统一走既有扩展方法（原三份等价实现 → 两份；`ProductionOverviewService` 那份属另一层，未动）。
...
>
> **产能工量(天)**（WorkOrderExecutionSummary 与 WorkOrderListSummary 两表同口径）：`Ceiling((主号总重量 − 成品采购FinishPlanWeight − 库存使用InventoryPlanWeight − 已完成批次有效产出completedOutput) / 1000 / 规格日产预估(吨/天))`，剩余 ≤ 0 → 0，**主号完成 → null**（显示「-」）。completedOutput = 已完成批次（Status=Completed 且 ProductionType≠"Rework" 且 ManufacturingItem∈{OrderFinished, SpecialDeliveryStatus}）有效产出 `Σ(批次有效原料重CurrentValidWeight × (1 − 有效工序组数 × 2.5%))`，**排除外购/库存批次**（外购重量已由成品采购计划扣减、库存已由库存使用计划扣减，防双重扣减导致负值低估）。
>
> **用料计划中的产能工量**（与执行层面同一口径）：`Ceiling((主号总重量 − 成品采购 − 库存使用 − completedOutput) / 1000 / 规格日产预估)`，主号完成→null。
>
...
**导航位置：** 工单管理 → 需求调整（V7.7 改名，原「工单操作 → 工单需求调整」；新序位列「工单生成」之后第 2 位）
...
> **列默认显隐（2026-09-08）**：基础数据「订单日期」「最终客户」与实时关注「主号-预计完成日」「主号-原锁备注」默认隐藏（列显隐可切换打开）；本页整页状态（排序/筛选/列显隐）一体存于 PageState，本次并入存储键 `page_state_workorders-demand-adjustment-v1`（`ColumnLayoutVersion="v1"`），旧持久化状态失效一次。
...
> **2026-09-08 一分为二**（计划归计划、核查归核查）：原「用料计划总览」按职责拆为 2 个独立菜单页（用料计划规划工作台 + 用料投料核查），数据层/读模型（`WorkOrderListSummary`）与后端 `WorkOrderService.GetPagedWithPlansAsync` 语义**零改动**，仅显示投影不同：
> - **用料计划（本页，规划工作台）**：列组=基础数据 / 实时关注（收窄为仅 3 字段：主号-关注/主号-原锁备注/主号-计划性）/ 用料计划（12 列全）；卡片仅保留**待投料量汇总**；两错疑卡（错疑-用料投料不一致、错误-用料计划及其执行）与卡↔表联动迁出；列偏好 key 升 v6；默认列显隐收敛 20 项 + 页面加载默认按 主号-关注=原料锁定（档位 2）过滤。
> - **用料投料核查**（见 5.9.1，核查「投料是否错疑」）：实时关注列组 + 两错疑卡联动，无用料计划列组、无汇总卡、无计划类型勾选。

### 5.9.1 用投料核查页

**页面路由：** `/material-input-consistency`

**页面文件：** `MES.Blazor/Pages/WorkOrders/MaterialInputConsistency.razor`

**功能说明：** 用料「核查」视图（独立终态页，仅显示投影）。列组=基础数据 / **实时关注**（仅 主号-关注/主号-原锁备注/主号-计划性 三字段，主号级标色）/ **用料及投料**（2026-09-08 自实时关注拆出新组：分类用料=计划量占比%、分类到料=各来源已到货量整数 kg 空格串 如「成1200 荒1500」（7 类顺序 穿/荒/成/库/改/在/主，与占比列同构）、原料未至、到料未投、生产投料量、投料比、投料状态）；基础数据默认隐藏 最终用户/最大长度；**用料及投料组除「投料状态」外均不支持排序/筛选**（列头静态展示）；投料状态「超量」=深色底白字 Chip（chip-dark）与「满足」绿色 Chip 区分。两张异常卡——**错疑-用料投料不一致**（ErrorDoubtInputItem，原料锁定档位）与**错误-用料计划及其执行**（InProductionInspectionDoubtItem，主号-关注=主号完成/生产执行/成品检验 三档，**已过投料期**；标题旁附注标签「**以下执行状态无需再投料**」）——自原用料计划页迁入，卡点击 → 主表联动筛选（ScheduleStage 档位 / 待料字段），卡片收起恢复。**错疑卡重量三列表头定名：工单重量 / 计划投料重量 / 到料重量**（到料重量=各来源已到货/接收重量之和，到货量口径，非「计划允许投的量」）；**错误卡聚合行首列表头=工单执行状态**，聚合列=原料未至（用料计划缺口）/到料未投（投料执行缺口）。无待投料量汇总卡、无计划类型勾选、无用料计划列组。列偏好 key `materialInputConsistency_v4`（列组重排升版）、页面状态 key `materialInputConsistency`。
...
| GET | `api/workorder/list-with-plans` | 分页查询工单列表（含用料计划聚合数据，读 `WorkOrderListSummary` 读模型） |
| GET | `api/workorder/list-all` | 获取全部工单列表 |
| GET | `api/workorder/{id}` | 获取工单详情 |
| GET | `api/workorder/{id}/operation-logs` | 获取工单操作日志 |
...
| POST | `api/workorder/order-print-list-file` | 打印当前勾选工单列表 PDF（Mode A 前端逐行转 dict 发送） |
| POST | `api/workorder/order-print-batch-file` | 按订单批量打印选中工单（直接返回 PDF 文件） |
| GET | `api/workorder/filter-contexts` | 筛选上下文 |

> **已下线端点**（依据《工单上下文接口设计》§7 注记 + `WorkOrderController.cs` 实际端点清单）：
> - `GET api/workorder/order-status` / `GET api/workorder/order-status-all` / `POST api/workorder/check-all-order-change`（原「订单状态监控」三端点）**已下线** —— 工单生成入口的订单/工单状态列表统一由 `GET api/workorder/list` 返回（`WorkOrderListItemDto.Status` 即工单状态），订单变更检测在订单项次 CRUD 时自动增量触发，无手动兜底端点。
> - `POST api/workorder/refresh-material-plan-readmodel`（手动刷新用料计划读模型）**已下线** —— 用料计划读模型改为数据源写路径自动失效重建。
...
| GET | `api/workorder-execution/dashboard-summary` | 执行状况看板汇总（首页/看板用） |
| GET | `api/workorder-execution/error-doubt-inputs` | 到料实投一致性「存疑」明细 |
| GET | `api/workorder-execution/in-production-inspection-doubt-items` | 在产/检验存疑明细 |
...
| PUT | `api/material-plan/semi/{id}` | 更新原料采购计划 |
...
| PUT | `api/material-plan/inventory/{id}` | 更新库存使用计划 |
...
| <del>POST</del> | <del>`api/material-plan/refresh-status/{workOrderId}`</del> | **已下线**（用料计划状态为查询时派生字段，无需也不存在独立刷新端点） |
...
| GET | `api/material-plan/in-process-batches/{workOrderId}` | 查询可用在产批次 (→仓库) |
| GET | `api/material-plan/pending-inprocess-rework` | 待处理在产改制待选批次 |
| POST | `api/material-plan/print/in-process-rework/{id}/file` | 打印在产改制计划（直接返回 PDF 文件） |
| GET | `api/material-plan/in-main-work-order/{workOrderId}` | 查询在产主工单计划列表 |
| POST | `api/material-plan/in-main-work-order` | 创建在产主工单计划 |
| PUT | `api/material-plan/in-main-work-order/{id}` | 更新在产主工单计划 |
| DELETE | `api/material-plan/in-main-work-order/{id}` | 删除在产主工单计划 |
| GET | `api/material-plan/main-work-order-batches/{workOrderId}` | 查询可用在产主工单批次 (→仓库) |
| GET | `api/material-plan/pending-in-main-work-order` | 待处理在产主工单待选批次 |
| POST | `api/material-plan/print/in-main-work-order/{id}/file` | 打印在产主工单计划（直接返回 PDF 文件） |
| GET | `api/material-plan/pending-batches/{warehouseId}` | 按仓库查询待处理计划批次（权限 `WarehouseView`） |
...
| GET | `api/notification/by-type/{type}` | 获取指定类型通知（服务端固定 `!IsRead` 过滤，**未读数 = 返回列表长度**） |
| POST | `api/notification/mark-type/{type}/read` | 标记指定类型全部已读 |

> **已下线端点**：`GET api/notification/unread-count` 与 `POST api/notification/read-all` **已下线** —— 未读数由 `GET api/notification/by-type/{type}` 返回的未读列表长度承载；「全部标记已读」由前端**逐类型**调用 `POST api/notification/mark-type/{type}/read` 实现。
...
| 方法 | 路由 | 说明 |
|------|------|------|
| GET | `api/fixed-length-work-order/list` | 获取全部定尺工单定尺数据列表（主号级按长度实时聚合；IMemoryCache 60 秒绝对过期兜底，断切生产记录/正式尺寸成检/工单重建/订单类成品入库等数据源写路径 `InvalidateCaches` 主动失效，2026-09-08） |
...
**功能说明：** 「查询定尺工单」联通视图，列表模式（Items 模式，对齐 RawMaterialLockPlan 模板），主号级按长度实时聚合展示定尺工单的切割/成检/成品入库/总现况数据。FixedLengthWorkOrder 冗余表随工单生成/编辑/删除自动重建（BuildFixedLengthWorkOrders 按 OrderItem.MaxLength 分组，LengthStatus==Fixed，WorkOrderNo/SalesOrderNo/ProductionMainNo 冗余字段从 WorkOrder 填充，无漂移风险）。
...
- 6 组字段：G1基础数据 / G2计划状态 / G3成品切割 / G4成检数据 / G5成品入库 / G6主号数据及现况分析（主号级汇总，三档恒等式：免切 + 理论待切 + 理论已切 = 理论可产支；现有效产支 = 免切 + 理论待切 + 实切 − 预计损耗 − 次品支数；总盈亏支数 = 现有效产支 − 需求总支）
- **列默认显隐（2026-09-08）**：G3成品切割、G4成检数据、G5成品入库三组默认整组隐藏（组头列显隐可整组打开）；基础数据「往来单位」「订单日期」默认隐藏；G6 主号级字段标签去掉「主号-」前缀并精简（2026-09-08 改名：需求总支/理论可产支/免切/理论待切/理论已切/实切支数/切割偏差/预计损耗/现有效产支/次品支数/盈亏支数/盈亏状态）。本页无列偏好持久化，加载即默认。
...
- **G6 切割偏差阈值（2026-09-08 配置化）**：`MainNoCutRationality` 判定原硬编码 5%，改为参数表 **FixedLengthCutRatio/CutDeviationRatio 驱动、默认 3.5%**（主号为汇总数据，比批次单批次 CutDoubtRatio 5% 更严）；Service 组装列表时读一次注入 `CutDeviationRatio`，参数表保存由 ConfigParameterService 即时失效定尺列表缓存实时生效（DbInitializer 种子 + 迁移 `SeedFixedLengthCutDeviationRatio` 已落真库）
- 数据缓存：`GetListAsync` 结果 IMemoryCache **60 秒绝对过期兜底**（键 `FixedLengthWorkOrderService:List`，专用常量 `CacheDefaults.FixedLengthListExpiry`，2026-09-08 由 5 分钟收敛），**数据源写路径主动失效**：断切生产记录新增/修改/删除（`ProductionRecordService` SectionName==Cut）、正式尺寸成检写（`FinalInspectionService` 尺寸+正式+定尺长度）、工单生成/更新/删除重建 FixedLengthWorkOrder 行（`WorkOrderService`）、订单类成品（OrderFinished/SpecialDeliveryStatus）入库/批量入库/入库更正/删除（`InventoryBatchWriteService`）→ `IFixedLengthWorkOrderService.InvalidateCaches()`（WorkOrderService 直接 Remove 键）；消除「改完断切回页仍旧值」的 TTL 窗口。剩余执行摘要快照/批次计划（理论量·CutRequirement）漏网点未接失效，由 60 秒兜底收敛
...
| 类型 | 实体 | 页面位置（Tab） | 创建页路由 | 说明 |
|------|------|----------------|-----------|------|
| 圆棒穿孔 | RoundBarPiercingPlan | 工单用料页 Tab1 | `/workorders/{WorkOrderId:int}/piercing-plan/create`（编辑 `/workorders/{WorkOrderId:int}/piercing-plan/edit/{Id:int}`，独立文件 `WorkOrderPiercingPlanEdit.razor`） | 圆棒经穿孔工序，含用料测算（调整壁厚、成材率、投料倍率、正品率）+ 圆棒规格/穿孔规格 + 工序组 |
| 荒管采购 | PurchaseSemiPlan | 工单用料页 Tab2 | `/workorders/{WorkOrderId:int}/material-plan/create`（编辑 `…/edit/{PlanId:int}`） | 外购荒管/半成品，含用料测算（调整壁厚、成材率、投料倍率、正品率）+ 工序组 |
| 成品采购 | PurchaseFinishedPlan | 工单用料页 Tab3 | `/workorders/{WorkOrderId:int}/finish-plan/create`（编辑 `…/edit/{PlanId:int}`） | 直接采购成品，极简设计，不涉及测算（含投料倍率字段），不采用工序组子实体 |
| 库存使用 | InventoryPlan (ReworkType=null) | 工单用料页 Tab4 | `/workorders/{WorkOrderId:int}/inventory-plan/create`（编辑 `…/edit/{PlanId:int}`） | 从可用库存列表中选取进行计划 + 工序组 |
| 库料改制 | InventoryPlan (ReworkType!=null) | 工单用料页 Tab5 | `/workorders/{WorkOrderId:int}/rework-plan/create`（编辑 `…/edit/{PlanId:int}`） | 三种改制类型：空拉改制/少道次改制/人工选择改制，各自不同的库存筛选规则 + 工序组 |
| **在产改制** | **InProcessReworkPlan** | 工单用料页 Tab6 | `/workorders/{WorkOrderId:int}/in-process-rework-plan/create`（**无编辑路由**，行内仅「打印」+「取消」） | 从可用在产批次列表中选取进行计划，改制类型恒为人工选择，外径上限无限制 + 工序组 |
| **在产主工单** | **InMainWorkOrderPlan** | 工单用料页 Tab7 | `/workorders/{WorkOrderId:int}/in-main-work-order-plan/create` | 从可用在产批次列表中选取，按主工单分配支数/重量，编辑完全锁死（可删除后重建），无工序组 |

> **页面路由**：7 类计划统一挂在合并页 `/workorders/{WorkOrderId:int}/material-plan`（`WorkOrderMaterialPlan.razor`，7 个 `MudTabPanel` 顺序即上表 Tab1~Tab7）。原「成品采购 / 库存使用 / 库料改制」等旧列表页**已删除**（旧路由不再保留），无独立列表页。
...
| 工单用料 | `/material-plan-overview` | 三级状态展示+关键字筛选+批量打印+状态颜色标识；支持按计划类型过滤（7个复选框）；计划日期列（可排序、可搜索）；数据源为 `WorkOrderListSummary` 读模型（增量刷新）；**2026-09-08 起为规划工作台**——实时关注收窄 3 字段、待投料量汇总卡保留、两错疑卡迁出（见用料投料核查）、默认列显隐 20 项 + 加载默认 主号-关注=原料锁定 |
| 用投料核查 | `/material-input-consistency` | **2026-09-08 拆分新增**（核查·投料错疑）：列组=基础数据 / 实时关注（3 字段主号级）/ 用料及投料（7 列新组，2026-09-08 拆出）；默认隐藏 最终用户/最大长度；用料及投料除投料状态外不支持排序筛选；投料状态超量=chip-dark 区分满足；两张错疑卡「错疑-用料投料不一致」「错误-用料计划及其执行」+ 卡↔主表联动（ScheduleStage/待料）；无用料计划列组、无汇总卡、无计划类型勾选；列偏好 key `materialInputConsistency_v4` |

> **注：** 原独立列表页（圆棒穿孔/荒管采购/成品采购/库存使用/库料改制）**已删除**，全部并入合并的工单用料计划页（7 个 Tab），旧路由**不再保留**（全仓已无 `@page "/workorders/{...}/piercing-plan|material-plan|finish-plan|inventory-plan|rework-plan"` 这类列表页路由，仅保留各自的 `…/create`、`…/edit/{PlanId}` 录入页）。
> **2026-09-08 两页拆分**：`/material-plan-overview`（计划工作台）· `/material-input-consistency`（投料核查）共用同一数据源 `WorkOrderListSummary` + `WorkOrderService.GetPagedWithPlansAsync`（`planTypeFilter:null` 返回全部行），**无新增读模型/表/迁移**；仅显示投影差异（列组、卡片、计划类型勾选）。

**每类计划的表格列**：包含计划日期、计划明细、状态、操作（打印+取消/删除）。

**操作列**（源 `WorkOrderMaterialPlan.razor` 各 `MudTabPanel` 实有按钮）：**Tab1~Tab5（圆棒穿孔/荒管采购/成品采购/库存使用/库料改制）** = 「编辑」+「打印」+「取消」（取消按钮包 `AuthorizeView Roles="@Roles.Policies.WorkOrderDelete"`）；**Tab6 在产改制 / Tab7 在产主工单** = 仅「打印」+「取消」（**无编辑入口**，行内不可改）。删除使用 ConfirmDialog 确认弹窗。"刷新状态"按钮已移除。

### 10.4 批量打印

用料计划页面工具栏提供"打印选中"按钮，支持按计划类型筛选打印内容（7个复选框：圆棒穿孔/荒管采购/成品采购/库存使用/库料改制/**在产改制/在产主工单**）。
...
| 列表打印 | POST /api/workorder/order-print-list-file | 打印当前勾选行列表 PDF（Mode A，前端逐行转 dict） |
| 按订单批量打印 | POST /api/workorder/order-print-batch-file | 传入选中的订单号数组，按订单分组打印，每页2个工单 |
...
> **默认工段配置化**：工序选中后 `ApplyProcessGroupDefaults` 从 `ProcessDefinitions.DefaultSections` 配置表读取默认工段列表并填充执行顺序，新增工序/改默认工段无需改代码，在配置上下文「工序配置」页面内联编辑逗号分隔的 SectionKey。
...
一句话:
  ...
  - 工单项次追溯统一使用订单关联页面展示，
  - 工单生成支持打印选中/全部工单（A4卡片样式，单工单每页1个，批量每页2个）。
```
