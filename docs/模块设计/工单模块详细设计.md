...
- **工单执行状况**：物化读模型展示工单维度的投料/执行汇总，9组指标（G1基础数据/G2用料计划含工艺周期默认22天/G3投料数据/G4有效数据/G5物料执行/G6返整执行/G7有效流转/G8成品入库/G9实时关注含计划性/产能工量/工艺预计完成日/交期相差天数/原锁备注），增量触发自动刷新
...
**功能说明：** 以工单维度展示执行状况汇总数据，包括基础信息、用料计划、投料数据、有效数据、物料执行、返整执行、有效流转、成品入库、实时关注共9组指标。数据通过物化读模型 `WorkOrderExecutionSummary` 存储，8 个数据源 CRUD 时自动增量触发刷新（无需手动操作）。
...
**Group 6 - 返整执行数据（12列，ProductionType=返整 且 ManufacturingItem=OrderFinished）：** 理论返整可产成支（Σ每条返整记录重量÷该记录原批次单支重）、理论返整可产成重（=过程检返整量×0.92+成品检返整量×0.96，无返整量为空）、待返整成支（=理论返整可产成支−返整理论成品支，无返整为空，负值归0）、待返整成重（=理论返整可产成重−返整理论成品重，无返整为空，负值归0）、附返整主号状态（主号级三档：0=未投料/1=部分/2=满足，在有效流转基础上加待返整后按主号总需求判定，定尺按支/非定尺按重，阈值=SupplySatisfiedRate）、是否必返整（是/否，主号级：附返整主号状态=满足 且 有效主号状态≠满足 时为"是"，其余一律"否"含无返整数据）、返整投料截止日、返整批次数、返整投料支数、返整投料重量、返整理论成品支数、返整理论成品重量
...
> **批次作用域原则（V6.0）：** 所有涉及批次数据的 Group（G3-G7、G8），在计算时使用 **当前工单自身的批次集合**（`woBatches`），而非同主号下所有兄弟工单的批次集合（`batches`）。曾因错误使用全族批次集合导致 A 工单误包含 B 工单创建的 SplitFromNumber 批次的投料量，已修复。

**Group 8 - 成品入库（7列，从 InventoryBatch 聚合）：** 入库起始日、入库截止日、入库总支数、入库总重量、工单入库状态（0=无入库/1=入库部分/2=入库完结）、主号入库状态（按「主号聚合入库量 vs 主号总需求」判定四档：0=无入库/1=部分/2=完结/3=超额）、订单入库状态（从主号状态上卷：全0→0/全达成(2/3)→2/否则→1）

> **入库状态口径（V6.8）：** 工单级（WoWarehousingStatus）独立判定不聚合：定尺按支数（≥需求→完结）、非定尺按重量容差（≥需求×0.95 且 ≥需求−100→完结）。主号级（MainNoWarehousingStatus）按主号聚合入库量 vs 主号总需求直接比较：定尺按支数（=需求→完结、>需求→超额）、非定尺按重量容差（同工单级容差，>需求→超额）。订单级（OrderWarehousingStatus）从主号状态上卷：全0→无入库、全达成(2/3)→入库完结、否则→入库部分。

**Group 9 - 实时关注（7列）：** 关注状态（五档：0=主号暂停/1=主号完成/2=原料锁定/3=生产执行/4=成品检验）、剩余总工量(天)、产能工量(天)、工单计划性（A+急/A急/B顺/C缓/D缓）、工艺预计完成日、交期相差天数、原锁备注（仅原料锁定有值：A质量影响/B已购未回/C计划未执行/D未完善计划）
...
> **关注状态判定逻辑（五档，优先级从高到低）**：IsPaused=true→0主号暂停；主号入库 MainNoWarehousingStatus≥2→1主号完成；有效主号状态 MainNoFlowStatus≠2→2原料锁定；存在未产/在产/暂停批次→3生产执行；其余→4成品检验。批次暂停(BatchStatus.Suspended)归入生产执行，与在产同等对待；**仅"主号暂停"，不允许"工单暂停"**——工单需求调整 SaveUrgingAsync 联动连带，同主号下未入库完结工单的 IsPaused 一并写入（已闭环工单不被牵连）
>
> **剩余总工量判定逻辑**：主号完成→空；主号暂停→主号最大剩余工量（停滞展示，恢复后继续）；原料锁定→关联主号最大 ProcessCycle + 3；生产执行→关联主号最大 FlowMaxRemainingWorkDays；成品检验→3
>
> **产能工量(天)**：`Ceiling((主号汇总总重量(kg) - 已完成批次有效成品重量(kg)) / 1000 / 日产估算(吨/天))`，主号完成时为空。已完成批次指 Status=Completed 且 ProductionType≠"Rework" 且 ManufacturingItem="OrderFinished"。已完成批次有效成品重量公式：`Σ(批次有效原料重 × (1 - 有效工序组数 × 2.5%))`
>
...
>
> ScheduleStage=1（主号完成）时，剩余总工量/产能工量/工单计划性/工艺预计完成日/交期相差天数均为空；主号暂停(0)剩余工量停滞展示不置空
>
> **原锁备注判定逻辑（仅 ScheduleStage=2 时有值，优先级 A>B>C>D）：**
...
**Group 3 - 实时关注 + 手工调整（11列）：** 关注状态、剩余总工量(天)、产能工量(天)、工单计划性、工艺预计完成日、交期相差天数、催单(MudCheckBox)、分批交货(MudCheckBox)、主号暂停(MudCheckBox)、原锁备注、调整备注(MudTextField)

> **内联编辑**：Group 3 中的 催单/分批交货/主号暂停 使用 MudCheckBox 内联编辑，调整备注使用 MudTextField 内联编辑。修改后自动调用 `SaveUrgingAsync` 保存（防抖 500ms）。**联动连带**：仅"主号暂停"，不允许"工单暂停"——勾选/取消任一工单的暂停，`SaveUrgingAsync` 自动将同主号（订单号+主号）下未入库完结（WoWarehousingStatus≠2）的其他工单 IsPaused 一并写入，已闭环工单不受牵连。
