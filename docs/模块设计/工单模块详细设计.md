...
> 版本：V7.4（2026-08-20）
> 状态：**已实施完成**。工单由订单项次手动生成（首次/覆盖/待修正/更新修改四种模式），订单项次 CRUD 时增量触发待修正检测；工单执行状况读模型按 18 组列展示并增量刷新（主号级实时关注五档、到料实投一致性七态、返整执行、入库状态、在产节点待量等）；含定尺工单联通视图、工单需求调整手工字段表；用料计划 7 类体系（圆棒穿孔/荒管采购/成品采购/库存使用/库料改制/在产改制/在产主工单）三级聚合、工序组子实体与批量打印；操作日志统一由基础设施 OperationLog（Module="WorkOrder"）记录。
...
- **定尺工单联通视图**：定尺工单（FixedLengthWorkOrder）冗余表随工单生成/编辑/删除自动重建（BuildFixedLengthWorkOrders 按 OrderItem.MaxLength 分组，LengthStatus==Fixed，冗余字段从 WorkOrder 填充无漂移风险），页面 `/fixed-length-work-order-view`「定尺工单定尺数据」，端点 `GET api/fixed-length-work-order/list`
...
>
> **用料计划总览中的产能工量**（与执行层面同一口径）：`Ceiling((主号总重量 − 成品采购 − 库存使用 − completedOutput) / 1000 / 规格日产预估)`，主号完成→null。
>
