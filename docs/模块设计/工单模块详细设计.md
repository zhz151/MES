...
- **标准工艺生产周期**：工厂牌号/原料类型/规格对应的标准周期天数，作为参考数据手工维护，同时系统在用料计划创建/更新和工序组保存时自动 upsert（V4.5 起）
...
所有子实体共享相同字段结构，与批次 ProcessGroup 一致（去掉 ProductionBatchId）：
