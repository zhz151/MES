...
> 版本：V140（2026-09-14；生成 2026-08-19）
> 用途/状态：Quick Reference - 按导航菜单分组的前端页面结构参考，§1 上下文总览 / §2 各上下文页面块 / §3 列表页全量清单 / §7 生产编号链接台账。
> 上次实质变更（V140）：**删除「手机横屏切电脑版」例外 + 删除「请横屏」提示条机制——手机/触屏设备一律走手机壳**（2026-09-14，**WASM（`ResponsiveLayout.razor` / `MobileLayout.razor` / `app.css` / `index.html`）+ 删除 2 个组件文件与 1 个测试类；后端 / API / DB / 迁移零改动**）：
