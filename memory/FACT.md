...
## 当前规范版本
- **04_开发规范.md V8.34**（2026-05-31）— 新增 §6.4.4 分页汇总行规范 + §10.7 支数/米数/重量/批次数整数显示规则 + §10.8 百分比 1 位小数显示规则 + B18/B19/B20 检查项
- **mes-code-check SKILL.md V8.34**（2026-06-02）— 重建，同步 04_开发规范.md V8.34
...
- 声明"已完成"的条件：7 步全部打勾，Step 2 无遗漏列
- 核心工作流纪律五：声明"已完成/符合规范"之前必须先执行 B10（全字段排序）+ B11（全文本搜索）+ B18（分页汇总行）+ B19（整数显示）+ B20（百分比显示）
