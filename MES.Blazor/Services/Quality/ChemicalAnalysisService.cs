...
public class ChemicalAnalysisService
{
    ...
    private const string BaseUrl = ApiEndpoints.ChemicalAnalysis;

    public ChemicalAnalysisService(AuthHttpClient http) => _http = http;

    public async Task<ApiResponse<PagedResult<ChemicalAnalysisDto>>> GetAllAsync(int pageIndex = 1, int pageSize = 20, string? keyword = null, string? sortBy = null, bool isDescending = true, DateTime? inspectionDateFrom = null, DateTime? inspectionDateTo = null, string? filters = null)
    {
        try
        {
            ...
            return await _http.GetFromJsonAsync<ApiResponse<PagedResult<ChemicalAnalysisDto>>>(url)
                   ?? ApiResponse<PagedResult<ChemicalAnalysisDto>>.Fail("获取数据失败");
        }
    ...
    public async Task<ApiResponse<ChemicalAnalysisDto>> UpdateAsync(int id, UpdateChemicalAnalysisRequest request)
    {
        try
        {
            return await _http.PutAsJsonAsync<UpdateChemicalAnalysisRequest, ApiResponse<ChemicalAnalysisDto>>($"{BaseUrl}/{id}", request)
                   ?? ApiResponse<ChemicalAnalysisDto>.Fail("更新失败");
        }
        catch (Exception ex) { return ApiResponse<ChemicalAnalysisDto>.Fail($"网络错误: {ex.Message}"); }
    }
    ...
    public async Task<ApiResponse<List<ChemicalAnalysisDto>>> BatchCreateAsync(List<CreateChemicalAnalysisRequest> requests)
    {
        try
        {
            return await _http.PostAsJsonAsync<List<CreateChemicalAnalysisRequest>, ApiResponse<List<ChemicalAnalysisDto>>>($"{BaseUrl}/batch", requests)
                   ?? ApiResponse<List<ChemicalAnalysisDto>>.Fail("批量创建失败");
        }
        catch (Exception ex) { return ApiResponse<List<ChemicalAnalysisDto>>.Fail($"网络错误: {ex.Message}"); }
    }
