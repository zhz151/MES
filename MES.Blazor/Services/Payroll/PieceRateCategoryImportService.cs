...
/// <summary>
/// 生产计件类别专用导入器（2026-09-02）——模板/导出下载 + 预览 + 事务导入。
/// kind = category（类别定义）| tier（维档系数），与后端 PieceRateImportKinds 对应。
/// </summary>
public class PieceRateCategoryImportService
{
    private readonly AuthHttpClient _http;
    private const string BaseUrl = ApiEndpoints.PieceRateProductionCategory;

    public PieceRateCategoryImportService(AuthHttpClient http) => _http = http;

    /// <summary>下载全量导出（双 sheet：类别 + 维档）</summary>
    ...
    public async Task<byte[]> GetTemplateAsync(string kind)
    {
