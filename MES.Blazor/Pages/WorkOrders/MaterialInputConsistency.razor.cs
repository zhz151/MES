...
public partial class MaterialInputConsistency
{
    ...
    // ========== 错疑-用料投料不一致卡片（工单执行状况读模型：原料锁定 + 到料实投一致性 2错误+2疑问 明细） ==========
    ...
    // ========== 错误-用料计划及其执行卡片（工单执行状况读模型：主号-关注=主号完成/生产执行/成品检验 三档 × 理论原料未至/工单到料未投 的工单数+累计重量） ==========
    ...
    private const string ColumnPrefsKey = "materialInputConsistency_v4";
    ...
    private static List<ColumnDef> GetAllColumnDefs() => new()
    {
        ...
        new() { Key = "EndCustomer",        Label = "最终用户",   SortKey = "EndCustomer", FilterType = "string", Width = "120", GroupKey = 1, GroupName = "基础数据", Visible = false },
        ...
        new() { Key = "MaxLength",          Label = "最大长度",   SortKey = "MaxLength", Width = "80", GroupKey = 1, GroupName = "基础数据", Visible = false },
        ...
        // ========== 3 用料及投料（2026-09-08 由实时关注拆出；除「投料状态」外均不支持排序/筛选） ==========
        new() { Key = "PlanProportion",       Label = "分类用料", Width = "150",
            GroupKey = 3, GroupName = "用料及投料" },
        new() { Key = "CategoryArrivalText",  Label = "分类到料", Width = "170",
            GroupKey = 3, GroupName = "用料及投料" },
        new() { Key = "TotalMissingWeight",   Label = "原料未至", Width = "90",
            GroupKey = 3, GroupName = "用料及投料" },
        new() { Key = "PendingInputWeight",   Label = "到料未投", Width = "80",
            GroupKey = 3, GroupName = "用料及投料" },
        new() { Key = "InputWeight",          Label = "生产投料量", Width = "80",
            GroupKey = 3, GroupName = "用料及投料" },
        new() { Key = "InputOutputRatio",     Label = "投料比", Width = "80",
            GroupKey = 3, GroupName = "用料及投料" },
        new() { Key = "InputStatus",          Label = "投料状态", SortKey = "InputStatus", FilterType = "enum", Width = "120",
            EnumOptions = DisplayHelper.GetFlowStatusOptions(), GroupKey = 3, GroupName = "用料及投料" },
    };
    ...
    {
        "TotalItemCount" or "MinLength" or "MaxLength" or "TotalQuantity" or "TotalWeight" or
        "InputWeight" or "InputOutputRatio" => "text-center",
        _ => ""
    };
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var savedState = await PageState.LoadAsync("materialInputConsistency");
        if (savedState != null)
        {
            ...
            if (savedState.Extras?.ContainsKey("columnFilters") == true)
            {
                try
                {
                    ...
                    if (dict != null)
                        // 仅恢复仍支持筛选的列（2026-09-08 起「用料及投料」除投料状态外不再提供筛选 UI，旧残留筛选丢弃防无法清除）
                        _columnFilters = dict
                            .Where(kv => _allColumns.Any(c => c.Key == kv.Key && c.FilterType != null))
                            .ToDictionary(kv => kv.Key, kv => new HashSet<string>(kv.Value));
                }
                catch { }
            }
            // 恢复卡片点击联动筛选（本页仅「错误-用料计划及其执行」卡片 ScheduleStage 联动；恢复时重建提示条文案）
            if (savedState.Extras?.ContainsKey("linkFilter") == true)
            {
                try
                {
                    ...
                    if (_linkFilter?.ScheduleStage.HasValue == true)
                    {
                        var stageText = IntStatusDisplayHelper.GetScheduleStageText(_linkFilter.ScheduleStage);
                        var fieldText = _linkFilter.HasMissingWeight == true ? "原料未至" : "到料未投";
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (!_isArrowNavSetup)
        {
            ...
            if (!await JS.InvokeAsync<bool>("enableTableArrowNav", "#material-input-consistency-list-table"))
                _isArrowNavSetup = false;
        }

        // 分组标题栏：测量实际列宽 + 同步滚动
        await JS.InvokeVoidAsync("initGroupHeaders", "#material-input-consistency-list-table");
    }
    ...
    // ========== 错误-用料计划及其执行卡片（工单执行状况读模型：主号-关注=主号完成/生产执行/成品检验 三档 × 理论原料未至/工单到料未投） ==========
    ...
    private async Task LoadInProdInspectionAsync()
    {
        ...
        try
        {
            ...
            if (result.Success && result.Data != null)
                ...
                Snackbar.Add(result?.Message ?? "获取错误-用料计划及其执行失败", Severity.Error);
        }
        catch (Exception ex)
        {
            Snackbar.Add($"获取错误-用料计划及其执行失败: {ex.Message}", Severity.Error);
        }
        finally
        {
    ...
    /// <summary>打印「错疑-用料投料不一致」卡片明细：抓取隐藏完整表（绑定排序+筛选后全部行，不受显示行数截断影响）</summary>
    private async Task PrintErrorDoubtTable()
    {
        try
        {
            ...
            {
                Snackbar.Add("未找到可打印的用料投料不一致表格", Severity.Warning);
                return;
            }
            await JS.InvokeVoidAsync("printRawHtml", table, "用料投料核查-错疑-用料投料不一致");
        }
        catch (Exception ex)
        {
    ...
    /// <summary>「错误-用料计划及其执行」卡片单元格点击：按 主号-关注档位 + 字段&gt;0 联动筛选下方工单列表（后端 linkFilter 生效）。
    ...
    private async Task ApplyStageDoubtLink(int scheduleStage, bool hasMissing, bool hasPending)
    {
        ...
        var fieldText = hasMissing ? "原料未至" : "到料未投";
        _linkLabel = $"{stageText}·{fieldText}";
    ...
    private string? GetCellRawValue(WorkOrderListDto item, string key) => key switch
    {
        ...
        "CategoryArrivalText" => string.IsNullOrEmpty(item.CategoryArrivalText) ? "-" : item.CategoryArrivalText,
    ...
    private RenderFragment RenderCell(WorkOrderListDto wo, ColumnDef col) => builder =>
    {
        switch (col.Key)
        {
            ...
            case "InputStatus":
                ...
                {
                    // 3=超量：深色底白字 Chip 与「满足」(绿色) 区分（超量不等于满足，需视觉警示）
                    var isExcess = wo.InputStatus.Value == 3;
                    builder.OpenComponent<MudChip>(0);
                    builder.AddAttribute(1, "Size", Size.Small);
                    builder.AddAttribute(2, "Color", isExcess ? Color.Default : DisplayHelper.GetInputStatusColor(wo.InputStatus.Value));
                    builder.AddAttribute(3, "Class", isExcess ? "chip-dark" : "");
                    builder.AddAttribute(4, "ChildContent", (RenderFragment)(b2 => b2.AddContent(0, IntStatusDisplayHelper.GetInputStatusText(wo.InputStatus.Value))));
                    builder.CloseComponent();
                }
                else
                {
            ...
            case "PlanProportion":
                ...
                break;
            case "CategoryArrivalText":
                if (!string.IsNullOrEmpty(wo.CategoryArrivalText))
                {
                    builder.OpenComponent<MudText>(0);
                    builder.AddAttribute(1, "Typo", Typo.caption);
                    builder.AddAttribute(2, "Class", "text-wrap");
                    builder.AddAttribute(3, "Style", "max-width:190px;");
                    builder.AddAttribute(4, "ChildContent", (RenderFragment)(b2 => b2.AddContent(0, wo.CategoryArrivalText)));
                    builder.CloseComponent();
                }
                else
                {
    ...
    private async Task SavePageStateAsync()
    {
        ...
        await PageState.SaveAsync("materialInputConsistency", state);
    }
}
