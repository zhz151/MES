...
public partial class FactoryInspectionRequirements
{
    private MudTable<FactoryInspectionRequirementDto>? table;
    private List<FactoryInspectionRequirementDto> _pageItems = new();
    ...
    private static List<ColumnDef> GetAllColumnDefs() => new()
    {
        ...
        new() { Key = "PmiInspection",          Label = "PMI检验",             SortKey = "pmiinspection",         FilterType = "string" },
        new() { Key = "SurfaceInspection",      Label = "表检",                SortKey = "surfaceinspection",     FilterType = "string" },
        new() { Key = "Dimension",              Label = "尺寸",                SortKey = "dimension",             FilterType = "string" },
        new() { Key = "Endoscopy",              Label = "内窥",                SortKey = "endoscopy",             FilterType = "string" },
        new() { Key = "HydrostaticTest",        Label = "液压检验",             SortKey = "hydrostatictest",       FilterType = "string" },
        new() { Key = "UnderwaterPressure",     Label = "水下气压",             SortKey = "underwaterpressure",    FilterType = "string" },
        ...
        new() { Key = "PortColoring",           Label = "端口着色",             SortKey = "portcoloring",          FilterType = "string" },
    ...
    private async Task<TableData<FactoryInspectionRequirementDto>> LoadDataFromServer(TableState state)
    {
        ...
        try
        {
            ...
            var result = await FactoryInspectionRequirementService.GetPagedAsync(query);

            // 竞态保护：丢弃过期请求结果（搜索/筛选并发时旧请求晚返回不得覆盖新结果）
            if (version != _loadVersion)
                return new TableData<FactoryInspectionRequirementDto> { Items = _pageItems, TotalItems = _totalCount };
        ...
        return new TableData<FactoryInspectionRequirementDto>
        {
            Items = _pageItems,
    ...
    private async Task LoadFilterContextsAsync()
    {
        try
        {
            var result = await FactoryInspectionRequirementService.GetFilterContextsAsync();
            if (result.Success && result.Data != null)
            {
    ...
    {
        await ColumnPrefs.SaveAsync("factory_inspection_requirements", null, _allColumns);
    }
    ...
    {
        _allColumns = GetAllColumnDefs();
        var saved = await ColumnPrefs.LoadAsync("factory_inspection_requirements", null);
        if (saved.Count > 0)
        {
        ...
        var savedState = await PageState.LoadAsync("factory_inspection_requirements");
        if (savedState != null)
        {
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (!_isArrowNavSetup)
        {
            ...
            if (!await JS.InvokeAsync<bool>("enableTableArrowNav", "#factory-inspection-requirements-list-table"))
    ...
    private void StartEdit(FactoryInspectionRequirementDto item)
    {
    ...
    private void CancelEdit(FactoryInspectionRequirementDto item)
    {
        _editingIds.Remove(item.Id);
        _editCache.Remove(item.Id);
    }

    private async Task SaveEdit(FactoryInspectionRequirementDto item)
    {
        ...
        try
        {
            var request = new UpdateFactoryInspectionRequirementRequest
            {
                StandardNo = cache.StandardNo,
                ChemicalComposition = cache.ChemicalComposition,
                PmiInspection = cache.PmiInspection,
                SurfaceInspection = cache.SurfaceInspection,
                Dimension = cache.Dimension,
                Endoscopy = cache.Endoscopy,
                HydrostaticTest = cache.HydrostaticTest,
                UnderwaterPressure = cache.UnderwaterPressure,
                EddyCurrent = cache.EddyCurrent,
                UltrasonicTest = cache.UltrasonicTest,
                PortColoring = cache.PortColoring,
            ...
            var result = await FactoryInspectionRequirementService.UpdateAsync(item.Id, request);
            if (result.Success)
            {
                item.StandardNo = cache.StandardNo;
                item.ChemicalComposition = cache.ChemicalComposition;
                item.PmiInspection = cache.PmiInspection;
                item.SurfaceInspection = cache.SurfaceInspection;
                item.Dimension = cache.Dimension;
                item.Endoscopy = cache.Endoscopy;
                item.HydrostaticTest = cache.HydrostaticTest;
                item.UnderwaterPressure = cache.UnderwaterPressure;
                item.EddyCurrent = cache.EddyCurrent;
                item.UltrasonicTest = cache.UltrasonicTest;
                item.PortColoring = cache.PortColoring;
    ...
    {
        Navigation.NavigateTo("/factory-inspection-requirements/create");
    }

    private async Task DeleteItem(FactoryInspectionRequirementDto item)
    {
        var dialog = DialogService.Show<ConfirmDialog>("确认", new DialogParameters
        {
            ["ContentText"] = $"确定要删除标准号 \"{item.StandardNo}\" 的工厂检验项要求记录吗？",
            ["ConfirmText"] = "确认删除",
            ["Color"] = Color.Error
        });
        ...
        if (!dialogResult.Canceled)
        {
            try
            {
                var result = await FactoryInspectionRequirementService.DeleteAsync(item.Id);
                if (result.Success)
                {
    ...
    private RenderFragment RenderCell(FactoryInspectionRequirementDto item, ColumnDef col) => builder =>
    {
    ...
    private RenderFragment RenderTextField(FactoryInspectionRequirementDto item, string key, EditCache? cache) => builder =>
    {
    ...
    private static string? GetFieldValue(FactoryInspectionRequirementDto item, string key) => key switch
    {
        "ChemicalComposition" => item.ChemicalComposition,
        "PmiInspection" => item.PmiInspection,
        "SurfaceInspection" => item.SurfaceInspection,
    ...
    private static void SetCacheFieldValue(EditCache cache, string key, string? value)
    {
        switch (key)
        {
            ...
            case "PmiInspection": cache.PmiInspection = value; break;
            case "SurfaceInspection": cache.SurfaceInspection = value; break;
            case "Dimension": cache.Dimension = value; break;
            case "Endoscopy": cache.Endoscopy = value; break;
            case "HydrostaticTest": cache.HydrostaticTest = value; break;
            case "UnderwaterPressure": cache.UnderwaterPressure = value; break;
            case "EddyCurrent": cache.EddyCurrent = value; break;
            case "UltrasonicTest": cache.UltrasonicTest = value; break;
            case "PortColoring": cache.PortColoring = value; break;
    ...
    private async Task SavePageStateAsync()
    {
        ...
        await PageState.SaveAsync("factory_inspection_requirements", state);
    }
}
