...
/// <summary>
/// 成检计件类别导入弹窗（2026-09-03）。kind=category 类别定义 / tier 维档系数。
/// 流程：下载模板 → 选文件 → 解析预览（新增/覆盖/错误）→ 执行导入（整体成功才提交）。
/// </summary>
public partial class PieceRateFinalInspectionCategoryImportDialog
{
    /// <summary>导入类型：category|tier（父级传入）</summary>
    [Parameter] public string Kind { get; set; } = "category";

    [Inject] private PieceRateFinalInspectionCategoryImportService Import { get; set; } = null!;
    ...
    private string _kindLabel => _isTier ? "成检维档系数" : "成检类别定义";
    ...
    private async Task DownloadTemplateAsync()
    {
        try
        {
            var bytes = await Import.GenerateTemplateAsync(Kind);
            var fileName = _isTier ? "成检维档模板.xlsx" : "成检类别模板.xlsx";
