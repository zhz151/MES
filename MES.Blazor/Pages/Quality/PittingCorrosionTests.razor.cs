...
public partial class PittingCorrosionTests
{
    ...
    private async Task SaveColumnPrefs() => await ColumnPrefs.SaveAsync("pitting-corrosion-test", ColumnPrefsVersion, _allColumns);
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var saved = await ColumnPrefs.LoadAsync("pitting-corrosion-test", ColumnPrefsVersion);
        if (saved.Count > 0)
        {
    ...
    private async Task DeleteItem(PittingCorrosionTestDto item)
    {
        ...
        {
            ["ContentText"] = $"确定要删除生产编号 \"{item.BatchNo}\" 的点腐蚀检验记录吗？\n\n删除后数据将不可恢复！",
            ["ConfirmText"] = "确认删除",
            ["Color"] = Color.Error
        });
    ...
    private async Task PrintSelected()
    {
        ...
        var apiUrl = $"{Http.BaseAddress}{ApiEndpoints.PittingCorrosionTest}/print-batch-file";
