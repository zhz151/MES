...
public partial class HardnessTests
{
    private MudTable<HardnessTestDto>? table;
    private List<HardnessTestDto> _pageItems = new();
    ...
    private static List<ColumnDef> GetAllColumnDefs() => new()
    {
        ...
        new() { Key = "HardnessMode",      Label = "硬度模式",   SortKey = "hardnessmode", FilterType = "string", Width = "100" },
        new() { Key = "HardnessValue",     Label = "硬度测定值", SortKey = "hardnessvalue", FilterType = "string", Width = "100" },
    ...
    private async Task<TableData<HardnessTestDto>> LoadDataFromServer(TableState state)
    {
        ...
        try
        {
            ...
            var result = await HardnessTestService.GetAllAsync(
        ...
        return new TableData<HardnessTestDto> { Items = _pageItems, TotalItems = _totalCount };
    }
    ...
    private async Task LoadFilterContextsAsync()
    {
        try
        {
            var result = await HardnessTestService.GetFilterContextsAsync();
            if (result.Success && result.Data != null)
            {
    ...
    private async Task SaveColumnPrefs() => await ColumnPrefs.SaveAsync("hardness-test", null, _allColumns);
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var saved = await ColumnPrefs.LoadAsync("hardness-test", null);
        if (saved.Count > 0)
        {
        ...
        var ss = await PageState.LoadAsync("hardness-test");
        if (ss != null)
        {
    ...
    private void StartEdit(HardnessTestDto item)
    {
        ...
        _editCache[item.Id] = new EditCache
        {
            ...
            InspectionStandard = item.InspectionStandard, HardnessMode = item.HardnessMode,
            HardnessValue = item.HardnessValue, Judgment = item.Judgment
        };
    }

    private void CancelEdit(HardnessTestDto item)
    { _editingIds.Remove(item.Id); _editCache.Remove(item.Id); }

    private async Task SaveEdit(HardnessTestDto item)
    {
        ...
        try
        {
            var result = await HardnessTestService.UpdateAsync(item.Id, new UpdateHardnessTestRequest
            {
                ...
                HardnessMode = cache.HardnessMode, HardnessValue = cache.HardnessValue, Judgment = cache.Judgment
            });
            if (result.Success && result.Data != null)
            {
    ...
    private RenderFragment RenderCell(HardnessTestDto item, ColumnDef col) => builder =>
    {
        ...
        switch (col.Key)
        {
            ...
            case "HardnessMode": RenderStr(builder, isEditing, cache?.HardnessMode, v => { if (cache != null) cache.HardnessMode = v; }, item.HardnessMode); break;
            case "HardnessValue": RenderStr(builder, isEditing, cache?.HardnessValue, v => { if (cache != null) cache.HardnessValue = v; }, item.HardnessValue); break;
    ...
    private async Task SavePageStateAsync()
    {
        ...
        await PageState.SaveAsync("hardness-test", new PageState
        {
    ...
    private void NavigateToCreate() => Navigation.NavigateTo("/quality/hardness-test/create");

    private async Task DeleteItem(HardnessTestDto item)
    {
    ...
    private async Task PrintSelected()
    {
        ...
        var apiUrl = $"{Http.BaseAddress}api/hardness-test/print-batch-file";
        var request = new HardnessTestPrintBatchRequest { Ids = selectedIds.ToArray(), Columns = GetPrintColumnDefs() };
    ...
    {
        var apiUrl = $"{Http.BaseAddress}api/hardness-test/print-all-file";
