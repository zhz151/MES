...
public partial class HardnessTests
{
    ...
    // 列偏好版本号：默认列显隐收敛后 +1，强制旧持久化失效（key=col_prefs_hardness-test_v1）
    ...
    private async Task SaveColumnPrefs() => await ColumnPrefs.SaveAsync("hardness-test", ColumnPrefsVersion, _allColumns);
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var saved = await ColumnPrefs.LoadAsync("hardness-test", ColumnPrefsVersion);
        if (saved.Count > 0)
        {
    ...
    private void StartEdit(HardnessTestDto item)
    {
        ...
        _editCache[item.Id] = new EditCache
        {
            ...
            HardnessMode = item.HardnessMode,
            HardnessValue = item.HardnessValue,
    ...
    private async Task SaveEdit(HardnessTestDto item)
    {
        ...
        try
        {
            var result = await HardnessTestService.UpdateAsync(item.Id, new UpdateHardnessTestRequest
            {
                ...
                HardnessMode = cache.HardnessMode,
                HardnessValue = cache.HardnessValue,
    ...
    private async Task PrintSelected()
    {
        ...
        var apiUrl = $"{Http.BaseAddress}{ApiEndpoints.HardnessTest}/print-batch-file";
