...
public partial class GrainSizeTests
{
    private MudTable<GrainSizeTestDto>? table;
    private List<GrainSizeTestDto> _pageItems = new();
    ...
    private static List<ColumnDef> GetAllColumnDefs() => new()
    {
        ...
        new() { Key = "GrainSizeGrade",    Label = "晶粒度级别", SortKey = "grainsizegrade", FilterType = "string", Width = "100" },
    ...
    private async Task<TableData<GrainSizeTestDto>> LoadDataFromServer(TableState state)
    {
        ...
        try
        {
            ...
            var result = await GrainSizeTestService.GetAllAsync(
        ...
        return new TableData<GrainSizeTestDto> { Items = _pageItems, TotalItems = _totalCount };
    }
    ...
    private async Task LoadFilterContextsAsync()
    {
        try
        {
            var result = await GrainSizeTestService.GetFilterContextsAsync();
            if (result.Success && result.Data != null)
            {
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var ss = await PageState.LoadAsync("grain-size-test");
        if (ss != null)
        {
    ...
    private void StartEdit(GrainSizeTestDto item)
    {
        ...
        _editCache[item.Id] = new EditCache
        {
            ...
            InspectionStandard = item.InspectionStandard, GrainSizeGrade = item.GrainSizeGrade,
            GrainSizeMethod = item.GrainSizeMethod, Magnification = item.Magnification, Judgment = item.Judgment
        };
    }

    private void CancelEdit(GrainSizeTestDto item)
    { _editingIds.Remove(item.Id); _editCache.Remove(item.Id); }

    private async Task SaveEdit(GrainSizeTestDto item)
    {
        ...
        try
        {
            var result = await GrainSizeTestService.UpdateAsync(item.Id, new UpdateGrainSizeTestRequest
            {
                ...
                GrainSizeGrade = cache.GrainSizeGrade, GrainSizeMethod = cache.GrainSizeMethod,
                Magnification = cache.Magnification, Judgment = cache.Judgment
            });
            if (result.Success && result.Data != null)
            {
                ...
                item.GrainSizeGrade = result.Data.GrainSizeGrade; item.GrainSizeMethod = result.Data.GrainSizeMethod;
                item.Magnification = result.Data.Magnification; item.Judgment = result.Data.Judgment;
    ...
    private bool IsCellEditable(string key) => key switch
    {
        "InspectionDate" or "Inspector" or "FurnaceNo" or "Grade" or "Specification"
            ...
            or "GrainSizeGrade" or "GrainSizeMethod" or "Magnification" or "Judgment" => true,
        _ => false
    };

    private RenderFragment RenderCell(GrainSizeTestDto item, ColumnDef col) => builder =>
    {
        ...
        switch (col.Key)
        {
            ...
            case "GrainSizeGrade": RenderStr(builder, isEditing, cache?.GrainSizeGrade, v => { if (cache != null) cache.GrainSizeGrade = v; }, item.GrainSizeGrade); break;
            case "GrainSizeMethod": RenderStr(builder, isEditing, cache?.GrainSizeMethod, v => { if (cache != null) cache.GrainSizeMethod = v; }, item.GrainSizeMethod); break;
    ...
    private async Task SavePageStateAsync()
    {
        ...
        await PageState.SaveAsync("grain-size-test", new PageState
        {
    ...
    private void NavigateToCreate() => Navigation.NavigateTo("/quality/grain-size-test/create");

    private async Task DeleteItem(GrainSizeTestDto item)
    {
        var dialog = DialogService.Show<ConfirmDialog>("确认", new DialogParameters
        {
            ["ContentText"] = $"确定要删除炉批号 \"{item.FurnaceNo}\" 的晶粒度检验记录吗？\n\n删除后数据将不可恢复！",
        ...
        if (!dr.Canceled)
        {
            try
            {
                var result = await GrainSizeTestService.DeleteAsync(item.Id);
