...
public partial class FlaringTests
{
    ...
    private async Task SaveColumnPrefs() => await ColumnPrefs.SaveAsync("flaring-test", ColumnPrefsVersion, _allColumns);
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var saved = await ColumnPrefs.LoadAsync("flaring-test", ColumnPrefsVersion);
        if (saved.Count > 0)
        {
    ...
    private async Task DeleteItem(FlaringTestDto item)
    {
        ...
        {
            ["ContentText"] = $"确定要删除生产编号 \"{item.BatchNo}\" 的扩口检验记录吗？\n\n删除后数据将不可恢复！",
            ["ConfirmText"] = "确认删除",
            ["Color"] = Color.Error
        });
