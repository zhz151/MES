...
public partial class TensileTests
{
    ...
    // ========== 数值列（试样编号 + 标距/强度/延伸率，数据格居中） ==========
    private static readonly HashSet<string> _centerColumnKeys = new(StringComparer.Ordinal)
    { "SampleNo", "OriginalGaugeLength", "FinalGaugeLength", "TensileStrength", "YieldStrengthRp02", "YieldStrengthRp1", "Elongation" };
    private static bool IsNumericColumn(ColumnDef col) => _centerColumnKeys.Contains(col.Key);
    // 列偏好版本号：默认列显隐收敛后 +1，强制旧持久化失效（key=col_prefs_tensile-test_v1）
    ...
    private async Task SaveColumnPrefs() => await ColumnPrefs.SaveAsync("tensile-test", ColumnPrefsVersion, _allColumns);
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var saved = await ColumnPrefs.LoadAsync("tensile-test", ColumnPrefsVersion);
        if (saved.Count > 0)
        {
    ...
    private async Task DeleteItem(TensileTestDto item)
    {
        ...
        {
            ["ContentText"] = $"确定要删除生产编号 \"{item.BatchNo}\" 的室温拉伸检验记录吗？\n\n删除后数据将不可恢复！",
            ["ConfirmText"] = "确认删除",
            ["Color"] = Color.Error
        });
