...
public partial class FlatteningTests
{
    private MudTable<FlatteningTestDto>? table;
    private List<FlatteningTestDto> _pageItems = new();
    ...
    private async Task<TableData<FlatteningTestDto>> LoadDataFromServer(TableState state)
    {
        ...
        try
        {
            ...
            var result = await FlatteningTestService.GetAllAsync(
        ...
        return new TableData<FlatteningTestDto> { Items = _pageItems, TotalItems = _totalCount };
    }
    ...
    private async Task LoadFilterContextsAsync()
    {
        try
        {
            var result = await FlatteningTestService.GetFilterContextsAsync();
            if (result.Success && result.Data != null)
            {
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var ss = await PageState.LoadAsync("flattening-test");
        if (ss != null)
        {
    ...
    private void StartEdit(FlatteningTestDto item)
    {
        ...
        _editCache[item.Id] = new EditCache
        {
            ...
            InspectionStandard = item.InspectionStandard, FlatteningGap = item.FlatteningGap,
            Observation = item.Observation, Judgment = item.Judgment
        };
    }

    private void CancelEdit(FlatteningTestDto item)
    { _editingIds.Remove(item.Id); _editCache.Remove(item.Id); }

    private async Task SaveEdit(FlatteningTestDto item)
    {
        ...
        try
        {
            var result = await FlatteningTestService.UpdateAsync(item.Id, new UpdateFlatteningTestRequest
            {
                ...
                FlatteningGap = cache.FlatteningGap, Observation = cache.Observation, Judgment = cache.Judgment
            });
            if (result.Success && result.Data != null)
            {
                ...
                item.FlatteningGap = result.Data.FlatteningGap; item.Observation = result.Data.Observation;
    ...
    private bool IsCellEditable(string key) => key switch
    {
        "InspectionDate" or "Inspector" or "FurnaceNo" or "Grade" or "Specification"
            ...
            or "FlatteningGap" or "Observation" or "Judgment" => true,
        _ => false
    };

    private RenderFragment RenderCell(FlatteningTestDto item, ColumnDef col) => builder =>
    {
        ...
        switch (col.Key)
        {
            ...
            case "FlatteningGap": RenderDecimal(builder, isEditing, cache?.FlatteningGap, v => { if (cache != null) cache.FlatteningGap = v; }, item.FlatteningGap); break;
    ...
    private async Task SavePageStateAsync()
    {
        ...
        await PageState.SaveAsync("flattening-test", new PageState
        {
    ...
    private void NavigateToCreate() => Navigation.NavigateTo("/quality/flattening-test/create");

    private async Task DeleteItem(FlatteningTestDto item)
    {
        var dialog = DialogService.Show<ConfirmDialog>("确认", new DialogParameters
        {
            ["ContentText"] = $"确定要删除生产编号 \"{item.FurnaceNo}\" 的压扁检验记录吗？\n\n删除后数据将不可恢复！",
        ...
        if (!dr.Canceled)
        {
            try
            {
                var result = await FlatteningTestService.DeleteAsync(item.Id);
