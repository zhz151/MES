...
public partial class IntergranularCorrosionTests
{
    ...
    // 列偏好版本号：默认列显隐收敛后 +1，强制旧持久化失效（key=col_prefs_intergranular-corrosion-test_v1）
    ...
    private async Task SaveColumnPrefs() => await ColumnPrefs.SaveAsync("intergranular-corrosion-test", ColumnPrefsVersion, _allColumns);
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var saved = await ColumnPrefs.LoadAsync("intergranular-corrosion-test", ColumnPrefsVersion);
        if (saved.Count > 0)
        {
