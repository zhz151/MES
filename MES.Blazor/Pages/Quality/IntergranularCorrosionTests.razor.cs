...
public partial class IntergranularCorrosionTests
{
    private MudTable<IntergranularCorrosionTestDto>? table;
    private List<IntergranularCorrosionTestDto> _pageItems = new();
    ...
    private static List<ColumnDef> GetAllColumnDefs() => new()
    {
        ...
        new() { Key = "SampleSize",               Label = "试样尺寸",         SortKey = "samplesize", FilterType = "string", Width = "120" },
        new() { Key = "InspectionStandard",       Label = "检验标准",         SortKey = "inspectionstandard", FilterType = "string", Width = "120" },
        new() { Key = "SensitizationTemperature", Label = "试样敏化温度",     SortKey = "sensitizationtemperature", FilterType = "string", Width = "120" },
        new() { Key = "SensitizationDuration",    Label = "敏化持续时间",     SortKey = "sensitizationduration", FilterType = "string", Width = "120" },
        ...
        new() { Key = "BendDegree",               Label = "试样弯曲度数",     SortKey = "benddegree", FilterType = "string", Width = "110" },
        new() { Key = "Magnification",            Label = "观察放大倍数",     SortKey = "magnification", FilterType = "string", Width = "110" },
        new() { Key = "ObservationResult",        Label = "观察结果",         SortKey = "observationresult", FilterType = "string", Width = "140" },
    ...
    private async Task<TableData<IntergranularCorrosionTestDto>> LoadDataFromServer(TableState state)
    {
        ...
        try
        {
            ...
            var result = await IntergranularCorrosionTestService.GetAllAsync(
            ...
            if (version != _loadVersion)
                return new TableData<IntergranularCorrosionTestDto> { Items = _pageItems, TotalItems = _totalCount };
    ...
    private async Task LoadFilterContextsAsync()
    {
        try
        {
            var result = await IntergranularCorrosionTestService.GetFilterContextsAsync();
            if (result.Success && result.Data != null)
            {
    ...
    private async Task SaveColumnPrefs() => await ColumnPrefs.SaveAsync("intergranular-corrosion-test", null, _allColumns);
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var saved = await ColumnPrefs.LoadAsync("intergranular-corrosion-test", null);
        if (saved.Count > 0)
        {
        ...
        var ss = await PageState.LoadAsync("intergranular-corrosion-test");
        if (ss != null)
        {
    ...
    private void StartEdit(IntergranularCorrosionTestDto item)
    {
    ...
    private void CancelEdit(IntergranularCorrosionTestDto item)
    { _editingIds.Remove(item.Id); _editCache.Remove(item.Id); }

    private async Task SaveEdit(IntergranularCorrosionTestDto item)
    {
        ...
        try
        {
            ...
            if (result.Success && result.Data != null)
            {
                ...
                item.SensitizationDuration = result.Data.SensitizationDuration;
                item.CorrosionSolution = result.Data.CorrosionSolution; item.CorrosionTime = result.Data.CorrosionTime;
                item.BendDegree = result.Data.BendDegree; item.Magnification = result.Data.Magnification;
    ...
    private bool IsCellEditable(string key) => key switch
    {
        "InspectionDate" or "Inspector" or "FurnaceNo" or "Grade" or "Specification"
            ...
            or "SensitizationTemperature" or "SensitizationDuration" or "CorrosionSolution"
            or "CorrosionTime" or "BendDegree" or "Magnification"
            or "ObservationResult" or "Judgment" => true,
        _ => false
    };

    private RenderFragment RenderCell(IntergranularCorrosionTestDto item, ColumnDef col) => builder =>
    {
        ...
        switch (col.Key)
        {
            ...
            case "SensitizationTemperature": RenderStr(builder, isEditing, cache?.SensitizationTemperature, v => { if (cache != null) cache.SensitizationTemperature = v; }, item.SensitizationTemperature); break;
            case "SensitizationDuration": RenderStr(builder, isEditing, cache?.SensitizationDuration, v => { if (cache != null) cache.SensitizationDuration = v; }, item.SensitizationDuration); break;
            ...
            case "BendDegree": RenderStr(builder, isEditing, cache?.BendDegree, v => { if (cache != null) cache.BendDegree = v; }, item.BendDegree); break;
            ...
            case "ObservationResult": RenderStr(builder, isEditing, cache?.ObservationResult, v => { if (cache != null) cache.ObservationResult = v; }, item.ObservationResult); break;
    ...
    private void NavigateToCreate() => Navigation.NavigateTo("/quality/intergranular-corrosion-test/create");

    private async Task DeleteItem(IntergranularCorrosionTestDto item)
    {
        ...
        if (!dr.Canceled)
        {
            try
            {
                var result = await IntergranularCorrosionTestService.DeleteAsync(item.Id);
    ...
    private async Task PrintSelected()
    {
        ...
        var apiUrl = $"{Http.BaseAddress}{ApiEndpoints.IntergranularCorrosionTest}/print-batch-file";
        var request = new IntergranularCorrosionTestPrintBatchRequest { Ids = selectedIds.ToArray(), Columns = GetPrintColumnDefs() };
    ...
    {
        var apiUrl = $"{Http.BaseAddress}{ApiEndpoints.IntergranularCorrosionTest}/print-all-file";
        var request = new IntergranularCorrosionTestPrintAllRequest
        {
            Keyword = string.IsNullOrWhiteSpace(_searchKeyword) ? null : _searchKeyword,
