...
public class GradeMappingServiceTests : TestBase
{
        ...
        string plantGrade = "Q345B", decimal density = 7.85m, bool specialMaterial = false)
    {
        ctx.StandardGradeMappings.Add(new StandardGradeMapping
        {
            ...
            SteelProperty = GradeMappingService.ComputeSteelProperty(plantGrade)
        });
    ...
    public async Task UpdateAsync_成功更新牌号对照()
    {
        ...
        result.SteelProperty.Should().Be("镍基合金");
        ...
        saved.SteelProperty.Should().Be("镍基合金");
    }
    ...
    [InlineData("321H", "奥氏体")]
    [InlineData("904L", "奥氏体")]
    [InlineData("9Cr18", "奥氏体")]
    [InlineData("2205", "双相钢")]
    [InlineData("2507", "双相钢")]
    [InlineData("2Cr13", "双相钢")]
    [InlineData("06Cr19Ni10", "镍基合金")]
    [InlineData("022Cr17Ni12Mo2", "镍基合金")]
    [InlineData("12Cr17Mn6Ni5N", "镍基合金")]
    [InlineData("", "镍基合金")]
    [InlineData(null, "镍基合金")]
    public void ComputeSteelProperty_工厂牌号首字_返回正确钢性(string? plantGrade, string expected)
    {
        var result = GradeMappingService.ComputeSteelProperty(plantGrade!);
        result.Should().Be(expected);
    }

    [Fact]
    public async Task CreateAsync_工厂牌号首字3_钢性为奥氏体()
    {
        var ctx = CreateDbContext();
        var svc = CreateService(ctx);
        ...
        var result = await svc.CreateAsync(new CreateGradeMappingRequest
        {
            StandardGrade = "TEST-3",
            PlantGrade = "3xx",
            Density = 7.85m
        });

        result.SteelProperty.Should().Be("奥氏体");
    }

    [Fact]
    public async Task UpdateAsync_工厂牌号变更_钢性重新计算()
    {
        var ctx = CreateDbContext();
        await SeedMappingAsync(ctx, standardGrade: "TEST-UPDATE", plantGrade: "Q345B");
        var id = await ctx.StandardGradeMappings.Select(g => g.Id).FirstAsync();
        var svc = CreateService(ctx);

        var result = await svc.UpdateAsync(id, new UpdateGradeMappingRequest
        {
            StandardGrade = "TEST-UPDATE",
            PlantGrade = "304",
            Density = 7.85m
        });
    ...
    public async Task GetFilterContextsAsync_返回正确选项()
    {
        ...
        contexts.Should().ContainKey("SteelProperty");
        contexts["SteelProperty"].Should().Contain(new[] { "双相钢", "镍基合金" });
    }
    ...
    public async Task GetFilterContextsAsync_无数据_返回空列表()
    {
        ...
        contexts["SteelProperty"].Should().BeEmpty();
    }
