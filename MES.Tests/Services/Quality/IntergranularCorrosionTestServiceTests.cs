...
/// <summary>
/// 晶间腐蚀检验服务测试：CRUD、关键字/日期范围过滤、null 补丁更新、缺失抛业务异常、批量创建、缓存筛选上下文去重。
/// </summary>
public class IntergranularCorrosionTestServiceTests : TestBase
{
    private static IntergranularCorrosionTestService CreateService(AppDbContext ctx)
        => new(ctx, NullLogger<IntergranularCorrosionTestService>.Instance, new MemoryCache(new MemoryCacheOptions()));

    private static async Task<IntergranularCorrosionTest> SeedAsync(AppDbContext ctx, DateTime date,
        string furnaceNo = "FUR-1", string inspector = "张三", string grade = "Q345B", string specification = "219*8",
        string? sensitizationTemperature = null, string? corrosionSolution = null)
    {
        var e = new IntergranularCorrosionTest
        {
            InspectionDate = date,
            Inspector = inspector,
            FurnaceNo = furnaceNo,
            ...
            SensitizationTemperature = sensitizationTemperature,
            CorrosionSolution = corrosionSolution
        };
        ctx.IntergranularCorrosionTests.Add(e);
        await ctx.SaveChangesAsync();
        return e;
    }

    private static CreateIntergranularCorrosionTestRequest NewCreate(string furnaceNo = "FUR-1") => new()
    {
        ...
        Grade = "Q345B",
        Specification = "219*8",
        SensitizationTemperature = "650°C",
        CorrosionSolution = "硫酸-硫酸铜"
    };
    ...
    public async Task CreateAsync_新增_落库返回Dto()
    {
        ...
        dto.SensitizationTemperature.Should().Be("650°C");
        dto.CorrosionSolution.Should().Be("硫酸-硫酸铜");
        var row = await ctx.IntergranularCorrosionTests.SingleAsync();
    ...
    [Fact]
    public async Task GetAllAsync_关键字命中浸蚀溶液列()
    {
        var ctx = CreateDbContext();
        await SeedAsync(ctx, new DateTime(2026, 3, 1), furnaceNo: "FUR-1");
        await SeedAsync(ctx, new DateTime(2026, 3, 2), furnaceNo: "FUR-2");
        await SeedAsync(ctx, new DateTime(2026, 3, 3), furnaceNo: "FUR-3", corrosionSolution: "草酸电解液");
        var svc = CreateService(ctx);

        var hit = await svc.GetAllAsync(new QueryParams { PageIndex = 1, PageSize = 20, Keyword = "草酸电解" });
    ...
    public async Task UpdateAsync_部分字段更新_未提供字段保持原值()
    {
        ...
        var dto = await svc.UpdateAsync(e.Id, new UpdateIntergranularCorrosionTestRequest
        {
            InspectionDate = new DateTime(2026, 3, 2),
            FurnaceNo = "FUR-1-NEW",
            CorrosionSolution = "硫酸-硫酸铁"
        });

        dto.FurnaceNo.Should().Be("FUR-1-NEW");
        dto.InspectionDate.Should().Be(new DateTime(2026, 3, 2));
        dto.CorrosionSolution.Should().Be("硫酸-硫酸铁");
    ...
    public async Task UpdateAsync_缺失_抛业务异常()
    {
        ...
        var act = async () => await svc.UpdateAsync(99999, new UpdateIntergranularCorrosionTestRequest
        {
            InspectionDate = new DateTime(2026, 3, 1)
        });

        await act.Should().ThrowAsync<BusinessException>().WithMessage("*晶间腐蚀检验记录不存在*");
    }
    ...
    public async Task DeleteAsync_成功删除()
    {
        ...
        ctx.IntergranularCorrosionTests.Should().BeEmpty();
    }
    ...
    public async Task BatchCreateAsync_批量落库()
    {
        ...
        var list = await svc.BatchCreateAsync(new List<CreateIntergranularCorrosionTestRequest>
        {
        ...
        ctx.IntergranularCorrosionTests.Count().Should().Be(2);
    }
