...
/// <summary>
/// 压扁检验服务测试：CRUD、关键字/日期范围过滤、null 补丁更新、缺失抛业务异常、批量创建、缓存筛选上下文去重。
/// </summary>
public class FlatteningTestServiceTests : TestBase
{
    private static FlatteningTestService CreateService(AppDbContext ctx)
        => new(ctx, NullLogger<FlatteningTestService>.Instance, new MemoryCache(new MemoryCacheOptions()));

    private static async Task<FlatteningTest> SeedAsync(AppDbContext ctx, DateTime date, string furnaceNo = "FUR-1",
        string inspector = "张三", string grade = "Q345B", string specification = "219*8",
        string? observation = null, decimal? flatteningGap = null)
    {
        var e = new FlatteningTest
        {
            InspectionDate = date,
            Inspector = inspector,
            FurnaceNo = furnaceNo,
            ...
            FlatteningGap = flatteningGap
        };
        ctx.FlatteningTests.Add(e);
        await ctx.SaveChangesAsync();
        return e;
    }

    private static CreateFlatteningTestRequest NewCreate(string furnaceNo = "FUR-1") => new()
    {
    ...
    [Fact]
    public async Task GetAllAsync_关键字命中可空观察列()
    {
        var ctx = CreateDbContext();
        await SeedAsync(ctx, new DateTime(2026, 3, 1), furnaceNo: "FUR-1");
        await SeedAsync(ctx, new DateTime(2026, 3, 2), furnaceNo: "FUR-2");
        await SeedAsync(ctx, new DateTime(2026, 3, 3), furnaceNo: "FUR-3", observation: "无裂纹无分层");
        var svc = CreateService(ctx);
    ...
    public async Task UpdateAsync_缺失_抛业务异常()
    {
        ...
        var act = async () => await svc.UpdateAsync(99999, new UpdateFlatteningTestRequest
        {
            InspectionDate = new DateTime(2026, 3, 1)
        });

        await act.Should().ThrowAsync<BusinessException>().WithMessage("*压扁检验记录不存在*");
    }
    ...
    public async Task DeleteAsync_成功删除()
    {
        ...
        ctx.FlatteningTests.Should().BeEmpty();
    }
    ...
    public async Task BatchCreateAsync_批量落库()
    {
        ...
        var list = await svc.BatchCreateAsync(new List<CreateFlatteningTestRequest>
        {
