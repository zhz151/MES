...
/// <summary>
/// 硬度检验服务测试：CRUD、关键字/日期范围过滤、null 补丁更新、缺失抛业务异常、批量创建、缓存筛选上下文去重。
/// </summary>
public class HardnessTestServiceTests : TestBase
{
    private static HardnessTestService CreateService(AppDbContext ctx)
        => new(ctx, NullLogger<HardnessTestService>.Instance, new MemoryCache(new MemoryCacheOptions()));

    private static async Task<HardnessTest> SeedAsync(AppDbContext ctx, DateTime date, string furnaceNo = "FUR-1",
        string inspector = "张三", string grade = "Q345B", string specification = "219*8",
        string? hardnessMode = null, string? hardnessValue = null)
    {
        var e = new HardnessTest
        {
            InspectionDate = date,
            Inspector = inspector,
            FurnaceNo = furnaceNo,
            ...
            HardnessMode = hardnessMode,
            HardnessValue = hardnessValue
        };
        ctx.HardnessTests.Add(e);
        await ctx.SaveChangesAsync();
        return e;
    }

    private static CreateHardnessTestRequest NewCreate(string furnaceNo = "FUR-1") => new()
    {
    ...
    public async Task CreateAsync_新增_落库返回Dto()
    {
        ...
        dto.HardnessMode.Should().Be("洛氏HRB");
        dto.HardnessValue.Should().Be("80");
        var row = await ctx.HardnessTests.SingleAsync();
    ...
    [Fact]
    public async Task GetAllAsync_关键字命中硬度模式列()
    {
        var ctx = CreateDbContext();
        await SeedAsync(ctx, new DateTime(2026, 3, 1), furnaceNo: "FUR-1");
        await SeedAsync(ctx, new DateTime(2026, 3, 2), furnaceNo: "FUR-2");
        await SeedAsync(ctx, new DateTime(2026, 3, 3), furnaceNo: "FUR-3", hardnessMode: "布氏HBW");
        var svc = CreateService(ctx);

        var hit = await svc.GetAllAsync(new QueryParams { PageIndex = 1, PageSize = 20, Keyword = "布氏HBW" });
    ...
    public async Task UpdateAsync_部分字段更新_未提供字段保持原值()
    {
        ...
        var dto = await svc.UpdateAsync(e.Id, new UpdateHardnessTestRequest
        {
        ...
        dto.HardnessValue.Should().Be("85");
    ...
    public async Task UpdateAsync_缺失_抛业务异常()
    {
        ...
        var act = async () => await svc.UpdateAsync(99999, new UpdateHardnessTestRequest
        {
            InspectionDate = new DateTime(2026, 3, 1)
        });

        await act.Should().ThrowAsync<BusinessException>().WithMessage("*硬度检验记录不存在*");
    }
    ...
    public async Task DeleteAsync_成功删除()
    {
        ...
        ctx.HardnessTests.Should().BeEmpty();
    }
    ...
    public async Task BatchCreateAsync_批量落库()
    {
        ...
        var list = await svc.BatchCreateAsync(new List<CreateHardnessTestRequest>
        {
        ...
        ctx.HardnessTests.Count().Should().Be(2);
    }
