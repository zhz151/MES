...
/// <summary>
/// 点腐蚀检验服务测试：CRUD、关键字/日期范围过滤、null 补丁更新、缺失抛业务异常、批量创建、缓存筛选上下文去重。
/// </summary>
public class PittingCorrosionTestServiceTests : TestBase
{
    private static PittingCorrosionTestService CreateService(AppDbContext ctx)
        => new(ctx, NullLogger<PittingCorrosionTestService>.Instance, new MemoryCache(new MemoryCacheOptions()));

    private static async Task<PittingCorrosionTest> SeedAsync(AppDbContext ctx, DateTime date,
        string furnaceNo = "FUR-1", string inspector = "张三", string grade = "Q345B", string specification = "219*8",
        string? polishingGrade = null, decimal? rawWeight = null)
    {
        var e = new PittingCorrosionTest
        {
            InspectionDate = date,
            Inspector = inspector,
            FurnaceNo = furnaceNo,
            ...
            PolishingGrade = polishingGrade,
            RawWeight = rawWeight
        };
        ctx.PittingCorrosionTests.Add(e);
        await ctx.SaveChangesAsync();
        return e;
    }

    private static CreatePittingCorrosionTestRequest NewCreate(string furnaceNo = "FUR-1") => new()
    {
    ...
    public async Task CreateAsync_新增_落库返回Dto()
    {
        ...
        dto.PolishingGrade.Should().Be("P800");
        dto.RawWeight.Should().Be(500m);
        var row = await ctx.PittingCorrosionTests.SingleAsync();
    ...
    [Fact]
    public async Task GetAllAsync_关键字命中研磨粒度列()
    {
        var ctx = CreateDbContext();
        await SeedAsync(ctx, new DateTime(2026, 3, 1), furnaceNo: "FUR-1");
        await SeedAsync(ctx, new DateTime(2026, 3, 2), furnaceNo: "FUR-2");
        await SeedAsync(ctx, new DateTime(2026, 3, 3), furnaceNo: "FUR-3", polishingGrade: "P1200");
        var svc = CreateService(ctx);

        var hit = await svc.GetAllAsync(new QueryParams { PageIndex = 1, PageSize = 20, Keyword = "P1200" });
    ...
    public async Task UpdateAsync_部分字段更新_未提供字段保持原值()
    {
        ...
        var e = await SeedAsync(ctx, new DateTime(2026, 3, 1), furnaceNo: "FUR-1", inspector: "张三",
            polishingGrade: "P800", rawWeight: 500m);
        var svc = CreateService(ctx);

        var dto = await svc.UpdateAsync(e.Id, new UpdatePittingCorrosionTestRequest
        {
    ...
    public async Task UpdateAsync_缺失_抛业务异常()
    {
        ...
        var act = async () => await svc.UpdateAsync(99999, new UpdatePittingCorrosionTestRequest
        {
            InspectionDate = new DateTime(2026, 3, 1)
        });

        await act.Should().ThrowAsync<BusinessException>().WithMessage("*点腐蚀检验记录不存在*");
    }
    ...
    public async Task DeleteAsync_成功删除()
    {
        ...
        ctx.PittingCorrosionTests.Should().BeEmpty();
    }
    ...
    public async Task BatchCreateAsync_批量落库()
    {
        ...
        var list = await svc.BatchCreateAsync(new List<CreatePittingCorrosionTestRequest>
        {
        ...
        ctx.PittingCorrosionTests.Count().Should().Be(2);
    }
