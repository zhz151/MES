...
/// <summary>
/// 标准号检验项要求服务测试：CRUD、标准号唯一约束、缺失抛业务异常、null 补丁更新、分页关键字、筛选上下文去重。
/// </summary>
public class StandardInspectionRequirementServiceTests : TestBase
{
    private static StandardInspectionRequirementService CreateService(AppDbContext ctx) => new(ctx);

    private static async Task<StandardInspectionRequirement> SeedAsync(AppDbContext ctx, string standardNo,
        string? flaring = null)
    {
        var e = new StandardInspectionRequirement { StandardNo = standardNo, FlaringTest = flaring };
        ctx.StandardInspectionRequirements.Add(e);
        await ctx.SaveChangesAsync();
        return e;
    }

    private static CreateStandardInspectionRequirementRequest NewCreate(string standardNo = "GB/T 14976") => new()
    {
    ...
    public async Task CreateAsync_新增_落库返回Dto()
    {
        ...
        ctx.StandardInspectionRequirements.Should().ContainSingle();
    }
    ...
    public async Task GetByIdAsync_缺失_抛业务异常()
    {
        ...
        await act.Should().ThrowAsync<BusinessException>().WithMessage("*标准号检验项要求记录不存在*");
    }
    ...
    public async Task UpdateAsync_部分字段更新_未提供字段保持原值()
    {
        ...
        var dto = await svc.UpdateAsync(e.Id, new UpdateStandardInspectionRequirementRequest
        {
    ...
    public async Task UpdateAsync_重命名为他人标准号_抛业务异常()
    {
        ...
        var act = async () => await svc.UpdateAsync(a.Id, new UpdateStandardInspectionRequirementRequest
        {
            StandardNo = "STD-B"
        });
    ...
    public async Task UpdateAsync_缺失_抛业务异常()
    {
        ...
        var act = async () => await svc.UpdateAsync(99999, new UpdateStandardInspectionRequirementRequest());
    ...
    public async Task DeleteAsync_成功删除()
    {
        ...
        ctx.StandardInspectionRequirements.Should().BeEmpty();
    }
    ...
    public async Task GetFilterContextsAsync_返回去重非空上下文()
    {
        ...
        contexts["FlaringTest"].Should().Equal("扩口不裂");
