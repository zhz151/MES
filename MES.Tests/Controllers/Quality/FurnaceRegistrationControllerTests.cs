...
public class FurnaceRegistrationControllerTests : ControllerTestBase
{
    ...
    public async Task GetAll_PassesSortBy_ToService()
    {
        ...
        // Act
        await _controller.GetAll(sortBy: "RawMaterialUnit", isDescending: false);

        // Assert
        _serviceMock.Verify(x => x.GetAllAsync(It.Is<QueryParams>(q => q.SortBy == "RawMaterialUnit" && q.IsDescending == false)), Times.Once);
    }

    [Fact]
    public async Task GetAll_DefaultSortBy_IsFurnaceNumber()
    {
        // Arrange
        _serviceMock.Setup(x => x.GetAllAsync(It.IsAny<QueryParams>()))
            .ReturnsAsync(new PagedResult<FurnaceRegistrationDto> { Items = new List<FurnaceRegistrationDto>() });
        ...
        // Assert
        _serviceMock.Verify(x => x.GetAllAsync(It.Is<QueryParams>(q => q.SortBy == "furnacenumber")), Times.Once);
    }
    ...
    public async Task GetAll_PassesFiltersJson_ToService()
    {
        ...
        var filtersJson = "[{\"Field\":\"RawMaterialUnit\",\"Operator\":\"contains\",\"Value\":\"test\"}]";
        ...
        _serviceMock.Verify(x => x.GetAllAsync(It.Is<QueryParams>(q =>
            q.Filters != null && q.Filters.Count == 1 && q.Filters[0].Field == "RawMaterialUnit")), Times.Once);
    }
