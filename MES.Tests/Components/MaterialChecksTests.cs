using Bunit;
using FluentAssertions;
using MES.Core.DTOs;
using MES.Core.Models;
using MES.Blazor.Pages;
using MES.Blazor.Services;

namespace MES.Tests.Components;

public class MaterialChecksTests : TestBase
{
    public MaterialChecksTests()
    {
        RegisterServices(typeof(ProductionRecordService));
        ConfigureEmptyResponse("/api/production-record/all/material-checks");
    }

    [Fact]
    public void Render_HasTitle()
    {
        var cut = Ctx.RenderComponent<MaterialChecks>();
        cut.Markup.Should().Contain("检验到料");
    }

    [Fact]
    public void Render_HasFilter()
    {
        var cut = Ctx.RenderComponent<MaterialChecks>();
        cut.Markup.Should().Contain("模糊搜索");
    }

    [Fact]
    public void Render_DisplaysData()
    {
        ConfigureListResponse();
        var cut = Ctx.RenderComponent<MaterialChecks>();
        cut.WaitForState(() => cut.Markup.Contains("BATCH-MC-001"));
        cut.Markup.Should().Contain("BATCH-MC-001");
    }

    private void ConfigureListResponse()
    {
        ConfigureEmptyResponse("/api/production-record/all/material-checks");
        var pagedResult = new PagedResult<MaterialReceiveCheckDto>
        {
            Items = new List<MaterialReceiveCheckDto>
            {
                new()
                {
                    Id = 1,
                    ProductionBatchId = 1,
                    BatchNo = "BATCH-MC-001",
                    ReceiveDate = DateTime.Today,
                    ReceivedQuantity = 50,
                    ReceivedWeight = 2000m,
                    Checker = "张三"
                }
            },
            TotalCount = 1,
            PageIndex = 1,
            PageSize = 20
        };
        ConfigureResponse("/api/production-record/all/material-checks", new ApiResponse<PagedResult<MaterialReceiveCheckDto>>
        {
            Success = true,
            Code = 200,
            Data = pagedResult
        });
    }
}
